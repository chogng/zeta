...
| `app::input_surface::ComposerMode` | private | 记录 Session 输入位置当前显示的具体交互，并机械委托高度、绘制和输入 | 不保存 ChatInput completion，不解释 能力内部多步页面 |
...
└─ loop
   ...
   └─ terminal event
      ...
      └─ Paste → App::handle_paste
         ...
         ├─ `ComposerMode` 有值 → current surface ListSelection/TextPrompt input
