...
`/resume`、`/rewind`、`/add-dir`、`/fork`、`/model`、`/theme` 和 `/new` 支持行内参数；产品命令拒绝图片参数。命令回显和结果始终更新同一正文单元，输出格式见[布局规格](../docs/spec/layout.md)。
...
通用导航、搜索和返回行为见[面板键表](../docs/spec/interaction.md#每个命令面板)。下面只列会影响请求实现的区别。
