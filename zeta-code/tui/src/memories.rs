...
use zeta_app_server_protocol::protocol::memory::MemoryScopeDescriptor;
...
pub(crate) fn execute<T: JsonRpcTransport>(
    ...
    let run = || -> Result<Page, zeta_app_server_client::ClientError> {
        match command {
            ...
            Command::Citation(reference) => {
                let citation = memories::MemoryCitation::parse(&reference).map_err(|error| {
                    zeta_app_server_client::ClientError::Protocol(error.to_string())
                })?;
                client
...
fn explain(error: zeta_app_server_client::ClientError) -> String {
    match error {
        zeta_app_server_client::ClientError::Server { code: -32133, .. } => {
            "This memory changed. Refresh and select it again; your draft is kept.".into()
        }
        zeta_app_server_client::ClientError::Server { code: -32134, .. } => {
            "The memory list changed. Refresh the list.".into()
        }
        zeta_app_server_client::ClientError::Server { code: -32131, .. } => {
            "This memory was deleted. Refresh the list.".into()
        }
        zeta_app_server_client::ClientError::Server { code: -32602, .. } => {
