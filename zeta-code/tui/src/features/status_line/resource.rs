...
/// Host-local, product-scoped status-line preferences for the Zeta Code TUI.
pub(crate) struct StatusLineResource {
    path: PathBuf,
...
impl StatusLineResource {
    pub(crate) fn new(path: PathBuf) -> Self {
        Self {
            path,
            observed: None,
            settings: StatusLineSettings::default(),
            revision: 0,
        }
    }

    pub(crate) fn refresh(&mut self) -> Result<StatusLineSettings, String> {
    ...
    pub(crate) fn apply_edit(
        ...
        if self.observed.as_ref() != Some(&current) {
            return Err(
                "status-line settings changed on disk; reopen /statusline and try again".into(),
            );
        }
        ...
        let mut contents = serde_json::to_vec_pretty(&settings)
            .map_err(|error| format!("could not serialize status-line settings: {error}"))?;
        contents.push(b'\n');
...
fn settings_from_snapshot(snapshot: &ResourceSnapshot) -> Result<StatusLineSettings, String> {
    match snapshot {
        ResourceSnapshot::Missing => Ok(StatusLineSettings::default()),
