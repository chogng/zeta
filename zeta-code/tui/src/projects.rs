...
impl Command {
    pub(crate) const fn request_name(&self) -> &'static str {
        match self {
            Self::OpenRoots => "zeta-tui-list-project-roots",
            Self::OpenAddRoot => "zeta-tui-open-project-root-editor",
            Self::AddRoot { .. } => "zeta-tui-add-project-root",
        }
    }
}
...
fn dir_id<T>(
    client: &mut AppServerClient<T>,
    path: &Path,
) -> Result<zeta_file_access::DirId, String>
where
    T: JsonRpcTransport,
{
    client
