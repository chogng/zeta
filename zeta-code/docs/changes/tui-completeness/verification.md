...
| AC-27 状态与资源采样 | 有生命周期测试入口 | [采样测试](../../../tui/src/host/process_resources_tests.rs)包含按需启停；[状态页](../../../tui/src/status/panel.rs)；合计口径与不可用状态仍待验 | 尚未验证 |
...
正文重构已有新的实现提交，但本轮没有验证它是否满足独立规格；内存诊断仍为未排期草案。发现新的必要漏项时，先补规格，再在本表增加记录。
