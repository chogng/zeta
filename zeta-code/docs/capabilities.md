...
| [ChatGPT 订阅](changes/chatgpt-provider/spec.md) | Providers 可查看账户、设备码登录、取消和退出；有 Codex 时只读复用，无 Codex 时维护登录和续期，缺失时创建兼容文件 | [账户页面与请求](../tui/src/config/subscription.rs)、[验收](changes/chatgpt-provider/verification.md) |
