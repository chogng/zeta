...
命令面板无论固定高度还是按内容申请高度，都使用 `─ 标题 ─…` 这一种标题栏。`CommandPanel` 一次分配标题栏、固定空白行、可选页签区和正文区；绘制、高度计算与鼠标命中共用这组区域。标题栏独占第一行，下方固定留一行空白，页签只出现在空白行之后，正文只出现在完整页签区之后。窄宽度导致页签换行时，只增加页签区高度，不改变标题与正文的顺序。页签区和正文区共用左右各两格的内容缩进；列表的状态标记单独占用正文左侧两格，不改变文字起点。完整字符、间距与样式规则见[样式契约](styles.md#命令面板标题)。
