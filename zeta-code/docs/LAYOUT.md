...
当前命令面板由标题、空白行、可选页签和正文共同计算期望高度。`ListSelection` 按可见条目数申请列表高度并受 `MAX_VISIBLE_ROWS` 限制，搜索框和条目说明另计；Status 按所有页签的最大内容高度申请；详情浮层按内容申请并受覆盖区域限制。这些都不是固定占满终端的面板，也不能笼统称为“固定高度面板”。
...
两类会话都使用 Space 打开只读会话预览，`i` 打开“Session details”元数据详情浮层。普通会话另支持 `p` 置顶。预览复用 Session 的正文绘制和历史分页，支持方向键、PageUp/PageDown、Home/End 和滚轮；不显示可编辑输入框，不接收输入、粘贴、提交或会话修改快捷键。Esc 关闭预览，恢复原列表当前项、展开状态、焦点和草稿。当前预览读取打开时的快照，上翻时读取更早历史，不订阅实时输出；再次打开读取最新内容。
