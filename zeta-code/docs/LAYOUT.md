...
具体部位见下文页面词典；高度策略见[高度与缩放规则](#高度与缩放规则)，按键上下文由[交互语义](tui-interaction.md#按键上下文)统一规定。
...
上下键跳过静态状态标题，但会选中 Archived 标题；Enter 或 Space 切换展开，右方向键展开，左方向键收起，鼠标点击执行相同动作。选中普通会话时 Enter 打开、Ctrl+X 归档；选中归档会话时 Enter 取消归档并保留管理页面、Ctrl+X 永久删除整个会话。取消归档恢复主会话的可输入状态，不重新运行已结束的 Turn 或子 agent。
