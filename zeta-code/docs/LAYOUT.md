...
| 标记 | 当前用途 | 常见位置 |
| --- | --- | --- |
| `> ` | 用户输入或可输入位置 | 输入框、历史用户消息、已结束的本地命令 |
| `● ` | Agent、系统或正在执行的输出；颜色再表达具体状态 | 正文单元、Agents 中当前查看的 Thread |
| `○ ` | 非当前项或非活动项 | Agents 中其他 Thread |
| `❯ ` | 键盘当前会操作的选项 | 审批、提问和各种选择列表 |
| `▸ ` / `▾ ` | 分组已折叠或已展开 | Session 管理页面的分组标题 |
| 两个空格 | 此行没有独立标记，但正文仍与其他行对齐 | 多行正文的续行、未选中的列表项 |

有层级关系的内容可以把标记栏扩展为连接前缀，例如展开详情使用 `└─ `，其中 `└─` 表达它属于上一条输出，后面的空格仍负责分隔正文。Session 行也会先用缩进表达它属于上方分组，再显示自己的状态图标。这些都是有语义的扩展，不应为了强行限制在两个格内而丢掉层级信息。

输入框边线、空行以及 Goal、Plan、Queue、状态行等整行辅助信息不需要伪造标记来填满标记栏；只有需要表达角色、状态、层级或当前操作目标的内容行才使用它。当前实现由各组件分别写入前缀，尚未分配一个独立的全局 `Rect`；定位问题时仍统一称为“标记栏”。
