...
> 状态：Current layout contract。本文拥有页面部位、空间分配和覆盖关系；讨论 UI 时优先使用本文中的中文名称。
>
> 整页区域由 [`app/layout.rs`](../tui/src/app/layout.rs) 分配，绘制顺序由 [`app/frame.rs`](../tui/src/app/frame.rs) 决定。[交互文档](tui-interaction.md)拥有焦点、按键和退出规则，[样式文档](styles.md)拥有字符与颜色，[TUI 架构](tui.md)拥有状态和实现边界。
...
| --- | --- | --- | --- |
| 正常聊天 | Session 页面 | 正文占满辅助区域之外的剩余空间 | 当前焦点区域；默认是输入框 |
...
具体部位见下文页面词典；高度策略见[高度与缩放规则](#高度与缩放规则)，按键上下文由[交互语义](tui-interaction.md#按键上下文)统一规定，各界面的完整键表见[统一按键与界面键表](tui-interaction.md#统一按键与界面键表)。
...
“模态”在本文只表示当前交互独占输入，背景不执行该交互消费的按键；具体分发规则见[按键上下文](tui-interaction.md#按键上下文)。模态不等于浮层：命令面板参与纵向布局也可以独占输入；输入补全虽然是浮层，仍与输入框共同完成编辑。只读也不等于模态：Status 是只读命令面板，Session 预览是只读页面内容，二者使用不同的空间分配。
...
“固定最大高度”是内容高度的可选上限，不等于始终占用该高度。短列表不应为了填满上限而留出大块空白；长正文也不应为了保持面板外形而丢掉可用阅读空间。需要页签稳定时，可按所有页签的最大内容高度申请空间，避免切页跳动；这仍是内容高度策略。浮层也遵守这三种策略，但它的可用空间来自覆盖区域，不从背景正文扣除。

当前命令面板由标题、空白行、可选页签和正文共同计算期望高度。`ListSelection` 按过滤后的全部条目数申请列表高度，没有固定 12 行上限，搜索框和条目说明另计；Status 按所有页签的最大内容高度申请；详情浮层按内容申请并受覆盖区域限制。这些都不是固定占满终端的面板，也不能笼统称为“固定高度面板”。
...
页面负责最终矩形，组件负责期望高度与内部滚动。标题、页签、正文和操作提示必须共用同一份布局结果进行绘制和鼠标命中；不能绘制时临时多挤一行。极小终端可能放不下完整组件，此时各区域裁剪到非负高度，不能覆盖底部或越界；“至少 4 行”以可用空间足够为前提。
...
有层级关系的内容可以把标记栏扩展为连接前缀，例如展开详情使用 `└─ `，其中 `└─` 表达它属于上一条输出，后面的空格仍负责分隔正文。Session 管理页面使用同级分组标题，下面的 Session 行在标记栏之后显示状态图标；标题与内容的横向错位表达归属。“静态”或“可折叠”只是标题的交互属性，不表示层级。Idle、Working 等状态组标题为静态；Archived 与它们左对齐，只显示名称与数量，不添加展开、收起符号。Archived 标题获得焦点时使用统一的当前项样式，不增加前置缩进；操作提示按当前状态显示 `Enter to expand` 或 `Enter to collapse`。这些都是有语义的层级表达，不应为了强行限制在两个格内而丢掉层级信息。
...
代码中的 `ChatPanel` 负责 Session 页面底部整块聊天交互区：固定内容包括 `TopTip` 和 `StatusLine`，按状态出现的内容包括 `ChatInput`、`Approval`、`Query` 和 `CommandPanel`。“固定”和“临时”只描述显示周期，不再各自形成一种容器类型；页面上的具体内容仍使用下表名称。
...
命令面板使用 `─ 标题 ─…` 这一种标题栏。`CommandPanel` 一次分配标题栏、固定空白行、可选页签区和正文区；绘制、高度计算与鼠标命中共用这组区域。标题栏独占第一行，下方固定留一行空白，页签只出现在空白行之后，正文只出现在完整页签区之后。窄宽度导致页签换行时，只增加页签区高度，不改变标题与正文的顺序。页签区和正文区共用左右各两格的内容缩进；列表的状态标记单独占用正文左侧两格，不改变文字起点。完整字符、间距与样式规则见[样式契约](styles.md#命令面板标题)。
...
`/status` 打开的是输入位置中的 Status 面板（代码类型为 `StatusPanel`），不是详情浮层。Thread 页显示模型、上下文窗口、当前 Thread 的累计模型调用数、输入 token、缓存读取 token、缓存读取占比、缓存写入 token、输出 token、推理输出 token、累计参考费用，以及 Session/Thread 身份。Processes 页显示本机进程合计的当前常驻内存、当前连续观测期间的内存峰值、1 分钟与 5 分钟内存变化、CPU 占用，以及 TUI 与本地 App Server 的明细；本地 App Server 先显示包含全部子进程的合计，再显示主进程和按父子层级排列的子进程。子进程使用操作系统进程名和 PID 展示，不根据命令参数猜测 LSP 或工具角色。远程 App Server 明确标为不计入本机合计。

Tab 和 Shift-Tab 循环切换页签，当前页签复用统一的交互强调样式，不在文字中添加方括号；每个页签独立保留滚动位置。空间足够时面板按所有页签中最大的完整内容高度展开，因此切换 Thread 与 Processes 不会让输入位置上下跳动；空间不足时至少保留 4 行正文，并用方向键、PageUp/PageDown、Home/End 滚动。缓存占比是 `缓存读取 token / 总输入 token`，不是请求次数的命中率。聚合缺少部分报告时，已知非零 token 以 `>=` 标记为下界，已知费用以 `≥` 标记为下界；没有可信值时显示 `unknown`。CPU 是占本机逻辑 CPU 总容量的比例；首次采样显示 `collecting`，系统不支持或读取失败时显示 `unavailable`。
...
| 横向部位 | 宽度与位置 | 内容与边界 |
| --- | --- | --- |
| 分组标题 | 独占整行，从最左侧开始 | 显示状态组名称和 Session 数量；状态组是静态标签；Archived 标题可被键盘选中或鼠标点击；展开状态由子项是否可见和当前操作提示表达 |
...
分组顺序为 Pinned、Needs input、Working、Ready for review、Failed、Stopped、Completed、Idle、Archived。空状态组不显示；Archived 始终显示数量，初次打开默认收起，在当前 TUI 运行期间保留展开状态。归档会话只出现在 Archived，不同时出现在状态组或 Pinned。

上下键或 k/j 跳过静态状态标题，但会选中 Archived 标题；Enter 或 Space 切换展开，右方向键展开，左方向键收起，鼠标点击执行相同动作。选中普通会话时 Enter 打开、Ctrl+X 归档；选中归档会话时 Enter 取消归档并保留管理页面、Ctrl+X 永久删除整个会话。取消归档恢复主会话的可输入状态，不重新运行已结束的 Turn 或子 agent。

两类会话都使用 Space 打开只读会话预览，`i` 打开“Session details”会话状态详情浮层，复用 `DetailOverlay` 与 `DetailList` 展示全部 Thread 的层级、执行状态、等待原因、归档状态及委派结果，打开期间持续刷新。普通会话另支持 `p` 置顶。预览复用 Session 的正文绘制和历史分页，支持方向键、PageUp/PageDown、Home/End 和滚轮；不显示可编辑输入框，不接收输入、粘贴、提交或会话修改快捷键。Esc 关闭预览，恢复原列表当前项、展开状态、焦点和草稿。当前预览读取打开时的快照，上翻时读取更早历史，不订阅实时输出；再次打开读取最新内容。
...
不要把审批面板、提问面板叫“弹窗”：它们会参与普通纵向布局。只有覆盖在页面之上的 `DetailOverlay` 和输入补全浮层才叫浮层。三种补全是同一个输入补全浮层的互斥内容，不是三个不同浮层；描述具体问题时使用“Slash Command 补全浮层”“Mention 补全浮层”或“Skill 补全浮层”。
