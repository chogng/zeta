...
输入框边线、空行以及 Goal、Plan、未聚焦的 Queue、状态行等整行辅助信息不需要伪造标记来填满标记栏；Queue 获得键盘焦点后，当前项使用 `>` 表达操作目标。标记栏不是独立布局区域，由各组件按上述前缀和对齐规则分别绘制；定位问题时仍统一称为“标记栏”。
...
| Queue 区 | `queue` | 输入在当前 Turn 结束后再发送时，显示排队消息；Alt-Up 或鼠标单击进入选择，普通输入框不属于这里 |
...
命令面板无论固定高度还是按内容申请高度，都使用 `─ 标题 ─…` 这一种标题栏。标题栏下固定留一行空白，包含页签栏的正文紧接着从下一行开始；完整字符、间距与样式规则见[样式契约](styles.md#命令面板标题)。
...
因此，剪贴板图片提示、`← for agents` 和 `shift+tab to cycle policy` 都属于顶部提示；输入框下方的权限模式、模型和 Git 信息才属于状态行。这里没有轮播：权限提示由首次进入对话或切换权限策略触发，剪贴板图片提示由启动和窗口重新获得焦点时的检测触发。临时通知、剪贴板图片提示和权限提示都在距最后一次触发 5 秒后直接消失，重复触发会重新计时；图片粘贴成功时还会立即清除剪贴板提示。
...
Tab 和 Shift-Tab 循环切换页签，当前页签复用统一的交互强调样式，不在文字中添加方括号；每个页签独立保留滚动位置。空间足够时面板按当前页完整内容展开；空间不足时至少保留 4 行正文，并用方向键、PageUp/PageDown、Home/End 滚动。缓存占比是 `缓存读取 token / 总输入 token`，不是请求次数的命中率。聚合缺少部分报告时，已知非零 token 以 `>=` 标记为下界，已知费用以 `≥` 标记为下界；没有可信值时显示 `unknown`。CPU 是占本机逻辑 CPU 总容量的比例；首次采样显示 `collecting`，系统不支持或读取失败时显示 `unavailable`。

- 避免压缩`to`在hit中例如 `Esc to close`。
