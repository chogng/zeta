...
从[功能总览](capabilities.md)找功能，再进入对应规格。查某次为什么改、验证到哪一步，使用[变更记录](changes/)。
...
| [changes/](changes/) | 某次为什么改、改了哪些要求、实际验证了什么？ | 记录一次工作；完成后保留，后续独立工作另建记录 |
...
2. 在[变更目录](changes/)保存本次工作记录，注明对应的长期规格与设计。
