...
本文规定页面部位、空间分配和正文输出，后半部分提供终端文字示例。按键行为见[交互规格](interaction.md)，颜色与字符规则见[样式规格](styles.md)。
...
“模态”在本文只表示当前交互独占输入，背景不执行该交互消费的按键；具体分发规则见[按键上下文](interaction.md#按键上下文)。模态不等于浮层：命令面板参与纵向布局也可以独占输入；输入补全虽然是浮层，仍与输入框共同完成编辑。只读也不等于模态：Status 是只读命令面板，Session 预览是只读页面内容，二者使用不同的空间分配。
...
有层级关系的内容可以把标记栏扩展为连接前缀，例如展开详情使用 `└─ `，表示下一行属于上一条输出。Session 管理页面的各分组标题处于同一层级，只显示名称和数量，不增加展开符号；当前标题仍使用统一的标记栏。标题和会话行的缩进表达归属，可折叠行为见[交互契约](interaction.md#session-管理列表的动作)。
...
分组顺序为 Pinned、Needs input、Working、Ready for review、Failed、Stopped、Completed、Idle、Archived。空状态组不显示；Archived 始终显示数量。归档会话只出现在 Archived，不同时出现在其他状态组或 Pinned。

所有分组的键盘导航、展开收起、会话打开、预览、归档、恢复和删除统一见[会话管理动作](interaction.md#session-管理列表的动作)。会话预览使用正文区，Session details 使用详情浮层；二者的数据读取与刷新边界由[交互契约](interaction.md#session-管理列表的动作)和[详情架构](../design/tui.md#会话切换预览与详情)说明，本布局文档不维护第二套键表。
...
以上示例只解释可见行和内容归属。字符与颜色映射统一见[正文样式](styles.md#正文样式)，展开和完整详情的键盘动作见[交互键表](interaction.md#页面辅助区域与浮层)。正文不属于当前增强鼠标捕获范围，不能从示例中的动作文字推断它具有鼠标点击入口。
