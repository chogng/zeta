...
正文状态只保存一份按顺序排列的 `TranscriptCell` 集合，但绘制边界必须把它分成互斥的两段：首个实时单元之前是已定稿前缀，从首个实时单元开始是当前可变尾部。`TranscriptHistory` 只把已定稿前缀写入终端回滚区，普通跟随最新画面只绘制当前可变尾部；用户进入正文浏览后才从同一集合组合完整正文。这样既不复制正文，也不会让同一个定稿单元同时出现在回滚区和普通画面。`terminal/session.rs` 逐个借用主屏幕顶行写入定稿单元，写入后恢复原顶行，不清空或滚动输入框、补全、命令面板和状态栏。恢复旧会话时，`ThreadSubscription::start` 自动读完历史分页，按从旧到新的顺序安装正文。
