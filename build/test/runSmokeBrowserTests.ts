...
if (mode !== "disconnected" && mode !== "full") {
  throw new Error("Usage: node build/test/runSmokeBrowserTests.ts <disconnected|full>");
}

if (mode === "full") {
  const preparation = await run(process.execPath, ["../build/desktop/prepareDevPackage.ts", "--javascript-runtime", "packaged-node"], process.env);
  if (preparation !== 0) {
...
const serverEnvironment = mode === "full"
  ...
  : testEnvironment;
const server = spawn(process.execPath, ["node_modules/vite/bin/vite.js", "--config", "../build/vite/vite.config.ts", "--force"], {
  cwd: process.cwd(),
