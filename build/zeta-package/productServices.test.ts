...
const official = { name: "zeta", trustedRoot: "marketplace-root.json" };
...
test("validates every configured Marketplace trust root in the package", async () => {
  const root = await mkdtemp(join(tmpdir(), "zeta-product-services-"));
  try {
...
test("rejects trust roots reached through a symbolic directory", { skip: process.platform === "win32" }, async () => {
  const parent = await mkdtemp(join(tmpdir(), "zeta-product-services-"));
