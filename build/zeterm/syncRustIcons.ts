...
interface RustIconOptions {
  readonly mode?: "check" | "generate";
  readonly outputFile?: string;
  readonly sourceDirectory?: string;
}

interface RustArtwork {
  readonly name: string;
  readonly resourceName: string;
}

function rustConstantName(fileName: string): string {
  return basename(fileName, ".svg").replaceAll("-", "_").toUpperCase();
}

function renderingConstructor(svg: string): "multicolor" | "symbolic" {
...
export async function syncRustIcons(options: RustIconOptions = {}): Promise<{ readonly count: number; readonly outputFile: string }> {
  ...
  if (mode === "check") {
    ...
    if (current !== generated) {
      throw new Error("Generated Rust product icons are stale. Run 'node build/zeterm/syncRustIcons.ts'.");
    }
