...
if [ -n "${ZETERM_BINARY:-}" ]; then
    python3 -B "${script_dir}/build_zeterm_package.py" "$@" --zeterm-bin "${ZETERM_BINARY}"
else
    python3 -B "${script_dir}/build_zeterm_package.py" "$@"
fi

python3 -B "${script_dir}/sign_zeterm_package.py" \
    --package-dir "${ZETERM_PACKAGE_DIR}" \
    --platform "${ZETERM_PLATFORM}"
python3 -B "${script_dir}/verify_zeterm_package.py" \
    --package-dir "${ZETERM_PACKAGE_DIR}" \
    --platform "${ZETERM_PLATFORM}"
