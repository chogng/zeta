...
const headingReplacements: ReadonlyArray<readonly [RegExp, string]> = [
...
function walkReadmes(directory: string): string[] {
...
function checkDocument(path: string): string[] {
  const failures: string[] = [];
  const source = readFileSync(path, "utf8").replace(/\r\n/g, "\n");
  const lines = source.split("\n");
  let fenced = false;
  let topLevelHeadings = 0;
  let firstContentLine: { index: number; value: string } | null = null;
  let changed = false;
