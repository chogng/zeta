...
`ash-prompts` 隔离共享提示词资产与渲染依赖，供根 Agent、子 Agent 和共享流程使用。
...
验证：`just test ash-prompts`、`just check ash-prompts`；真实上下文组装与恢复由 `just test ash-core` 覆盖。编译资源由 `BUILD.bazel` 的 `templates/**` 清单维护。
