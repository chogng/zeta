...
`ash-instructions` 对一个目录的 `.ash/instructions/*.md` 执行有界、非递归发现，校验
...
| 文件状态 | 结果 |
| --- | --- |
| `.ash/instructions` 不存在 | 空 catalog，无错误 |
| 合法 `.md` | 进入确定性 catalog |
...
```bash
cargo test -p ash-instructions
cargo clippy -p ash-instructions --all-targets --no-deps -- -D warnings
```
