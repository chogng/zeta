...
fn parses_utf8_match_ranges_as_utf16_offsets() {
    ...
    assert_eq!(
        parsed.ranges,
        [ContentSearchMatchRange { start: 10, end: 16 }]
    );
}

#[test]
fn builds_typed_ripgrep_arguments_without_shell_parsing() {
    let arguments = search_arguments(&ContentSearchQuery {
        query: "needle".into(),
        pattern: ContentSearchPattern::Literal,
...
fn rejects_globs_that_can_invert_or_escape_the_typed_filter() {
    for pattern in ["!src/**", "../outside", "/absolute", "C:/absolute"] {
        assert_eq!(
            ...
            Err(ContentSearchError::InvalidInput)
        );
    }
}
...
fn result_limit_is_reported_only_when_an_additional_match_exists() {
    ...
    let exact_state = Arc::new(Mutex::new(ContentSearchJobState::default()));
    ...
    let overflow_state = Arc::new(Mutex::new(ContentSearchJobState::default()));
