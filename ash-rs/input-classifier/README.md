...
InputClassifier::classify
  ...
  → ShellContext::analyze
      ash-shell-completion::ShellCompletionEngine::analyze
...
不应把 PTY 或命令执行运行时移进 `ash-input-classifier` 或 `ash-shell-completion`。
...
路由顺序与失败语义仍由 `ash-input-classifier` 拥有。
