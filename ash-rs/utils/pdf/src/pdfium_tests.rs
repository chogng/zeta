...
fn missing_runtime_library_is_reported_before_binding() {
    let runtime = PdfiumRuntime::from_bundled_root("/definitely-missing-ash-pdfium");
    assert!(matches!(
...
fn extracts_text_with_a_release_staged_runtime_when_configured() {
    let Some(runtime_root) = env::var_os("ASH_PDFIUM_ROOT") else {
        return;
    };
    let Some(document_path) = env::var_os("ASH_PDFIUM_TEST_DOCUMENT") else {
        return;
    };
