...
`ash-typst` 把调用方提供的一段 Typst 源码编译为 PDF。它拥有 Typst `World` 实现，明确不
...
`typst-assets` 的 `fonts` 功能提供当前固定字体集。`ash-typst` 仍受仓库根目录 `LICENSE`
...
Desktop 发布包还必须携带 `ash-ts/THIRD_PARTY_NOTICES.md`，并从本目录的权威文件把对应许可证材料复制到发布 staging。源码树不保存第二份副本；改变 Typst 版本、资源功能或字体来源时，必须重新审查许可证材料并检查确定性输出。

## 测试与修改影响

```text
cargo test -p ash-typst
```
