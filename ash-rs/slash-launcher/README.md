...
> [`ash-slash-commands`](../slash-commands/README.md) 拥有。

`ash-slash-launcher` 把产品选择的若干列表组合成一个可搜索、可选择的无渲染快照。它不认识
...
```bash
cargo test -p ash-slash-launcher
cargo clippy -p ash-slash-launcher --all-targets -- -D warnings
```
