...
> [`ash-code/tui/README.md`](../../ash-code/tui/README.md) 维护。模型侧的文件内容搜索通过
> Agent 的文件内容搜索由 App Server 的 `grep` Tool 完成，编辑器工作区搜索见
> [`ash-rs/content-search/README.md`](../content-search/README.md)；可执行文件的
> discovery 与冻结边界见 [`ash-rs/shell-command/README.md`](../shell-command/README.md)。

`ash-file-search` 只拥有 directory-relative 文件路径的后台索引、增量 fuzzy matching 和独立
...
已经漂移；这些职责分别属于 Agent grep/编辑器 Search、Tool registry 和 `ash-tui`。
