...
pub(super) fn root() -> Result<PathBuf> {
    Ok(win::program_data()?.join(format!("Ash-Sandbox-{}", win::current_user()?)))
}
...
fn checked_state() -> Result<(PathBuf, State, Vec<win::Handle>)> {
    ...
    if hash(&runner)? != state.runner_hash {
        return Err("the Ash runtime executable changed; setup is required".into());
    }
    state
        .rules
        .as_ref()
...
pub(super) fn setup(slots: usize, approved: &str) -> Result<()> {
    ...
    if root.try_exists().map_err(|error| error.to_string())? {
        ...
        println!("Ash user runtime is already provisioned.");
        return Ok(());
    }
...
pub(super) fn remove(approved: &str) -> Result<()> {
    ...
    println!("Verified removal of the Ash execution accounts, network rules, and runtime files.");
    Ok(())
}
