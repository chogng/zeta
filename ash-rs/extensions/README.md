...
`ash-extensions` 接收产品 host 选择的可信根以及 authority provider 选择的 exact immutable package
...
cargo clippy --manifest-path Cargo.toml -p ash-extensions --all-targets --no-deps -- -D warnings
```
