...
生产环境的 ChatGPT 与 Kimi 订阅实现分别位于 `ash-chatgpt` 和 `ash-kimi`，都在本机执行 device OAuth、refresh 并使用 SecretStore。本 crate 保持供应商无关，也不依赖这些适配器。这里的测试使用 fake driver，使身份、取消、版本和事件语义不依赖任何供应商网络实现。
