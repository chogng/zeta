...
description: Review new commits in the checked-out VS Code repository since the recorded checkpoint, decide which changes affect Ash, and record review and alignment progress without modifying product source. Use only when the user explicitly invokes $vscode-change-review in the current request; never activate it from task similarity, schedules, prior context, or agent delegation.
---
...
比较 `../vscode` 两个提交之间的真实改动，找出可能需要应用到 Ash 的变化。该 skill 只读取 VS Code 和 Ash 源码，只允许更新自身的 [checkpoint.json](checkpoint.json)；不得修改、生成、删除或格式化任何产品源码。

## 两个检查点

- `reviewed`：已经逐提交、逐差异审查到的 VS Code 提交。
- `aligned`：用户已经确认相关 Ash 改动全部完成或无需应用的 VS Code 提交。
...
对范围内每个提交读取完整补丁、提交说明和相关测试，不只看文件名或提交标题。批量按 Ash 对应层归组，然后逐组确认：

- 公开 API、职责归属、生命周期、调用顺序、副作用、错误语义或可观察行为是否变化；
- Ash 是否存在对应文件、API、调用方和测试；
- 变化应应用到 Ash、无需应用，还是需要用户决定；
- 后续实现应使用哪些 Ash 路径和定向测试。
...
- 需要应用的变化写入 `pending`，包含稳定 `id`、引入提交、VS Code 路径、Ash 路径、`apply` 或 `decision` 类型以及简短原因；
