...
description: Refactor or retire project files by comparing TypeScript responsibilities with VS Code and Rust responsibilities with Codex, then aligning local ownership, callers, tests, and documentation. Do not use VS Code as the reference for Rust files.
---
...
The local implementation has one canonical owner corresponding to the relevant upstream responsibility, and all callers, tests, build metadata, and current architecture documentation point to that owner. When the user asked for retirement, remove the original file; when the user asked only for refactoring, preserve files that still have a legitimate responsibility.
...
3. Select the reference repository from the target language before searching upstream. Inspect the matching upstream responsibility by exports, callers, lifecycle and ownership role, side effects, and neighboring modules. Do not edit an upstream tree unless the user explicitly asks for a cross-repository change.
4. Locate the local counterpart semantically; a matching basename alone is insufficient. For TypeScript, use established mappings such as `src/vs/...` to `zeta-ts/src/zeta/...` when repository evidence supports them. For Rust, derive the mapping from Codex crate and module responsibilities rather than VS Code layout.
...
- Compare responsibilities, not just filenames. If the selected upstream has no safe local counterpart, or two local files plausibly own the same responsibility, stop and explain the evidence instead of guessing.
