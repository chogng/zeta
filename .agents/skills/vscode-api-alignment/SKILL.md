...
当 Zeta 的 TypeScript 编辑器实现对应 VS Code 的模块、视图部件或公共契约时，以 `../vscode` 为只读参照，以 `zeta-ts/src/zeta/editor` 及其必要调用方为主要修改范围。目标是用 Zeta 独立编写的实现建立可以沿调用链定位问题的真实对应关系，不能复制、粘贴、批量同步或逐行翻译上游代码，也不能把通过类型检查本身当作完成。对齐过程必须让生产文件图单调收敛：仅本地文件只能减少，新增生产文件必须能在 VS Code 中找到大小写一致的同相对路径文件。
...
- 内部实现与测试必须由 Zeta 独立编写。现有 Zeta owner 若承担必要且合法的基础或产品职责，应在同一正确 owner 中实现 VS Code 契约，并以真实生产调用和行为测试证明语义一致；不得复制、粘贴、批量同步、逐行翻译或轻微改写上游实现、注释和测试代码。
- VS Code 的内部设计不是质量上限。满足对应 owner、生命周期、调用顺序、副作用、错误语义和可观察行为后，优先采用更简单、更清晰且适合 Zeta 的数据结构、算法与控制流；不得为了形似上游引入不必要的状态、间接层、分支或抽象，也不得以简化为由改变文件拆分、公开契约或职责边界。
- 读取上游文件只用于提取文件图、公开契约、职责、调用顺序、副作用、错误语义和可观察行为。对应文件没有 Zeta 专属职责时，也必须依据这些约束重新实现，不能把上游文件内容当作起点或模板。
- 文件名、路径、API 名称和结构一致不构成复制实现的许可。若发现仓库已有上游实质代码，保留其现有版权与许可证声明，记录来源并停止继续扩散；不得通过删除许可证头把复制内容变成本地实现。
...
- 不得复制、粘贴、批量同步、逐行翻译或轻微改写 VS Code 的实现、注释和测试代码；不得先复制再重构、改名或格式化来制造独立实现。
...
* 对齐 VS Code 时，只把上游用于确认文件图、公开契约、职责和可观察行为；Zeta 的实现与测试必须独立编写，禁止复制、粘贴、批量同步或逐行翻译上游代码。文件名、路径和 API 名称一致不构成实现复制的许可。

* VS Code 是文件图、公开契约、职责和可观察行为的参照，不是内部设计上限；在保持对应 owner、生命周期、调用顺序、副作用和错误语义一致的前提下，优先采用更简单、更清晰且适合 Zeta 的独立实现，不得为了模仿上游保留不必要的复杂度。
