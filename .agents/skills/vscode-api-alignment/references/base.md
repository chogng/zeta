...
仅在目标位于 `zeta-ts/src/zeta/base` 时读取。共同的范围分类、单一实现、删除确认和验证规则由主 `SKILL.md` 负责。
...
- 仅本地能力只有在确实通用、没有服务依赖并有真实跨领域调用方时才能归为 Zeta 产品切片。带有编辑器、工作台或产品语义的能力应回到更高层，不能为保留本地文件而抽象成 `base` API。
