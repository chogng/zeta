...
本 crate 把 `zeta-workbench::{TabPart, TabGroup, TabInput}` 投影为横向或纵向界面。它负责 Workbench 交互标识、无障碍节点、`TabContainer`、`TabContainerToolbar`、`Titlebar`、`WorkbenchUiStyle` 和可调整宽度的 `TabContainerState`。
...
| 文件 | 职责 |
| --- | --- |
| `workbench/tabs.rs` | 把 Workbench 分组和输入投影成横向或纵向 Tab 界面。 |
