...
pub(super) const PANEL_WIDTH: f32 = 720.0;
pub(super) const PANEL_HEIGHT: f32 = 470.0;
pub(super) const PANEL_MARGIN: f32 = 20.0;
pub(super) const CONTENT_INSET: f32 = 22.0;
...
pub(super) fn list_bounds(panel: Rect) -> Rect {
    Rect::from_xywh(
...
pub(super) fn form_left(panel: Rect) -> f32 {
    let list = list_bounds(panel);
    list.right() + COLUMN_GAP
}

pub(super) fn input_bounds(panel: Rect, field: RemoteConnectionManagerField) -> Rect {
    let top = match field {
...
pub(super) fn status_bounds(panel: Rect) -> Rect {
    let left = form_left(panel);
    Rect::from_xywh(
        left,
...
pub(super) fn new_bounds(panel: Rect) -> Rect {
    let list = list_bounds(panel);
    Rect::from_xywh(
        list.origin.x,
        panel.bottom() - 50.0,
...
pub(super) fn delete_bounds(panel: Rect) -> Rect {
    Rect::from_xywh(
        form_left(panel),
        panel.bottom() - 50.0,
        120.0,
...
pub(super) fn connect_bounds(panel: Rect) -> Rect {
    Rect::from_xywh(
...
pub(super) fn list_button_style(palette: ShellPalette) -> ButtonStyle {
...
pub(super) fn action_button_style(palette: ShellPalette, primary: bool) -> ButtonStyle {
    let base = if primary {
        palette.accent
    } else {
        palette.surface_raised
