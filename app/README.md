...
`app` 是与 `zeta` Electron Desktop 和 `zeta code` TUI 同级的纯 Rust Desktop 产品入口，在发布边界导出为 `app`。三条产品线的宿主边界见 [`docs/product-lines.md`](../docs/product-lines.md)。

`app` 按 Warp 的 app/crate 方式组织：`app` package 本身是产品 app library，
[`src/bin/app.rs`](src/bin/app.rs) 只负责进程入口并调用 [`src/lib.rs`](src/lib.rs) 的
`run()`；产品组合和生命周期留在 app crate 内。`zeta-rs` 是共享核心 crate workspace，提供
App Server、协议、Session、Terminal、Remote 等能力。`zui`、`zeta-ui` 和
`features/workspace` 等是 UI/功能模块，不反向依赖产品 App Server。

产品模块按职责位于 app crate 内：
...
状态字段只对该模块及其 feature host 子模块可见，crate root 的同名路径只是兼容性 re-export，
不再提供 crate-wide 的字段访问。
...
启动时 `zeta-theme` 从共享 design-token manifest、`app` 默认主题入口与 device-local 用户主题生成不可变 snapshot；
`shell_style` 将其适配为 Shell、CodeEditor、MultiDiffEditor、scrollbar 与 terminal ANSI palette。
`zeta-workbench` 拥有 Tab/Pane 逻辑状态，`zeta-workbench-layout` 拥有 Workbench/Pane 的结构几何，`zeta-workbench-host` 拥有通用 binding 协调；`zeta-ui` 只拥有
可复用组件。`zeta-composer` 拥有 Composer
...
窗口级产品 pane topology 位于 [`zeta-workbench-layout`](workbench-layout/README.md)；通用框架可脱离产品运行的
...
`app` 通过根 workspace 的 path dependencies 使用 shared backend；`zeta-rs` 不反向依赖 app 或
Native UI。Bazel 从同一个根 Cargo graph 生成 `@crates`，并以 `//app:app`、
`//app:app_sources`、`//app:app_release_inputs` 和
`//app:app_ci` 拥有产品 target、输入、发布契约与 boundary check。app 仍是独立产品目录和
发布入口，但不再是第二个 workspace 根；
当前完整 app binary 的 macOS 验证仍受 apple-cf Swift bridge 在沙箱中启动 SwiftPM 的环境限制，
Cargo workspace check/test 是产品源码的首要验证入口。发布 staging 入口是：

```bash
just app-package --package-dir /absolute/path/to/app-package --app-bin /path/to/app
bazel test //app:app_ci
```

平台 release job 使用 `APP_PACKAGE_DIR`、`APP_PLATFORM` 以及对应的 signing identity 环境
调用 `build/release/release_app_package.sh`；该入口完成签名和验签后才留下 `verified` artifact。

package staging 只生成 unsigned artifact、binary digest 和可选的 binary-bound Remote runtime
bundle；签名、平台验证与发布顺序见
[`app` release graph](docs/app-release-graph.md)。

若发布包要支持“本机只安装 app，远端尚未安装 zeta”，先把各 POSIX target 的 canonical
packaged-node Zeta package 组装成确定性 Remote runtime bundle，再在构建 app 时绑定它：
...
package builder 会把 catalog SHA-256 编译进 app binary，再把 catalog 与 archive 放到
`zeta-remote-runtimes/`。staging 和 signing 都会确认 binary 实际包含该摘要；catalog 再以每个
archive 的大小、展开大小和 SHA-256 绑定远端代码。使用预构建 `--app-bin` 时，该 binary 也必须
已用同一 `APP_REMOTE_RUNTIME_CATALOG_SHA256` 编译，否则 staging 拒绝。release script 可通过
`APP_REMOTE_RUNTIME_BUNDLE` 接入同一 bundle。

不希望把所有 POSIX runtime 装进 app 产品包时，发布 job 可只绑定已发布 catalog：
...
builder 会把 URL 和摘要都通过 `APP_REMOTE_RUNTIME_CATALOG_URL` /
`APP_REMOTE_RUNTIME_CATALOG_SHA256` 编译进 binary；staging 和 signing 验证两者确实存在于签名
...
`app` 是产品宿主，不是第二个 UI framework。它保留窗口、平台事件、产品状态映射、App Server
...
| `ShellPresentation` 的 scene/interaction frame 聚合 | `UiFrame<InteractionFrame>` | frame 是唯一 owner；accessibility 在提交时派生，renderer、input 和 Inspector 通过窄 accessor 消费 |
...
因此不能把 `app` binary 标成 deprecated：它是当前产品宿主。旧 split scene/interaction
...
| 能力 | 当前 owner | 状态 |
| --- | --- | --- |
| `app` 发布名与用户可见显示 | `PRODUCT_DISPLAY_NAME` / Cargo `[[bin]]` | ✅ |
| 产品窗口标题、初始尺寸与 event meaning | `app::NativeApp` | ✅；实现 `zui::app::App` |
| Application lifecycle、window registry、renderer initialization 与 event routing | `zui` | 委托；一次性 `ready`、重复 `resumed`、window-all-closed、phase、exit reason、retained-handle control command 与异步窗口创建由 shared lifecycle core 统一 |
| Event loop 与 native window adapter | `zui::app` + private `window` integration | 委托；`WindowHandle` 不延长 runtime window lifecycle，可跨 callback 请求 close、position/center、window level、尺寸约束和 pointer/window gesture，关闭或平台不支持时返回显式 operation error |
...
| Shell product layout 与 scene composition | `zeta-workbench-layout` + `zeta-composer` + `shell_scene` / `composer_panel` | 分层；布局 crate 只解析 Workbench 结构几何，`zeta-composer` 是 Composer 状态与几何的单一 owner，产品宿主仍拥有 scene composition |
...
| Changed-file collection、整体滚动与每文件 diff 视口 | [`features/workspace`](src/features/workspace/workspace_panes.rs) 的 `ScmState` / `EditorPaneState` / `zeta-ui::ScrollState` | 委托；app 只把 Git 投影映射为 `ScmDiff` 并执行刷新动作 |
| Titlebar 背景、窗口拖拽区、语言服务器设置入口与左右侧栏开关 | `titlebar::Titlebar` | ✅；不绘制可见窗口标题 |
| Top Bar 的 Tab Container 与 Workspace Pane toggle `ActionBar` | `titlebar::Titlebar` | ✅ |
...
| NavBar 导航容器（计划） | `zeta-ui` presentation composition + Native Titlebar/body `TabContainer` host | 尚未作为独立 public component；只计划承载横向/纵向排列、slot 与 overflow/scroll geometry，不拥有 TabInput 或 provider state |
...
| Session Tab 右键菜单、关闭策略与 action 映射 | `session::session_context_menu::SessionContextMenu` / `SessionContextMenuState` | ✅；Close 会请求 App Server Stop 并清理对应 Tab、Pane 与 runtime，右键目标按 `TabInputKey` 解析；Pin/Rename/Fork mutation 尚未执行 |
| 共享主题加载与平台中立 snapshot | [`zeta-theme`](../zeta-rs/theme/README.md) | 委托；选择数据化 `app` 默认入口，并与 Desktop/TUI 消费同一 manifest 和用户主题 JSON |
...
| 稳定 product command identity、request 与注册式执行入口 | `zeta-commands::{AppCommandId, CommandRequest, CommandRegistry}` / `NativeApp::dispatch_command` | ✅；pointer、menu 与 shortcut 汇合到宿主注册的 handler |
...
| 快捷键设置、录制与 Chord 提示 | [`app-keybinding-ui`](keybinding-ui/README.md) | 委托；app 提供命令行、事件 adapter 与保存接线 |
...
| accessibility semantics → 平台屏幕阅读器 | `zui` private AccessKit adapter | 委托；现有 tree/focus/action 由 `present_frame` 从同一 interaction frame 派生并发布 |
...
| Session Tab 到 terminal runtime 的一对一 binding、活动/非活动 runtime 切换 | `workbench::terminal_workspace::TerminalWorkspace` | ✅；Local/Remote 共用 pending key、乱序 ready 和非活动 runtime 管理，不拥有 App Server Session/Thread authority |
...
| App Server host、Session adapter 与 transient Thread/language projection / stream-gap recovery | `app_server::AppServerHost` / `agent_session`（含 `agent_session::remote`） / `language_service_remote` / `language_service_remote_session` / `thread_projection` | 部分具备；App Server 是 Agent、Language、Terminal 共用的横向 backend contract，`Local`/`Remote` 只是连接方式；本地 profile/Workspace authority 与 SSH Remote 均通过同一 host 选择；Remote Agent 与独立 Remote language connection 均在 30 秒窗口内退避重连，断线期命令和语言请求不会延迟回放 |
...
```text
app → zui::{app, window, input, ui, runtime, services, render}
            → zeta-ui → zui
            → zeta-workbench-host
                ├─ zeta-workbench
                └─ zeta-workbench-layout → zui
            → zeta-commands
            → zeta-keybinding
            → app-keybinding-ui → zeta-keybinding
                                  → zeta-ui → zui
            → app_server::AppServerHost
                ├─ Local → zeta-app-server-client → zeta-server-host / app-server-daemon
                └─ Remote → zeta-remote → zeta-remote-connections → zeta-app-server-client
...
`app` 可以拥有可丢弃的 presentation state 和产品交互，但不能复制 Session、Thread、
...
| 当前源码 | 当前能力 | 目标产品语义 | 状态 |
| --- | --- | --- | --- |
| `titlebar::Titlebar` | 窗口拖拽区、横向 `TabContainer`、Tab Container toggle 与 Workspace Pane toggle `ActionBar`；不绘制独立标题文案 | Top Bar | ✅ |
| `zeta-workbench-layout::WorkbenchLayout` | 用结构布局 spec 解析 titlebar、Tab Container、main 和可选 Inspector bounds | 窗口根布局 | ✅；返回 immutable snapshot，产品宿主负责 scene composition |
| `ShellLayout` | 组合 titlebar、可选 Tab Container，并把剩余区域交给 `WorkspaceLayout` | Top Bar 与 Workspace 外部布局 | ✅；Tab Container 使用外层单轴 split |
| `zeta-workbench-layout::PaneGroupLayout` / `zui::GridLayout` | 用递归 Grid 投影 Workbench `PaneNode` 的每个 Pane 与 owning-split Sash | Workspace Pane geometry adapter | ✅；PaneTree mutation、Session-to-PTY binding 与逐 Pane runtime 仍由产品 host 负责 |
| `zeta-commands` / `command_dispatch` / `keybindings` / `keybindings_resource` / `keyboard_shortcuts` | 把 pointer/menu 的 `ElementId` 与标准化键盘事件映射到同一 `CommandRequest`，再交给 `CommandRegistry` 的宿主 handler；向 `app-keybinding-ui` 提供命令行、稳定 identity 和保存 adapter | Product command 与快捷键输入层 | ✅；支持完整 `when` 表达式、冲突/错误诊断、最多四段 Chord 与 keycap UI |
...
| `features/workspace::files::{FilesLayout, FilesToolbar, FilesState, FilesPane}` | Files 自己拥有 36px 功能 toolbar、Refresh、ahead/behind、Search、文件树与搜索结果；Native 只通过 App Server 适配 `DirectoryEntry` | Files Pane | ✅；pointer/Enter/Space 产生 feature action，由宿主加载目录或打开文件 |
| `features/workspace::scm::{ScmLayout, ScmState, EditorPaneState, EditorPane}` | SCM 自己拥有 Changes toolbar slot、changed-file snapshot、language-aware `DiffEditorDocument`、整体滚动位置、scrollbar pointer capture/animation、每文件 `DiffEditorState` 和 `MultiDiffEditorLayout` | Changes Pane | ✅；Native 只提供 `ScmDiff` 和主题投影 |
...
| multi-session projection / switching | App Server Session/Thread 的创建、动态 tab projection 和切换已接通；每个 Session Tab 绑定独立本地 PTY，切换时保留各自 TerminalCore 与 shell 进程 | 多会话入口 | 已实现；Session/Thread/用户消息与 PaneGroup 不跨 app 重启恢复，Pin/Rename/Fork action mutation 仍是当前限制 |

CodeEditor/DiffEditor/MultiDiffEditor 的实现 ownership、`DiffSideRows` 投影、显示列 contract、
测试和当前限制由 [`zeta-editor` README](editor/README.md) 维护。app 负责 changed-file
...
当前已用 `zui::SplitViewLayout` 与 `zeta-ui::Sash` 支持 Tab Container/Main 的单轴 resize，并由
`zeta-workbench-layout` 通过 `zui::GridLayout` 解析 Workbench 的递归几何。`zeta-workbench-host`
为每个 Session Tab 协调通用 binding；产品 host 继续保存 active Pane、独立 `TerminalSession`/scroll/selection 状态和 split command。窗口 resize 与 Sash 调整会按每个 Pane 的 logical viewport 计算 rows/columns，并分别同步对应 terminal grid 和 PTY。
...
main
  ...
  → NativeApp::window_event
      ...
      → AppServerHost::start → zeta-app-server-client::AppServerSession
          → AgentSessionEvent::Snapshot/Update → TabPart/TerminalWorkspace binding + ThreadProjection
      ...
      → Tab Container Sash pointer
          → TabContainerState drag snapshot
      ...
      → zeta_workbench_layout::WorkspaceLayout
          → WorkbenchLayout + PaneGroupLayout → active main PaneGroup + optional file-editor Inspector bounds
...
`InspectorPartState` 保存文件编辑器 Inspector 的显隐、preferred width 与当前
`SplitViewResizeSnapshot`，并向外层 Grid 提供右侧 Sash 所需的 pane sizing；宽度限制为
360–800px，始终为 main Pane 保留至少 400px。`WorkspaceLayout` 解析 Inspector
bounds，当前 Session 的 `PaneGroup` 再把主区域投影给 active Pane；`files::FilesLayout` 和
`scm::ScmLayout` 分别把各自的功能 bounds 解析为 36px toolbar slot 与 active content pane，
跨功能选择由 host 的 PaneInput binding 驱动。
`files::FilesState` 保存文件搜索、树与滚动，`scm::ScmState` 保存 changed-file snapshot 和
`EditorPaneState`。Native 的
`WorkspacePaneHost` 只负责将 App Server/`WorkspaceContext` 快照适配为 crate 类型，并执行
...
每个 Session Tab 也已绑定独立 PTY。Close 会通过权威 Session Stop contract 执行，并清理本地
Tab、Pane 和 runtime；Pin、Rename、Fork 仍未执行，不得由 presentation state 伪造结果。
...
`ShellPresentation` 只保存单一 `UiFrame<InteractionFrame>`，不再缓存第二份 accessibility projection。
`WindowContext::present_frame` 会在绘制前从该 frame 与当前 `UiDispatch` 派生现有树、bounds、focus、
selection 和 expansion，再通过 ZUI 私有 AccessKit adapter 发布。
...
只有从该 frame 解析出的 `UiScene` 被传给 `Renderer`，因此 GPU backend 不承担 hit-test、focus、
...
`ComputedElement` 并自动建立 parent chain。`TabContainer`、`ComposerPanel` 这类非组件 composition
函数以及 `ContextView::draw` 这类 content-closure surface 使用 `UiScene::with_element` 进入相同管线；
产品代码不再手写检查元数据。
Native 当前所有拥有稳定 bounds 的 Component 均已接入，包括 Titlebar、Tab Container Toolbar、Workspace Pane Toolbar、
...
状态，不丢弃用户改动。Changes Button 会刷新 Git projection，并把当前 Session 的普通 Pane
切换为 Changes Pane；environment picker 尚未接入。
...
Sessions 搜索区域当前真实链路
是 `TabContainer → TabContainerToolbar → SearchBox → InputBox`；不存在单独的 Header 组件。
...
```text
app --remote <openssh-host> --workspace <absolute-remote-path> \
...
```text
app remote save <name> --host <openssh-host> --workspace <absolute-remote-path> [--replace]
app remote list
app remote connect <name> [--runtime <remote-zeta-executable>] [--ssh <local-ssh-executable>]
app remote tunnel <name> --remote-port <port> [--local-port <port>] [--ssh <local-ssh-executable>]
...
运行中的 app 点击底部 `Local/Remote` location 按钮，或调用可绑定命令
`workbench.action.pickExecutionLocation`，会从同一目录打开可搜索的 Native modal picker。选择连接后
Native host 用当前 app executable 启动 `remote connect <name>` 新进程；它不经过 shell、不向 UI
...
`remote tunnel` 是 app 对共享 host-side Tunnel primitive 的前台 CLI 消费者。它只从命名连接读取
...
app 的 `remote_tunnel_process::NativeRemoteTunnelHost` adapter 从 `AppServerHost` 复用该窗口
...
关闭面板不停止 Tunnel；再次打开可查看或逐项 Stop，关闭 app 窗口则由 Native host owner 收掉全部
...
若 runtime 缺失或 schema 明确不兼容，正式 app
...
启动窗口。因此本机只安装 app、远端没有预装 `zeta-server`，或只安装了旧版 `zeta-server`，也能连接。source build
...
并使用服务端旋转后的 token 作为下一次恢复凭据。app 已能从 release-bound 本地或网络
...
app 的整体 Remote 状态见 [`docs/remote-development.md`](../docs/remote-development.md)。
