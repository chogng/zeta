use super::{ThreadState, ThreadUpdateResult};
use zeta_protocol::{
...
fn snapshot_items_are_followed_by_transient_agent_text() {
    ...
    state.replace_snapshot(thread_snapshot(4));
...
fn stream_gap_discards_transient_state_and_requires_resubscribe() {
    ...
    assert_eq!(
        state.apply_update(update(4, 1, started.clone())),
        ThreadUpdateResult::Applied
    );
    assert_eq!(
        state.apply_update(update(4, 3, started)),
        ThreadUpdateResult::ResubscribeRequired
    );
    assert_eq!(state.items().count(), 1);
}
...
fn update_for_another_thread_is_ignored() {
    ...
    assert_eq!(state.apply_update(envelope), ThreadUpdateResult::Ignored);
    assert_eq!(state.items().count(), 1);
}
...
fn snapshot_exposes_the_latest_durable_plan() {
    ...
    assert_eq!(
        state.plan().and_then(|plan| plan.explanation.as_deref()),
...
fn typed_tool_output_streams_remain_separate_until_the_durable_result() {
    ...
    assert_eq!(state.tool_output(&call_id), Some(("out", "err")));
}
