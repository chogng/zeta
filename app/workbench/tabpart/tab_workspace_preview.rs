//! Hover summary for the Workspace roots attached to one Workbench tab.
...
pub(super) fn workspace_preview_id(tab: ElementId) -> ElementId {
    ElementId::scoped(PREVIEW_SCOPE, mounted_element_local_id(tab))
}

pub(super) fn workspace_preview_disclosure_id(tab: ElementId) -> ElementId {
    ElementId::scoped(DISCLOSURE_SCOPE, mounted_element_local_id(tab))
}

pub(super) fn workspace_preview_scroll_id(tab: ElementId) -> ElementId {
    ElementId::scoped(SCROLL_SCOPE, mounted_element_local_id(tab))
}

fn workspace_preview_scroll_backward_id(tab: ElementId) -> ElementId {
    ElementId::scoped(SCROLL_BACKWARD_SCOPE, mounted_element_local_id(tab))
}

pub(super) fn workspace_preview_scroll_forward_id(tab: ElementId) -> ElementId {
...
pub(super) struct TabWorkspacePreview<'a> {
    viewport: Rect,
    anchor: Rect,
...
impl Component for TabWorkspacePreview<'_> {
    fn element(&self) -> ComponentElement {
        Element::leaf("TabWorkspacePreviewHost").in_bounds(self.anchor)
    }

    fn compose(&self, context: &mut ComponentContext<'_, '_>, _element: &ComputedElement) {
        let disclosure_id = workspace_preview_disclosure_id(self.tab.id);
        let expanded = self.dispatch.is_expanded(disclosure_id);
        let root_count = self.tab.workspace_roots.len().max(1);
        ...
        preview.draw_components(context, |context, content_bounds| {
            context.draw_component(
                &InteractionRegion::new(
                    "TabWorkspacePreview",
                    workspace_preview_id(self.tab.id),
                    preview.bounds(),
                    AccessibilityRole::Group,
                    format!("Workspace summary for {}", self.tab.name),
                )
            ...
            let scroll_id = workspace_preview_scroll_id(self.tab.id);
            context.draw_component(
                &InteractionRegion::new(
                    "TabWorkspacePreviewRoots",
                    scroll_id,
                    rows_bounds,
                    AccessibilityRole::List,
                    "Workspace directories",
                )
                .with_parent(workspace_preview_id(self.tab.id)),
            );
            ...
            scroll_view.draw_components(context, |context, viewport| {
                ...
                for index in first..last {
                    let label = self
                        ...
                        .unwrap_or_else(|| self.tab.workspace.into());
            ...
            if has_disclosure {
                ...
                if scroll_needed {
                    ...
                    let backward_id = workspace_preview_scroll_backward_id(self.tab.id);
                    ...
                    if backward_enabled {
                        context.draw_component(
                            &InteractionRegion::new(
                                "TabWorkspacePreviewScrollBackward",
                                backward_id,
                                backward_bounds,
                                AccessibilityRole::Button,
                                "Scroll workspace directories up",
                            )
                            .with_parent(workspace_preview_id(self.tab.id))
                            .with_cursor(CursorFeedback::Pointer)
                            .with_focus(FocusBehavior::TabStop)
                    ...
                    let forward_id = workspace_preview_scroll_forward_id(self.tab.id);
                    ...
                    if forward_enabled {
                        context.draw_component(
                            &InteractionRegion::new(
                                "TabWorkspacePreviewScrollForward",
                                forward_id,
                                forward_bounds,
                                AccessibilityRole::Button,
                                "Scroll workspace directories down",
                            )
                            .with_parent(workspace_preview_id(self.tab.id))
                            .with_cursor(CursorFeedback::Pointer)
                            .with_focus(FocusBehavior::TabStop)
