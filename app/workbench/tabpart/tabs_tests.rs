...
fn tab_container_resolves_stable_ui_identity_for_each_tab_input() {
    ...
    assert_eq!(
        tab_input_element_id(&part, Some(&first_key)),
        FIRST_TAB_CONTAINER_SESSION_TAB
    );
    assert_eq!(
        tab_input_element_id(&part, Some(&second_key)),
...
fn body_mount_arranges_tabs_vertically_with_two_line_session_information() {
    ...
    let container =
        TabContainer::from_tab_part(bounds, &part, Some(&first_key), test_style(), &dispatch);
    ...
    let inspected_search = frame
        ...
        .expect("tab container header should expose the search input hierarchy");
    assert_eq!(
        frame
            .scene()
            .inspection()
    ...
    assert_eq!(
        frame.scene().text_blocks()[1].style().weight(),
        FontWeight::Bold
    );
    assert_eq!(
        frame.scene().text_blocks()[2].style().color(),
        Color::rgb(38, 38, 41)
    );
}
...
fn body_mount_scrolls_overflowing_tabs_inside_its_viewport() {
    ...
    let first = mounted_tab_element_id(&part, &keys[0]).unwrap();
    let last = mounted_tab_element_id(&part, &keys[7]).unwrap();
    let dispatch = UiDispatch::default();
    let bounds = Rect::from_xywh(0.0, 0.0, 220.0, 180.0);
    let initial =
        TabContainer::from_tab_part(bounds, &part, Some(&keys[0]), test_style(), &dispatch);
...
fn tab_actions_button_is_hidden_at_rest_and_opens_actions_for_the_stable_tab_key() {
    ...
    let mut dispatch = UiDispatch::default();
    let container =
        TabContainer::from_tab_part(bounds, &part, Some(&second_key), test_style(), &dispatch);
    ...
    assert_eq!(
        ...
        Some(TabIntent::OpenActions(first_key))
    );
    assert_eq!(
        tab_intent_for_element(&part, TAB_CONTAINER_SETTINGS_ACTION),
...
fn dirs_preview_expands_all_roots_and_resets_after_hover_ends() {
    ...
    let initial_container =
        TabContainer::from_tab_part(bounds, &part, Some(&key), test_style(), &dispatch)
