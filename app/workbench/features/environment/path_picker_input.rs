...
impl ProductApp {
    pub(super) fn toggle_path_picker(&mut self) {
        if self.path_picker.is_open() {
        ...
        if let Err(error) = self.path_picker.open(
            anchor,
        ...
        self.rebuild_and_focus_path_search();
    }

    pub(super) fn activate_path_picker_element(&mut self, id: ElementId) -> bool {
        let Some(index) = self.path_picker.item_index(id) else {
            return false;
        };
        let activation = match self.path_picker.activate(index) {
    ...
    pub(super) fn route_path_picker_pointer_move(&mut self, point: Point) -> bool {
        if !self.path_picker.is_open() {
            return false;
        }
        let outcome =
    ...
    pub(super) fn route_path_picker_button(
        &mut self,
        state: ElementState,
        button: MouseButton,
    ) -> bool {
        ...
        match state {
            ElementState::Pressed
                if target.is_some_and(|id| self.path_picker.is_picker_element(id)) =>
            {
    ...
    pub(super) fn route_path_picker_wheel(&mut self, delta: MouseScrollDelta) -> bool {
        if !self.path_picker.is_open() {
            return false;
        }
        let Some(metrics) = self
            .presentation
            .as_ref()
            .and_then(|presentation| presentation.path_picker_scroll_metrics)
        else {
            return true;
        };
        if self
            .path_picker
            .apply_scroll(path_picker_scroll_command(delta), metrics)
        {
            self.project_path_picker_hover_after_scroll();
            self.rebuild_overlay_on_next_redraw();
        }
        true
    }

    fn project_path_picker_hover_after_scroll(&mut self) {
        ...
        let Some(viewport) = presentation.path_picker_item_viewport else {
            return;
        };
        ...
        let content_y =
            point.y - viewport.origin.y + self.path_picker.scroll_state().vertical_offset();
        let index = (content_y / PICKER_ITEM_HEIGHT).floor() as usize;
        self.ui_dispatch
            .hover_element(path_item_id(index), presentation.interaction_frame());
    }

    pub(super) fn route_path_picker_keyboard(&mut self, event: &KeyEvent) -> bool {
        if !self.path_picker.is_open() {
            return false;
        }
        let Some(presentation) = self.presentation.as_ref() else {
        ...
        if self.ui_dispatch.is_focused(DIRECTORY_SEARCH_INPUT) {
            match &event.logical_key {
                ...
                Key::Named(NamedKey::ArrowDown) => {
                    ...
                    self.apply_path_picker_navigation(outcome);
                }
                ...
                Key::Named(NamedKey::Enter) => {
                    if let Some(id) = self.path_picker.first_action_id() {
                        self.activate_path_picker_element(id);
                    }
                }
                Key::Character(text)
                    if is_shortcut(self.modifiers) && text.eq_ignore_ascii_case("c") =>
                {
                    if let Some(text) = self.path_picker.selected_search_text()
                    ...
                    if is_shortcut(self.modifiers) && text.eq_ignore_ascii_case("v") =>
                {
                    ...
                    self.path_search_changed();
                }
                _ => {
                    if let Some(command) =
                        crate::terminal_input::text_input_command(event, self.modifiers)
                    {
                        self.path_picker.apply_search(command);
                        self.path_search_changed();
                    }
                }
            }
    ...
    pub(super) fn dismiss_path_picker(&mut self) -> bool {
        if !self.path_picker.is_open() {
            return false;
        }
        let restore_focus = self.path_picker.dismiss();
    ...
    fn rebuild_and_focus_path_search(&mut self) {
        self.rebuild_presentation();
        let focus_outcome = self
            .presentation
            .as_ref()
    ...
    fn path_search_changed(&mut self) {
    ...
    fn apply_path_picker_navigation(&mut self, outcome: DispatchOutcome) {
        self.apply_dispatch_outcome(outcome);
        let Some(index) = self
            .ui_dispatch
            .focused()
            .and_then(|id| self.path_picker.item_index(id))
        else {
            return;
        };
        let Some(metrics) = self
        ...
        if self.path_picker.ensure_item_visible(index, metrics) {
            self.rebuild_presentation();
            self.request_redraw();
        }
    }
...
fn path_picker_scroll_command(delta: MouseScrollDelta) -> ScrollCommand {
