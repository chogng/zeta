...
/// Result of applying one App Server Thread update to feature state.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ThreadUpdateResult {
    Applied,
    Ignored,
    ResubscribeRequired,
}

/// Rebuildable state for one canonical Agent Thread.
///
/// Durable state is supplied by the App Server session adapter's authoritative snapshot/update
/// handoff. This state only owns transient streaming text and presentation ordering between
...
impl ThreadState {
    pub fn replace_snapshot(&mut self, thread: Thread) {
    ...
    pub fn plan(&self) -> Option<&PlanUpdate> {
        self.thread.as_ref()?.turns.last()?.plan.as_ref()
    }

    pub fn items(&self) -> impl Iterator<Item = &ThreadItem> {
        self.thread
            .iter()
    ...
    pub fn tool_output(&self, tool_call_id: &ToolCallId) -> Option<(&str, &str)> {
        self.transient_tool_outputs
            .get(tool_call_id)
            .map(|output| (output.stdout.as_str(), output.stderr.as_str()))
    }

    pub fn apply_update(&mut self, envelope: ThreadUpdateEnvelope) -> ThreadUpdateResult {
        let Some(thread) = self.thread.as_ref() else {
            return ThreadUpdateResult::ResubscribeRequired;
        };
        if envelope.thread_id != thread.thread_id || envelope.session_id != thread.session_id {
            return ThreadUpdateResult::Ignored;
        }
