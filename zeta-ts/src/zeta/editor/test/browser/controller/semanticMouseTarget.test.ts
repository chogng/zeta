...
test("SemanticMouseTargetFactory preserves editor targets and classifies browser regions", () => {
	...
	assert.equal(factory.create(mouseEvent(node("line-numbers")))?.kind, SemanticMouseTargetKind.LineNumber);
	...
	assert.equal(factory.create(mouseEvent(node("stanza-editor-scrollbar-track stanza-editor-scrollbar-track-vertical")))?.kind, SemanticMouseTargetKind.Scrollbar);
	assert.equal(factory.create(mouseEvent(node("stanza-editor-zone-widget")))?.kind, SemanticMouseTargetKind.ViewZone);
	assert.equal(factory.create(mouseEvent(node("stanza-editor-content-widget")))?.kind, SemanticMouseTargetKind.Widget);
	...
	assert.equal(factory.create(mouseEvent(fingerprintedWidget))?.kind, SemanticMouseTargetKind.Widget);
	...
	assert.equal(factory.create(mouseEvent(fingerprintedOverlayWidget))?.kind, SemanticMouseTargetKind.Widget);
	...
	assert.equal(factory.create(mouseEvent(fingerprintedViewZone))?.kind, SemanticMouseTargetKind.ViewZone);

	const textTarget = factory.create(mouseEvent(node("stanza-editor-text")));
	assert.equal(textTarget?.kind, SemanticMouseTargetKind.Text);
