...
import { getStanzaDragScrollVelocity } from "../../../browser/controller/dragScrolling.js";
...
test("Pointer autoscroll velocity follows overflow on each axis", () => {
	assert.deepEqual(
		getStanzaDragScrollVelocity(bounds, {
			clientX: 200,
			clientY: 100,
		}),
		{ left: 0, top: 0 },
...
test("Pointer autoscroll velocity validates bounds and points", () => {
	assert.throws(() => getStanzaDragScrollVelocity(
		{ ...bounds, right: 50 },
