...
test("Textarea routes navigation, typing, history, deletion, and Tab", () => {
	...
	input.textArea!.dispatchEvent(beforeInput(dom.window, "historyUndo"));
	assert.deepEqual({
		text: model.getText(),
		selection: selections.selections.primary,
	}, {
	...
	input.textArea!.dispatchEvent(beforeInput(dom.window, "historyRedo"));
	...
	input.textArea!.dispatchEvent(tab);
	...
	input.textArea!.dispatchEvent(composing);
	assert.equal(composing.defaultPrevented, false);
	assert.equal(model.getText(), "abXY\n\ncd!");
	const paste = beforeInput(dom.window, "insertFromPaste", "paste");
	input.textArea!.dispatchEvent(paste);
	assert.equal(paste.defaultPrevented, false);

	input.textArea!.value = "transient";
	input.textArea!.dispatchEvent(new dom.window.InputEvent("input", {
		bubbles: true,
		inputType: "insertText",
		data: "transient",
	}));
	assert.equal(input.textArea!.value, "");

	input.textArea!.blur();
	assert.equal(viewport.element.classList.contains("input-focused"), false);
	input.dispose();
	assert.equal(viewport.element.contains(input.textArea!), false);
	const disposedText = model.getText();
	input.textArea!.dispatchEvent(beforeInput(dom.window, "insertText", "Z"));
...
test("Textarea keyboard fallback routes undo and redo without browser history input events", () => {
	...
	input.textArea!.dispatchEvent(undo);
	...
	input.textArea!.dispatchEvent(redo);
...
test("Textarea mirrors the focused document and primary selection for assistive technology", async () => {
	...
	assert.equal(input.textArea!.getAttribute("aria-multiline"), "true");
	assert.equal(input.textArea!.value, "alpha\nbeta");
	...
	await Promise.resolve();
	assert.equal(input.textArea!.getAttribute("aria-description"), "2 selections. Primary at line 2, column 3.");
