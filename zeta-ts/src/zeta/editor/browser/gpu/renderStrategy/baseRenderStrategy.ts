...
export abstract class BaseRenderStrategy extends Disposable implements IGpuRenderStrategy {
	...
	constructor(public readonly glyphRasterizer: IGpuRenderStrategy['glyphRasterizer']) { super(); }
	...
	protected createFrame(input: GpuRenderStrategyInput, lineIndexes: Iterable<number>): GpuRenderFrame {
		...
		for (const visualLineIndex of lineIndexes) {
			...
			for (let index = 0; index < text.length; index += 1) {
				...
				if (segment.segment === '\t') {
					const space = input.atlas.getGlyph(this.glyphRasterizer, ' ', style, deviceX);
	...
	private canRenderLine(input: GpuRenderStrategyInput, text: string, tokens: readonly ResolvedSemanticToken[]): boolean {
