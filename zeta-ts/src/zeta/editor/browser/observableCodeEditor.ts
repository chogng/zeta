...
export class ObservableCodeEditor extends Disposable {
	...
	private readonly selectionsState: ObservableState<readonly Selection[]>;
	...
	public readonly selections: IObservable<readonly Selection[]>;
	...
	private constructor(editor: CodeEditorWidget) {
		...
		this.positions = derived(reader => Object.freeze(
			this.selections.read(reader).map(selection => selection.getSelectionStart()),
		));
		...
		this.cursorSelection = derived(reader => this.selections.read(reader)[0]!);
