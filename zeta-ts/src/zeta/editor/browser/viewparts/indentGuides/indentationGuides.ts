...
export function createAsterIndentationGuides(text: string, tabSize: number): readonly IndentationGuide[] {
	if (typeof text !== "string") throw new TypeError("Aster indentation guides require text");
	if (!Number.isSafeInteger(tabSize) || tabSize < 1) throw new RangeError("Aster indentation guide tab size must be a positive safe integer");
