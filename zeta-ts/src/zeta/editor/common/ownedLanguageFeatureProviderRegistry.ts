...
import { assertLanguageId, assertLanguageSelector } from "./languages/languageId.js";

/** Metadata used by Zeta's replaceable provider groups. */
export interface LanguageFeatureProviderMetadata {
	readonly languageIds: readonly string[];
	readonly providerId?: string;
}

/** One caller-owned provider set that can be replaced atomically. */
...
/** Registry for Zeta provider groups whose ownership can be replaced as one unit. */
export class OwnedLanguageFeatureProviderRegistry<TProvider extends LanguageFeatureProviderMetadata> extends Disposable {
	...
	registerGroup(providers: readonly TProvider[]): LanguageFeatureProviderRegistration<TProvider> {
		...
		const registration = toDisposable(() => {
			if (disposed) return;
			disposed = true;
			if (this.deleteOwner(owner)) this.changeEmitter.fire();
	...
	private replace(owner: object, providers: readonly TProvider[]): void {
		...
		const removed = this.deleteOwner(owner);
		for (const provider of providers) this.providers.set(this.nextProviderHandle++, { owner, provider });
		if (removed || providers.length > 0) this.changeEmitter.fire();
	}
