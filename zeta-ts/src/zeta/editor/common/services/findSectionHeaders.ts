...
export function findSectionHeaders(model: SectionHeaderFinderTarget, options: FindSectionHeaderOptions): readonly SectionHeader[] {
	...
	return Object.freeze(headers.sort((left, right) => TextPosition.compare(left.range.start, right.range.start)));
}
...
function collectRegionHeaders(model: SectionHeaderFinderTarget, markers: LanguageFoldingMarkers): SectionHeader[] {
	...
	for (let lineIndex = 0; lineIndex < model.lineCount; lineIndex += 1) {
		...
		headers.push(Object.freeze({
			range: TextRange.from(TextPosition.at(lineIndex, startColumn), TextPosition.at(lineIndex, line.length)),
			...header,
...
function collectMarkHeaders(model: SectionHeaderFinderTarget, source: string): SectionHeader[] {
	...
	while ((match = regex.exec(text)) !== null) {
		...
		if ((label || hasSeparatorLine) && (headers.at(-1)?.range.end.lineIndex ?? -1) < start.lineIndex) {
			headers.push(Object.freeze({
...
function positionAtOffset(lineStarts: readonly number[], lines: readonly string[], offset: number): TextPosition {
	let low = 0;
	...
	return TextPosition.at(low, Math.min(offset - lineStarts[low]!, lines[low]!.length));
}
