...
import { normalizeTextLineEndings, TextRange, type TextEdit, type TextPosition } from '../core/text.js';
...
export class TypeWithoutInterceptorsOperation {
	public static getEdits(model: TextModel, selections: TextSelectionSet, edits: readonly SelectionEdit[], historyMode: EditorCommandHistoryMode): EditorEditCommand {
...
export class AutoClosingOvertypeOperation {
	public static getEdits(model: TextModel, selections: TextSelectionSet, text: string): EditorEditCommand {
		...
		return TypeWithoutInterceptorsOperation.getEdits(model, selections, selections.selections.map(selection => {
			const range = selection.collapsed && graphemeCount > 0
				? TextRange.from(selection.active, advancePositionInLine(model, selection.active, graphemeCount))
				: selection.range;
...
function buildSelectionEditCommand(model: TextModel, selections: TextSelectionSet, replacements: readonly SelectionReplacement[], historyMode: EditorCommandHistoryMode): EditorEditCommand {
...
function advancePositionInLine(model: TextModel, position: TextPosition, count: number): TextPosition {
