...
registerEditorContribution({ id: "editor.contrib.codeAction", install: context => {
	...
	context.own(new CodeActionController(context.view.element, context.viewport, context.selections, service, diagnostics, context.languageId, context.options.input.resource, context.options.onApplyWorkspaceEdit, context.onLanguageError));
} });
