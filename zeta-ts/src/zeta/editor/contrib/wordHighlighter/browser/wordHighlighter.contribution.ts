...
registerEditorContribution({ id: "editor.contrib.wordHighlighter", configure: context => {
	...
	context.addDecorationSource(createStanzaDecorationSource(decorations, () => DecorationPresentation.OccurrenceHighlight));
}, install: context => {
	if (context.kind !== "text" || context.model.largeFile.tooLargeForTokenization) return;
	context.register(new OccurrenceHighlightController(context.selections, context.getCapability(TextEditorCapability.occurrenceDecorations), {
