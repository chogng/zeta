...
registerTextEditorCapabilityContribution({ id: "editor.contrib.languageNavigation", install: context => {
	...
	context.register(new LanguageNavigationController(context.view.element, context.editor, context.viewport, context.selectionController, service, context.options.input.resource, context.languageId, context.options.onOpenLocation, context.onLanguageError));
} });
