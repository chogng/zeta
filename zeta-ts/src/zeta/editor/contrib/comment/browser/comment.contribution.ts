...
registerEditorContribution({ id: "editor.contrib.comment", install: context => {
	...
	context.register(new LineCommentController(context.view.element, context.viewport, context.selections, options));
	context.register(new BlockCommentController(context.view.element, context.viewport, context.selections, options));
} });
