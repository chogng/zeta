...
class LinesOperationsContribution extends Disposable implements IEditorContribution {
	...
	transpose(): void {
		const command = createTransposeCommand(this.context.model, this.context.selectionController.getSelections());
		if (!command) return;
	...
	private onKeydown(event: KeyboardEvent): void {
		...
		if (event.key === 'Tab' && !event.ctrlKey && !event.altKey && !event.metaKey) {
			const hasRange = this.context.selectionController.getSelections().some(selection => !selection.isEmpty());
			...
			this.runCommands(
				...
				this.context.selectionController.getSelections().map(selection => new ShiftCommand(selection, {
					isUnshift: unshift,
		...
		if ((event.ctrlKey || event.metaKey) && event.shiftKey && !event.altKey && event.key.toLowerCase() === 'k') {
			stopEvent(event);
			this.run('editor.action.deleteLines', createDeleteLinesCommand(this.context.model, this.context.selectionController.getSelections()));
			return;
		}
		if ((event.ctrlKey || event.metaKey) && !event.altKey && event.key === 'Enter') {
			...
			this.run(
				...
				createInsertLineCommand(this.context.model, this.context.selectionController.getSelections(), direction),
			);
			return;
		}
		if (isJoinChord(event)) {
			stopEvent(event);
			this.run('editor.action.joinLines', createJoinLinesCommand(this.context.model, this.context.selectionController.getSelections()));
			return;
		}
		...
		if (!event.shiftKey) {
			...
			this.runCommands(
				...
				this.context.selectionController.getSelections().map(selection => new MoveLinesCommand(
					selection,
					direction === 'down',
		...
		this.runCommands(
			...
			this.context.selectionController.getSelections().map(selection => new CopyLinesCommand(selection, direction === 'down')),
		);
	}
