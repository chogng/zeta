...
registerTextEditorCapabilityContribution({ id: "editor.contrib.languageHierarchy", install: context => {
	...
	context.register(new LanguageHierarchyController(context.view.element, context.viewport, context.viewModel, service, context.options.input.resource, context.languageId, context.options.onOpenLocation, context.onLanguageError));
} });
