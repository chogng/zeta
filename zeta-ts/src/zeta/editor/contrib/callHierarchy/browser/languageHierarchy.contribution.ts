...
registerEditorContribution({ id: "editor.contrib.languageHierarchy", install: context => {
	...
	context.own(new LanguageHierarchyController(context.view.element, context.viewport, context.selections, service, context.options.input.resource, context.languageId, context.options.onOpenLocation, context.onLanguageError));
} });
