...
import { UriListPasteProvider, captureStanzaClipboardTextTransfer, normalizeStanzaClipboardPasteProviders, provideStanzaClipboardPaste, type ClipboardPasteProvider, type ClipboardTextTransfer } from "../../browser/clipboardPasteProvider.js";

test("Clipboard paste providers receive only a synchronous textual transfer snapshot", () => {
	const transfer = captureStanzaClipboardTextTransfer({
...
test("Clipboard paste providers validate identity and preserve declared precedence", async () => {
	assert.throws(() => normalizeStanzaClipboardPasteProviders([
		provider("duplicate", ["application/x-first"], () => "first"),
		provider("duplicate", ["application/x-second"], () => "second"),
	]), /Duplicate Stanza clipboard paste provider/);
	assert.throws(() => normalizeStanzaClipboardPasteProviders([
		provider("missing-types", [], () => "ignored"),
	]), /requires MIME types/);

	const providers = normalizeStanzaClipboardPasteProviders([
	...
	assert.equal(await provideStanzaClipboardPaste(providers, transfer), "provided");
});
...
test("URI-list paste omits comments and preserves stable URI order", async () => {
	...
	assert.equal(await provideStanzaClipboardPaste([UriListPasteProvider], transfer), "file:///workspace/one.rs\nhttps://example.test/two");
});
