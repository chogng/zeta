...
/**
 * Invokes the packaged `zeta-remote-server remote install` command from Electron Main.
 *
...
export class RemoteRuntimeInstaller {
	...
	async install(host: string, request: RemoteRuntimeInstallRequestOptions = {}): Promise<string> {
		...
		if (executable === undefined || !isCanonicalAbsolutePosixPath(executable) || !executable.endsWith("/bin/zeta-remote-server")) {
