	...
	try {
		...
		const provisioner = new RemoteRuntimeProvisioner({
			...
			runCommand: async (_executable, args, _environment, observer) => {
				...
				return { exitCode: 0, stdout: "/srv/zeta/remote/runtime/bin/zeta-remote-server\n", stderr: "" };
			},
		});

		assert.equal(await provisioner.install("Build-Linux", { signal: cancellation.signal }), "/srv/zeta/remote/runtime/bin/zeta-remote-server");
...
test("network provisioner fetches the authenticated target before invoking the same SSH installer", async () => {
	...
	const provisioner = new RemoteRuntimeProvisioner({
		...
		runCommand: async (_executable, args, _environment, observer) => {
			...
			return { exitCode: 0, stdout: "/srv/zeta/runtime/bin/zeta-remote-server\n", stderr: "" };
		},
	});

	assert.equal(await provisioner.install("build-linux"), "/srv/zeta/runtime/bin/zeta-remote-server");
