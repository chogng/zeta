	...
	const service = new RemoteRuntimeBootstrapMainService({
		...
		runtimeInstaller: {
			install: async (_host, options) => {
				...
				return "/opt/zeta/bin/zeta-remote-server";
			},
		},
	});
	try {
		...
		assert.equal(await provisionRuntime("build-linux"), "/opt/zeta/bin/zeta-remote-server");
...
test("Remote runtime bootstrap delegates exact profile operations", async () => {
	...
	const profiles: IRemoteRuntimeConnectionProfiles = {
		get: async (host, workspace) => {
			...
			return { activeRuntime: "/opt/zeta/active/bin/zeta-remote-server", previousRuntime: "/opt/zeta/previous/bin/zeta-remote-server" };
		},
		...
		rollback: async (host, workspace, sshExecutable) => {
			...
			return { activeRuntime: "/opt/zeta/previous/bin/zeta-remote-server" };
		},
	};
	const service = new RemoteRuntimeBootstrapMainService({
		workspace: URI.parse("zeta-remote://ssh+build-linux/workspace/project"),
		...
		remoteExecutable: "zeta",
		localEnvironment: {},
		runtimeInstaller: { install: async () => "/opt/zeta/new/bin/zeta-remote-server" },
		connectionProfiles: profiles,
	});
	try {
		assert.equal(await service.processLauncher.options.resolveRuntime?.("build-linux", "/workspace/project"), "/opt/zeta/active/bin/zeta-remote-server");
		await service.processLauncher.options.activateRuntime?.("build-linux", "/workspace/project", "/opt/zeta/new/bin/zeta-remote-server");
		assert.equal(await service.processLauncher.options.rollbackRuntime?.("build-linux", "/workspace/project", "custom-ssh"), "/opt/zeta/previous/bin/zeta-remote-server");
		assert.deepEqual(calls, [
			"get:build-linux:/workspace/project",
			"activate:build-linux:/workspace/project:/opt/zeta/new/bin/zeta-remote-server",
