...
test("Electron Main delegates installation to the shared zeta remote command", async () => {
	...
	const installer = new RemoteRuntimeInstaller({
		...
		runCommand: async (executable, args, environment) => {
			...
			return { exitCode: 0, stdout: "/srv/zeta runtime/runtimes/x86_64-unknown-linux-gnu/0.1.0/abc/bin/zeta-remote-server\n", stderr: "" };
		},
	});

	const executable = await installer.install("Build-Linux");

	assert.equal(executable, "/srv/zeta runtime/runtimes/x86_64-unknown-linux-gnu/0.1.0/abc/bin/zeta-remote-server");
	assert.deepEqual(invocation, {
		executable: "/Applications/Zeta.app/Contents/Resources/bin/zeta-remote",
		args: [ "install",
			"--host", "build-linux",
...
test("installer rejects nonzero commands and malformed receipts", async () => {
	...
	const malformed = new RemoteRuntimeInstaller({
		...
		environment: {},
		artifact,
		runCommand: async () => ({ exitCode: 0, stdout: "relative/bin/zeta-remote-server\n", stderr: "" }),
	});
