...
test('explicit and default home resolve to the same physical directory', async context => {
	const user = await mkdtemp(join(tmpdir(), 'zeta-home-'));
	context.after(() => rm(user, { recursive: true, force: true }));
	const path = join(user, '.zeta');
	await mkdir(path);
	assert.equal(resolveHome({ environment: {}, userHome: user }), realpathSync.native(path));
	assert.equal(resolveHome({ environment: { ZETA_HOME: path }, userHome: '' }), realpathSync.native(path));
});
...
test('a missing home is resolved without creating directories', async context => {
	...
	assert.equal(resolveHome({ environment: { ZETA_HOME: path }, userHome: '' }), join(realpathSync.native(user), 'new', 'data'));
	assert.equal(resolveHome({ environment: {}, userHome: user }), join(realpathSync.native(user), '.zeta'));
...
test('invalid home overrides and missing user homes fail without selecting another root', async context => {
	...
	for (const path of ['', 'relative']) {
		assert.throws(() => resolveHome({ environment: { ZETA_HOME: path }, userHome: user }), /absolute/);
	}
	...
	assert.throws(() => resolveHome({ environment: { ZETA_HOME: file }, userHome: user }), /not a directory/);
	assert.throws(() => resolveHome({ environment: { ZETA_HOME: join(file, 'data') }, userHome: user }));
...
test('retired home overrides require migration even when both variables agree', () => {
	for (const configured of [undefined, tmpdir()]) {
		assert.throws(() => resolveHome({ environment: { ZETA_PROFILE_ROOT: tmpdir(), ZETA_HOME: configured }, userHome: tmpdir() }), /remove ZETA_PROFILE_ROOT/);
	}
});
...
test('Windows homes require a drive or a complete UNC root', { skip: process.platform !== 'win32' }, () => {
	for (const path of ['\\folder', '/folder', 'C:folder', '\\\\server']) {
		assert.throws(() => resolveHome({ environment: { ZETA_HOME: path }, userHome: tmpdir() }), /absolute/);
	}
});
...
test('home resolves directory aliases and rejects dangling links', { skip: process.platform === 'win32' }, async context => {
	...
	assert.equal(resolveHome({ environment: { ZETA_HOME: join(alias, 'new') }, userHome: '' }), join(realpathSync.native(real), 'new'));
	const dangling = join(user, 'dangling');
	await symlink(join(user, 'missing'), dangling);
	assert.throws(() => resolveHome({ environment: { ZETA_HOME: dangling }, userHome: '' }));
	assert.throws(() => resolveHome({ environment: { ZETA_HOME: join(dangling, 'new') }, userHome: '' }));
});
