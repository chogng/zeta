...
class Transport implements AppServerTransport {
	...
	public send(event: string, payload?: unknown): void {
		...
		if (request.method === 'initialize') {
			...
			if (request.method === 'memory/stop') { this.status = 'stopped'; }
			if (request.method === 'memory/start') { this.status = 'recording'; }
...
test('duplicate start shares one recording and stopping prevents late evidence submission', async () => {
	...
	try {
		...
		assert.equal(transport.requests.filter(method => method === 'memory/start').length, 1);
		assert.equal(transport.requests.filter(method => method === 'memory/submit').length, 0);
...
test('stop followed immediately by start finishes cleanup before starting the next recording', async () => {
	...
	try {
		...
		assert.deepEqual(transport.requests.filter(method => method === 'memory/start' || method === 'memory/stop'), ['memory/start', 'memory/stop', 'memory/start']);
	} finally { service.dispose(); client.dispose(); }
	tracker.assertNoLeaks();
});
...
test('disposing before a queued start runs does not create backend work', async () => {
	...
	assert.equal(transport.requests.filter(method => method === 'memory/start').length, 0);
	client.dispose();
	tracker.assertNoLeaks();
});
...
test('collector failure during stop cannot leave the backend recording', async () => {
	...
	try {
		...
		assert.equal((await stopped).status, 'stopped');
		assert.ok(transport.requests.includes('memory/stop'));
	} finally { service.dispose(); client.dispose(); }
	tracker.assertNoLeaks();
});
...
test('explicit retry resumes the existing recording after a collector failure', async () => {
	...
	try {
		...
		await service.start();
		await waitFor(() => transport.requests.includes('memory/submit'));
		assert.equal(transport.requests.filter(method => method === 'memory/start').length, 1);
