...
pub struct FileSearchLimits {
    max_file_bytes: usize,
    max_results: usize,
    max_scanned_files: usize,
}

impl FileSearchLimits {
    pub fn new(
        max_file_bytes: usize,
        max_results: usize,
        max_scanned_files: usize,
        ...
        for (value, kind) in [
            ...
            if value == 0 {
                return Err(FileSearchError::InvalidLimit { kind });
            }
        }
        Ok(Self {
            max_file_bytes,
...
impl Default for FileSearchLimits {
    fn default() -> Self {
        Self {
...
/// Error raised while configuring the file-search tool.
...
impl FileSearchTool {
    pub fn new(
        ...
        Ok(Self {
            ...
            workspace,
            limits,
            definition: file_search_definition()?,
        })
    }
    ...
    fn run(&self, invocation: ToolInvocation) -> ToolExecutionOutcome {
        ...
        let input: FileSearchInput = match decode_arguments(&invocation) {
            Ok(input) => input,
        ...
        match state.search(&start, self.workspace.path()) {
            ...
            Err(SearchFailure::Cancelled) => not_started("file search was cancelled"),
            Err(SearchFailure::Io(error)) => returned_error(format!("file search failed: {error}")),
        }
    }
}
...
fn file_search_definition() -> Result<ToolDefinition, FileSearchError> {
    ToolDefinition::function(
        ToolName::new("file-search").map_err(definition_error)?,
...
fn definition_error(error: impl fmt::Display) -> FileSearchError {
    FileSearchError::Definition(error.to_string())
}
