...
| `InteractionFrame::ancestry` | public | 从命中节点投影到父节点 hover path |
| `InteractionFrame::focus_order` | public | 按注册顺序返回 `TabStop` |
...
当前没有 disabled、expanded、checked、live region、text selection range 或 accessibility
action adapter；这些状态只有在真实组件和平台 adapter 需要时才扩展。当前也没有 retained
widget tree、layout、paint、command registry 或业务 reducer。把这些能力加入本 crate 会改变
ownership，需要同步评审本 README 与 `docs/native-terminal-ui.md`。
