...
```sh
just check zeta-network-proxy --features server
just test zeta-network-proxy --features server
just test zeta-tool-executor managed_command_can_reach_only_its_authorized_proxy_destinations
just test zeta-app-server managed_network_approval_resumes_the_same_shell_process_through_rpc
```
