...
fn managed_refresh_rotates_tokens_and_preserves_omitted_fields_and_metadata() {
    ...
    let auth = runtime(home.path(), client.clone(), ChatGptAuthManagement::Zeta);
...
fn expiration_window_and_unknown_expiry_age_match_codex() {
    ...
    for (token, last, refreshes) in [
        ...
        runtime(home.path(), client.clone(), ChatGptAuthManagement::Zeta)
            .api_target()
            .unwrap();
        assert_eq!(
...
fn two_managers_share_one_refresh_and_codex_handoff_stops_new_refreshes() {
    ...
    let first = runtime(home.path(), client.clone(), ChatGptAuthManagement::Zeta);
    let second = runtime(home.path(), client.clone(), ChatGptAuthManagement::Zeta);
...
fn temporary_failure_keeps_valid_access_and_permanent_failure_allows_new_login() {
    ...
    assert!(matches!(
        ...
        zeta_login::BeginLogin::DeviceCode { .. }
    ));
...
fn recovery_reloads_new_tokens_and_never_changes_accounts() {
    ...
    let manager = AuthMaintenance::new(ChatGptAuthManagement::Zeta);
    let client = Client::new([]);
    std::fs::write(
        &path,
...
fn codex_cli_accepts_refreshed_credentials() {
    ...
    runtime(home.path(), client, ChatGptAuthManagement::Zeta)
        .api_target()
        .unwrap();
...
fn managed_logout_clears_auth_but_codex_owned_disconnect_preserves_it() {
    for mode in [ChatGptAuthManagement::Zeta, ChatGptAuthManagement::Codex] {
        let home = tempfile::tempdir().unwrap();
        let path = home.path().join("auth.json");
        ...
        assert!(client.requests.lock().unwrap().is_empty());
        if mode == ChatGptAuthManagement::Zeta {
            assert!(!path.exists());
        } else {
