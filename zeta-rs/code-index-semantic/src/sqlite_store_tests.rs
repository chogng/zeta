...
use super::SqliteCodeIndexVectorStore;
use super::metadata;
...
fn large_projection_uses_ann_candidates_and_falls_back_when_projection_is_unavailable() {
    ...
    let model = CodeIndexEmbeddingModelId::new("ann-test-v1").expect("model");
    let target = 777usize;
    let chunks = (0..ANN_MIN_CHUNKS)
        .map(|index| embedded_chunk(&root_id, index))
        .collect::<Vec<_>>();
    let query = chunks[target].embedding.clone();
    let store = SqliteCodeIndexVectorStore::open(&CodeIndexSemanticStorage::Memory).expect("store");
    store
...
fn embedded_chunk(root_id: &zeta_code_index::IndexRootId, index: usize) -> EmbeddedCodeChunk {
    EmbeddedCodeChunk {
