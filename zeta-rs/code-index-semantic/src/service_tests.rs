...
struct RecordingMetrics {
    metrics: Mutex<Vec<CodeIndexSemanticMetric>>,
}

impl CodeIndexSemanticMetricsSink for RecordingMetrics {
...
fn persistent_sqlite_projection_reopens_without_reembedding_chunks() {
    ...
    let first_store: Arc<dyn CodeIndexVectorStore> = Arc::new(
        SqliteCodeIndexVectorStore::open(&CodeIndexSemanticStorage::Persistent(path.clone()))
            .expect("open store"),
    );
    ...
    let reopened: Arc<dyn CodeIndexVectorStore> = Arc::new(
        SqliteCodeIndexVectorStore::open(&CodeIndexSemanticStorage::Persistent(path))
            .expect("reopen store"),
    );
...
fn persistent_projection_reuses_unchanged_chunks_across_lexical_generations() {
    ...
    let first = CodeIndexSemanticService::new(
        Arc::clone(&index),
        model_id(),
        Arc::clone(&embedding),
        first_store,
    ...
    let second = CodeIndexSemanticService::new(index, model_id(), embedding, reopened)
        .sync()
        .expect("reuse sync");
...
fn persistent_projection_embeds_only_changed_chunks() {
    ...
    let store: Arc<dyn CodeIndexVectorStore> = Arc::new(
        SqliteCodeIndexVectorStore::open(&CodeIndexSemanticStorage::Persistent(path))
            .expect("open store"),
    );
    let service = CodeIndexSemanticService::new(index.clone(), model_id(), embedding, store);
...
fn stale_semantic_generation_is_rejected_after_lexical_refresh() {
    ...
    assert!(matches!(
        ...
        Err(CodeIndexSemanticError::VectorStore(_))
    ));
}
...
fn sync_rejects_model_output_that_does_not_match_workspace_chunks() {
    ...
    let service = CodeIndexSemanticService::new(index, model_id(), embedding, memory_store());

    assert!(matches!(
        service.sync(),
        Err(CodeIndexSemanticError::InvalidModelResponse(
...
fn cancelled_sync_reuses_completed_batches_on_retry() {
    ...
    let service = CodeIndexSemanticService::new(index, model_id(), embedding.clone(), store);
...
fn transient_embedding_failure_is_retried_and_reported() {
    ...
    let service =
        CodeIndexSemanticService::new(index, model_id(), embedding.clone(), memory_store())
    ...
    assert!(matches!(
        ...
        [CodeIndexSemanticMetric::SyncCompleted { retry_count: 1, .. }]
    ));
}
...
) -> CodeIndexSemanticService {
    let embedding: Arc<dyn EmbeddingInvoker> = Arc::new(KeywordEmbedding);
    let service = CodeIndexSemanticService::new(index, model_id(), embedding, store);
    match rerank {
...
fn memory_store() -> Arc<dyn CodeIndexVectorStore> {
    Arc::new(InMemoryCodeIndexVectorStore::default())
}

fn model_id() -> CodeIndexEmbeddingModelId {
    CodeIndexEmbeddingModelId::new("keyword-embedding-v1").expect("model id")
}

fn query(text: &str) -> CodeIndexSemanticQuery {
    CodeIndexSemanticQuery::new(text, NonZeroUsize::new(10).expect("limit")).expect("query")
}
