...
pub struct SqliteCodeIndexVectorStore {
    connection: Mutex<Connection>,
}

impl SqliteCodeIndexVectorStore {
    pub fn open(storage: &CodeIndexSemanticStorage) -> Result<Self, CodeIndexVectorStoreError> {
        let connection = match storage {
            CodeIndexSemanticStorage::Memory => Connection::open_in_memory(),
            CodeIndexSemanticStorage::Persistent(path) => {
        ...
        connection
            ...
            .map_err(store_error)?;
        if matches!(storage, CodeIndexSemanticStorage::Persistent(_)) {
            connection
...
impl CodeIndexVectorStore for SqliteCodeIndexVectorStore {
        ...
        let mut statement = connection
            .prepare(
                "SELECT embedding FROM semantic_embedding_cache
                 WHERE root_id = ?1 AND embedding_model = ?2 AND path = ?3
    ...
    fn cache_embeddings(
        ...
        cache_embeddings(&transaction, root_id, embedding_model, chunks)?;
    ...
    fn published_generation(
        ...
        if metadata(&connection, "root_id")
            ...
            .as_deref()
            != Some(root_id.as_str())
            || metadata(&connection, "embedding_model")
                .map_err(store_error)?
                .as_deref()
                != Some(embedding_model.as_str())
        {
            return Ok(None);
        }
    ...
    fn replace_generation(
        ...
        set_metadata(&transaction, "embedding_model", embedding_model.as_str())
            .map_err(store_error)?;
    ...
    fn search(
        ...
        require_metadata(&connection, "embedding_model", embedding_model.as_str())?;
    ...
    fn delete_index(&self, root_id: &IndexRootId) -> Result<(), CodeIndexVectorStoreError> {
        let connection = self
            .connection
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    connection.execute_batch(
        "CREATE TABLE IF NOT EXISTS semantic_metadata (
         ...
         CREATE TABLE IF NOT EXISTS semantic_embedding_cache (
             ...
             language TEXT NOT NULL,
             embedding BLOB NOT NULL,
             PRIMARY KEY(root_id, embedding_model, path, chunk_key, content_hash, language)
         );",
    )
}
    ...
    let mut insert = connection
        .prepare(
            "INSERT INTO semantic_embedding_cache(
                root_id, embedding_model, path, chunk_key, content_hash, language, embedding
             ) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)
             ON CONFLICT(root_id, embedding_model, path, chunk_key, content_hash, language)
...
fn require_metadata(
    ...
    match metadata(connection, key).map_err(store_error)? {
        ...
        Some(_) => Err(CodeIndexVectorStoreError::new(format!(
            "semantic index {key} is not current"
        ))),
        None => Err(CodeIndexVectorStoreError::new(
            "semantic index has not been synchronized",
        )),
    }
}

fn validate_dimensions(chunks: &[EmbeddedCodeChunk]) -> Result<(), CodeIndexVectorStoreError> {
...
fn ann_candidate_rowids(
    ...
    query: &EmbeddingVector,
    result_limit: NonZeroUsize,
) -> Result<Option<Vec<i64>>, CodeIndexVectorStoreError> {
...
) -> Result<Vec<SearchRow>, CodeIndexVectorStoreError> {
...
fn search_hit(
    ...
    query: &EmbeddingVector,
    row: SearchRow,
) -> Result<VectorSearchHit, CodeIndexVectorStoreError> {
    let (
        relative_path,
        source_revision,
        chunk_key,
...
fn unpack_embedding(bytes: &[u8]) -> Result<EmbeddingVector, CodeIndexVectorStoreError> {
    ...
    let values = bytes
        ...
        .collect();
    EmbeddingVector::new(values).map_err(|error| CodeIndexVectorStoreError::new(error.to_string()))
}
...
fn store_error(error: impl std::fmt::Display) -> CodeIndexVectorStoreError {
    CodeIndexVectorStoreError::new(error.to_string())
}

fn index_identity_error(error: zeta_code_index::CodeIndexError) -> CodeIndexVectorStoreError {
    CodeIndexVectorStoreError::new(error.to_string())
}
...
#[cfg(unix)]
fn prepare_persistent_database(path: &Path) -> Result<(), CodeIndexVectorStoreError> {
    ...
    match fs::symlink_metadata(path) {
        Ok(metadata) if !metadata.file_type().is_file() => {
            return Err(CodeIndexVectorStoreError::new(
                "semantic code-index database must be a regular file",
            ));
        }
        Ok(_) => {}
...
#[cfg(not(unix))]
fn prepare_persistent_database(_path: &Path) -> Result<(), CodeIndexVectorStoreError> {
    Ok(())
}
