...
use crate::CodeIndexSemanticMetric;
use crate::CodeIndexSemanticMetricsSink;
use crate::CodeIndexSemanticProgressSink;
use crate::CodeIndexSemanticQuery;
use crate::CodeIndexSemanticQueryResult;
use crate::CodeIndexSemanticSyncPhase;
use crate::CodeIndexSemanticSyncProgress;
use crate::CodeIndexSemanticSyncResult;
...
impl CodeIndexSemanticService {
    ...
    pub fn lexical_generation(&self) -> Result<u64, CodeIndexSemanticError> {
        Ok(self.index.snapshot()?.generation)
    }

    /// Returns the generation currently published by the searchable semantic projection.
    pub fn published_generation(&self) -> Result<Option<u64>, CodeIndexSemanticError> {
        self.store
            .published_generation(self.index.root_id(), &self.embedding_model)
            .map_err(Into::into)
    }

    /// Synchronizes the rebuildable semantic projection to the exact current lexical generation.
    pub fn sync(&self) -> Result<CodeIndexSemanticSyncResult, CodeIndexSemanticError> {
    ...
    pub fn sync_with_control(
        ...
        progress: Option<&dyn CodeIndexSemanticProgressSink>,
    ) -> Result<CodeIndexSemanticSyncResult, CodeIndexSemanticError> {
        ...
        if let Some(metrics) = &self.metrics {
            match &result {
                Ok(result) => metrics.record(CodeIndexSemanticMetric::SyncCompleted {
                ...
                Err(CodeIndexSemanticError::Cancelled) => {
                    metrics.record(CodeIndexSemanticMetric::SyncCancelled {
                        processed_count: tracking.processed_chunk_count.load(Ordering::Relaxed),
                    })
                }
                Err(_) => metrics.record(CodeIndexSemanticMetric::SyncFailed),
            }
        }
        result
    }
    ...
    fn sync_inner(
        ...
        if manifest.snapshot.generation == 0 {
            return Err(CodeIndexSemanticError::IndexNotReady);
        }
        let chunks = self.index.materialize_chunks(&manifest.chunks)?;
        report(
            progress,
            CodeIndexSemanticSyncProgress {
                phase: CodeIndexSemanticSyncPhase::Preparing,
        ...
        let mut embeddings =
            self.store
                .reusable_embeddings(self.index.root_id(), &self.embedding_model, &chunks)?;
        if embeddings.len() != chunks.len() {
            return Err(crate::CodeIndexVectorStoreError::new(
        ...
        let mut current = CodeIndexSemanticSyncProgress {
            phase: CodeIndexSemanticSyncPhase::Embedding,
            generation: manifest.snapshot.generation,
            total_chunk_count: chunks.len(),
        ...
        for batch in missing.chunks(EMBEDDING_BATCH_SIZE) {
            ...
            if response.vectors().len() != batch.len() {
                return Err(CodeIndexSemanticError::InvalidModelResponse(
            ...
            self.store
                .cache_embeddings(self.index.root_id(), &self.embedding_model, &cached)?;
        ...
        current.phase = CodeIndexSemanticSyncPhase::Publishing;
        report(progress, current.clone());
        ...
        current.phase = CodeIndexSemanticSyncPhase::Complete;
        report(progress, current);
        Ok(CodeIndexSemanticSyncResult {
            generation: manifest.snapshot.generation,
    ...
    ) -> Result<CodeIndexSemanticQueryResult, CodeIndexSemanticError> {
    ...
    pub fn query_with_cancellation(
        ...
        if let Some(metrics) = &self.metrics {
            match &result {
                Ok(result) => metrics.record(CodeIndexSemanticMetric::QueryCompleted {
                    candidate_count: result.candidates.len(),
                    retry_count,
                    elapsed_millis: elapsed_millis(started),
                }),
                Err(_) => metrics.record(CodeIndexSemanticMetric::QueryDegraded),
            }
        }
        result
    }
    ...
    pub fn delete_index(&self) -> Result<(), CodeIndexSemanticError> {
...
struct TrackingProgressSink<'a> {
    downstream: Option<&'a dyn CodeIndexSemanticProgressSink>,
    processed_chunk_count: AtomicUsize,
}

impl CodeIndexSemanticProgressSink for TrackingProgressSink<'_> {
...
fn report(
    sink: Option<&dyn CodeIndexSemanticProgressSink>,
    progress: CodeIndexSemanticSyncProgress,
) {
...
fn check_cancelled(cancellation: &CancellationToken) -> Result<(), CodeIndexSemanticError> {
    cancellation
        .check()
        .map_err(|_| CodeIndexSemanticError::Cancelled)
}
...
) -> Result<T, CodeIndexSemanticError> {
    ...
    loop {
        ...
        match invoke() {
            ...
            Err(zeta_model_provider::ModelProviderError::Cancelled(_)) => {
                return Err(CodeIndexSemanticError::Cancelled);
            }
