...
pub(super) fn write_binding(
    ...
    if let Some(source) = record.binding.source_thread_id() {
        ...
        zeta_thread_store::validate_binding_source(record, &source)?;
    }
    install_binding(
        connection,
        &record.binding,
    ...
    for id in ids {
        let events = {
            ...
            statement
                ...
                .map_err(storage_error)?
                .map(|row| {
                    serde_json::from_str::<zeta_history::StoredEvent>(&row.map_err(storage_error)?)
                        .map_err(storage_error)
                })
                .collect::<Result<Vec<_>, _>>()?
        };
        ...
        let zeta_protocol::ThreadEvent::ThreadCreated {
            session_id,
            thread_id,
            origin,
            ..
        ...
        for event in &events {
            if event.schema_version < 16 {
                if let Some(recorded) = zeta_history::inherited_thread_origin(&event.event) {
                    origin = recorded;
                }
            }
        }
        let record = ThreadBinding {
            agent_id: zeta_history::created_thread_agent_id(first).map_err(storage_error)?,
