...
2. `PackageLease` 和 `acquire_package_lease_for_executable` 让 `zeta-app-server`、`zeta-app-server-daemon` 与 `zeta-code-mode-host` 在存活期间持有共享 `.lease`；启动 daemon 的父进程持有租约直到子进程完成接管。清理固定保留当前包和一个回滚包，只删除可取得独占租约的旧包、孤立包与未提交清单。
