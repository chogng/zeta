...
impl<'a> FileEditorPane<'a> {
    ...
    pub(crate) fn register_interactions(&self, frame: &mut InteractionFrame) {
        ...
        frame.register(
            ...
            .with_parent(FILE_EDITOR_PANE),
        );
        for (index, tab) in self.host.tabs().iter().enumerate() {
            frame.register(
                UiNode::new(
                    file_editor_tab_id(index),
                    self.tab_bounds(index),
                    AccessibilityRole::Tab,
                    tab.label(),
                )
                .with_parent(FILE_EDITOR_TAB_LIST)
            ...
            frame.register(
                UiNode::new(
                    ...
                    self.tab_close_bounds(index),
                    AccessibilityRole::Button,
                    format!("Close {}", tab.label()),
                )
                .with_parent(file_editor_tab_id(index))
                .with_cursor(CursorFeedback::Pointer)
                .with_focus(FocusBehavior::TabStop)
                .with_action(NodeAction::Activate),
            );
        ...
        if self.search_mode != FileEditorSearchMode::Hidden {
            ...
            if let Some(query) = self.search_query.as_ref() {
                frame.register(
                    ...
                    .with_parent(FILE_EDITOR_SEARCH_BAR)
            ...
            for action in self.search_actions() {
                frame.register(
                    UiNode::new(
                        action.element_id(),
                        self.search_action_bounds(action),
                        AccessibilityRole::Button,
                        action.label(),
                    )
                    .with_parent(FILE_EDITOR_SEARCH_BAR)
        ...
        if let Some(notice) = self.notice() {
            ...
            if notice.modal {
                frame.set_modal_root(FILE_EDITOR_NOTICE);
            }
            ...
            for (index, action) in notice.actions.iter().copied().enumerate() {
                frame.register(
                    UiNode::new(
                        action.element_id(),
                        self.notice_action_bounds(notice.actions.len(), index),
                        AccessibilityRole::Button,
                        action.label(),
                    )
                    .with_parent(FILE_EDITOR_NOTICE)
                    .with_cursor(CursorFeedback::Pointer)
                    .with_focus(FocusBehavior::TabStop)
                    .with_action(NodeAction::Activate)
        ...
        let Some(tab) = self.host.active() else {
            return;
        };
        frame.register(
            UiNode::new(
        ...
        for (index, control) in editor.fold_controls().into_iter().enumerate() {
            frame.register(
                UiNode::new(
...
impl Component for FileEditorPane<'_> {
    fn element(&self) -> ComponentElement {
        Element::leaf("FileEditorPane").in_bounds(self.bounds)
    }
    ...
    fn paint(&self, scene: &mut UiScene) {
        ...
        for (index, tab) in self.host.tabs().iter().enumerate() {
            ...
            scene.draw_text(TextBlock::new(
                ...
                zeta_ui::Size::new(
                    (bounds.size.width - TAB_HORIZONTAL_PADDING * 2.0 - TAB_CLOSE_SIZE).max(1.0),
                    18.0,
                ),
                TextStyle::new(
                    12.0,
        ...
        if self.search_mode != FileEditorSearchMode::Hidden {
            ...
            scene.draw_text(TextBlock::new(
                summary,
                zeta_ui::Point::new(summary_bounds.origin.x + 4.0, summary_bounds.origin.y + 4.0),
        ...
        if let Some(notice) = self.notice() {
            ...
            scene.draw_text(TextBlock::new(
                ...
                zeta_ui::Size::new((bounds.size.width - action_width - 28.0).max(1.0), 18.0),
        ...
        } else if let (Some(point), Some(hover)) = (self.pointer_position, self.language_hover) {
        ...
        if let (Some(caret), Some(completions)) = (editor.caret_bounds(), self.language_completions)
        {
