...
pub(crate) enum FileEditorAutoScrollDirection {
    #[default]
    Idle,
    Up,
    Down,
...
impl FileEditorAutoScrollDirection {
    pub(crate) const fn row_delta(self) -> isize {
        match self {
            Self::Idle => 0,
            Self::Up => -1,
            Self::Down => 1,
...
pub(crate) struct FileEditorAutoScrollState {
    direction: FileEditorAutoScrollDirection,
    deadline: Option<Instant>,
}

impl FileEditorAutoScrollState {
    pub(crate) fn update(&mut self, point: Point, bounds: Rect, now: Instant) {
    ...
    pub(crate) fn advance(&mut self, now: Instant) -> FileEditorAutoScrollDirection {
...
#[cfg(test)]
#[path = "file_editor_auto_scroll_tests.rs"]
mod tests;
