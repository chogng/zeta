...
impl NativeLanguageService {
    ...
    pub(crate) fn active_editor_diagnostics(
        &self,
        host: &FileEditorHost,
    ...
    pub(crate) fn active_completions(&self, host: &FileEditorHost) -> Option<&LanguageCompletions> {
        let tab = host.active()?;
    ...
    pub(crate) fn dismiss_hover(&mut self) {
        self.hover = None;
        self.pending_requests.clear(LanguageRequestKind::Hover);
    }

    pub(crate) fn request_active(&mut self, host: &FileEditorHost, kind: LanguageRequestKind) {
    ...
    pub(crate) fn request_active_at(
        &mut self,
        host: &FileEditorHost,
    ...
    pub(crate) fn handle_event(&mut self, event: LanguageServiceEvent, host: &FileEditorHost) {
        match event {
...
fn server_preference(
    ...
    let Some(config) = configuration.language_servers.get(server_id) else {
        return LanguageServerPreference::automatic();
    };
...
#[cfg(test)]
#[path = "language_service_host_tests.rs"]
mod tests;
