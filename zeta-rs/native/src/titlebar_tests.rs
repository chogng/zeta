...
use zui::{InteractionFrame, UiDispatch, UiIntent};
...
fn titlebar_places_actions_after_native_window_controls_and_component_gap() {
    ...
    dispatch.pointer_moved(Point::new(837.0, 16.0), &frame);
    let _ = dispatch.press_primary(&frame);
    assert!(dispatch.is_pressed(LANGUAGE_SERVER_SETTINGS_TOGGLE));
    assert_eq!(
        dispatch
            .release_primary(Point::new(837.0, 16.0), &frame)
            .intent,
