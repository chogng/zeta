//! Native popovers for product-neutral language hover and completion results.

use zeta_language_service::{LanguageCompletions, LanguageHover};
use zeta_ui::{
...
pub(crate) struct LanguageHoverPopover<'a> {
    bounds: Rect,
...
impl Component for LanguageHoverPopover<'_> {
    ...
    fn paint(&self, scene: &mut UiScene) {
        paint_surface(scene, self.bounds, self.palette);
        scene.draw_text(
            TextBlock::new(
...
pub(crate) struct LanguageCompletionPopover<'a> {
    bounds: Rect,
...
impl Component for LanguageCompletionPopover<'_> {
    ...
    fn paint(&self, scene: &mut UiScene) {
        ...
        if self.completions.items.is_empty() {
            paint_completion_row(scene, self.bounds, 0, "No suggestions", None, self.palette);
            return;
        }
...
fn paint_completion_row(
    ...
    if let Some(detail) = detail {
        scene.draw_text(TextBlock::new(
            ...
            TextStyle::new(11.0, palette.text_muted).with_line_height(18.0),
        ));
    }
}
...
fn paint_surface(scene: &mut UiScene, bounds: Rect, palette: ShellPalette) {
    scene.draw_rect(
        PaintRect::new(bounds, palette.surface_raised)
            .with_border(Border::new(Edges::uniform(1.0), palette.border)),
    );
}
...
#[cfg(test)]
#[path = "file_editor_language_features_tests.rs"]
mod tests;
