...
use crate::types::CodeIndexManifest;
...
impl IndexStore {
    pub fn open(storage: &CodeIndexStorage, root_id: &IndexRootId) -> Result<Self, CodeIndexError> {
        let connection = match storage {
            CodeIndexStorage::Memory => Connection::open_in_memory()?,
            CodeIndexStorage::Persistent(path) => {
        ...
        if matches!(storage, CodeIndexStorage::Persistent(_)) {
    ...
    pub fn replace_workspace(
        ...
        let mut connection = self.connection.lock().expect("code-index store poisoned");
        let transaction = connection.transaction()?;
        transaction.execute("DELETE FROM code_index_chunks_fts", [])?;
        transaction.execute("DELETE FROM code_index_chunks", [])?;
        transaction.execute("DELETE FROM code_index_files", [])?;
    ...
    pub fn snapshot(&self, root_id: &IndexRootId) -> Result<CodeIndexSnapshot, CodeIndexError> {
        let connection = self.connection.lock().expect("code-index store poisoned");
    ...
    pub fn manifest(&self, root_id: &IndexRootId) -> Result<CodeIndexManifest, CodeIndexError> {
        let connection = self.connection.lock().expect("code-index store poisoned");
        load_manifest(&connection, root_id)
    }

    pub fn source(&self, relative_path: &Path) -> Result<Option<StoredSource>, CodeIndexError> {
        let connection = self.connection.lock().expect("code-index store poisoned");
        let path = storage_path(relative_path);
        connection
            .query_row(
                "SELECT source_revision, source_bytes FROM code_index_files WHERE path = ?1",
                [path],
                |row| {
                    Ok(StoredSource {
    ...
    pub fn has_descendants(&self, relative_path: &Path) -> Result<bool, CodeIndexError> {
        let connection = self.connection.lock().expect("code-index store poisoned");
        let prefix = relative_path.to_path_buf();
        let mut statement = connection.prepare("SELECT path FROM code_index_files")?;
    ...
    pub fn search(
        ...
        expression: &str,
        result_limit: usize,
    ) -> Result<Vec<SearchHit>, CodeIndexError> {
        let connection = self.connection.lock().expect("code-index store poisoned");
        let mut statement = connection.prepare(
            "SELECT c.path, c.source_revision, c.chunk_key, c.content_hash, c.language, \
                    ...
                    bm25(code_index_chunks_fts) \
             FROM code_index_chunks_fts \
             JOIN code_index_chunks c ON c.rowid = code_index_chunks_fts.chunk_rowid \
             WHERE code_index_chunks_fts MATCH ?1 \
             ORDER BY bm25(code_index_chunks_fts), c.path, c.ordinal \
             LIMIT ?2",
        )?;
    ...
    fn initialize(&self, root_id: &IndexRootId) -> Result<(), CodeIndexError> {
        let mut connection = self.connection.lock().expect("code-index store poisoned");
    ...
    connection.execute_batch(
        "CREATE TABLE IF NOT EXISTS code_index_metadata (
         ...
         CREATE TABLE IF NOT EXISTS code_index_chunks (
             ...
             path TEXT NOT NULL REFERENCES code_index_files(path) ON DELETE CASCADE,
         ...
         CREATE INDEX IF NOT EXISTS code_index_chunks_key ON code_index_chunks(chunk_key);
         CREATE VIRTUAL TABLE IF NOT EXISTS code_index_chunks_fts USING fts5(
             path,
             content,
...
#[cfg(unix)]
fn prepare_persistent_database(path: &Path) -> Result<(), CodeIndexError> {
        ...
        Ok(metadata) if !metadata.file_type().is_file() => {
            return Err(CodeIndexError::Io {
                ...
                source: std::io::Error::new(
                    ...
                    "code-index database must be a regular file",
                ),
            });
        }
        Ok(_) => {}
...
#[cfg(not(unix))]
fn prepare_persistent_database(_path: &Path) -> Result<(), CodeIndexError> {
    Ok(())
}

fn reset_projection(connection: &mut Connection) -> rusqlite::Result<()> {
    connection.execute_batch(
        "DROP TABLE IF EXISTS code_index_chunks_fts;
         DROP TABLE IF EXISTS code_index_chunks;
         DROP TABLE IF EXISTS code_index_files;
         DROP TABLE IF EXISTS code_index_metadata;",
    )?;
    create_schema(connection)
}
...
fn insert_file(transaction: &Transaction<'_>, file: &PreparedFile) -> rusqlite::Result<()> {
    ...
    for (ordinal, chunk) in file.chunks.iter().enumerate() {
        ...
        transaction.execute(
            "INSERT INTO code_index_chunks_fts(path, content, chunk_rowid) VALUES (?1, ?2, ?3)",
...
fn remove_file(transaction: &Transaction<'_>, relative_path: &Path) -> rusqlite::Result<()> {
    let path = storage_path(relative_path);
    transaction.execute(
        "DELETE FROM code_index_chunks_fts WHERE chunk_rowid IN (
             SELECT rowid FROM code_index_chunks WHERE path = ?1
         )",
        [&path],
    )?;
    transaction.execute("DELETE FROM code_index_chunks WHERE path = ?1", [&path])?;
    transaction.execute("DELETE FROM code_index_files WHERE path = ?1", [&path])?;
    Ok(())
}
...
fn snapshot_from_connection(
    ...
    let (file_count, source_bytes) = connection.query_row(
        "SELECT COUNT(*), COALESCE(SUM(source_bytes), 0) FROM code_index_files",
        [],
        |row| Ok((row.get::<_, i64>(0)?, row.get::<_, i64>(1)?)),
    )?;
    let chunk_count =
        connection.query_row("SELECT COUNT(*) FROM code_index_chunks", [], |row| {
            row.get::<_, i64>(0)
        })?;
    let truncated_file_count = connection.query_row(
        "SELECT COUNT(*) FROM code_index_files WHERE chunk_limit_hit = 1",
        [],
        |row| row.get::<_, i64>(0),
    )?;
...
fn metadata(connection: &Connection, key: &str) -> rusqlite::Result<Option<String>> {
    connection
        .query_row(
            "SELECT value FROM code_index_metadata WHERE key = ?1",
            [key],
            |row| row.get(0),
        )
        .optional()
...
fn set_metadata(connection: &Connection, key: &str, value: &str) -> rusqlite::Result<()> {
    connection.execute(
        "INSERT INTO code_index_metadata(key, value) VALUES (?1, ?2)
