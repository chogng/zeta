...
#[derive(Default)]
pub(crate) struct CodeIndexOverlay {
...
impl CodeIndexOverlay {
    pub fn snapshot(&self) -> CodeIndexOverlaySnapshot {
        let state = self
            .state
            .read()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        CodeIndexOverlaySnapshot {
    ...
    pub fn synchronize(
        ...
        {
            return Err(CodeIndexError::OverlayRevisionConflict);
        }
    ...
    pub fn close(&self, relative_path: &Path) -> CodeIndexOverlaySnapshot {
        let mut state = self
            .state
            .write()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    pub fn handoff(&self, manifest: &CodeIndexManifest) -> CodeIndexOverlaySnapshot {
    ...
    pub fn materialize_chunk(
        &self,
        reference: &ChunkReference,
    ) -> Option<Result<MaterializedChunk, CodeIndexError>> {
        let state = self
            .state
            .read()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        ...
        Some(
            document
                ...
                .cloned()
                .ok_or(CodeIndexError::OverlaySupersedesPersistentSource),
        )
    }
    ...
    ) -> Option<Result<MaterializedExcerpt, CodeIndexError>> {
        let state = self
            .state
            .read()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    pub fn materialize_source(
        &self,
        reference: &IndexedSourceReference,
    ) -> Option<Result<MaterializedSource, CodeIndexError>> {
        let state = self
            .state
            .read()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
...
fn materialize_overlay_excerpt(
    ...
    if document.source.reference.source_revision != reference.source_revision {
        return Err(CodeIndexError::OverlaySupersedesPersistentSource);
    }
...
fn snapshot_from_state(state: &OverlayState) -> CodeIndexOverlaySnapshot {
    CodeIndexOverlaySnapshot {
        generation: state.generation,
        documents: state.documents.values().cloned().collect(),
    }
