...
pub(crate) fn load_manifest(
    ...
    root_id: &IndexRootId,
) -> Result<CodeIndexManifest, CodeIndexError> {
    ...
    let mut source_statement = connection.prepare(
        "SELECT path, source_revision, language, source_bytes
         FROM code_index_files ORDER BY path",
    )?;
    ...
    let mut chunk_statement = connection.prepare(
        "SELECT path, source_revision, chunk_key, content_hash, language,
         ...
         FROM code_index_chunks ORDER BY path, ordinal",
    )?;
...
fn load_snapshot(
    ...
    root_id: &IndexRootId,
    generation: u64,
) -> rusqlite::Result<CodeIndexSnapshot> {
