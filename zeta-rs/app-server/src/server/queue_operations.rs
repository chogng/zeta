...
impl AppServer {
    ...
    pub(super) fn queue_edit(&self, value: &Value) -> Result<Value, RpcError> {
        let params: zeta_app_server_protocol::protocol::queue::QueueEditParams = decode(value)?;
        self.read_session_thread(&params.session_id, &params.thread_id)?;
        let action = match params.action {
            zeta_app_server_protocol::protocol::queue::QueueEditAction::Pause => {
                queue::QueueEdit::Pause
            }
            zeta_app_server_protocol::protocol::queue::QueueEditAction::Move { direction } => {
                queue::QueueEdit::Move { direction }
            }
            zeta_app_server_protocol::protocol::queue::QueueEditAction::Send { turn_id } => {
                queue::QueueEdit::Send { turn_id }
            }
            zeta_app_server_protocol::protocol::queue::QueueEditAction::Replace { input } => {
    ...
    pub(super) fn extension_items(&self, value: &Value) -> Result<Value, RpcError> {
        let params: zeta_app_server_protocol::protocol::extension_items::ExtensionItemsParams =
            decode(value)?;
        let thread = self.read_session_thread(&params.session_id, &params.thread_id)?;
        let items = self
            .agent_extensions
            .contribute_items(zeta_extension_api::ThreadContext {
                session_id: &params.session_id,
                thread_id: &params.thread_id,
                sequence: thread.sequence,
            })
            .map_err(|_| RpcError::new(-32603, AppServerErrorName::InternalError))?;
        result(&zeta_app_server_protocol::protocol::extension_items::ExtensionItemsResult { items })
    }
        ...
        if let Some(turn_id) = &request.steer_turn {
            ...
            let result = self.threads.steer_turn(
                ...
                zeta_core::SteerTurnRequest {
                    ...
                    expected_sequence: zeta_core::SequenceExpectation::Exact(snapshot.sequence),
...
fn accepted(
...
) -> Result<Option<zeta_protocol::TurnId>, ()> {
    ...
    let matches = match &entry.receipt.command {
        zeta_protocol::ThreadCommand::StartTurn {
            input,
            tool_mode,
            approval_mode,
            ..
        ...
        zeta_protocol::ThreadCommand::SteerTurn { turn_id, input } => {
