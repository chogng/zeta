...
fn instruction_benchmark_selection() {
    ...
    let profiles = zeta_models_manager::ModelInstructionCatalog::new([
        zeta_models_manager::ModelInstructionProfile {
            model: model.clone(),
            instructions: zeta_prompts::PromptArtifact::new(
                "models-manager",
                "benchmark/guidance",
                "v1",
                zeta_prompts::AGENT_INSTRUCTIONS.body(),
            ),
        },
    ])
    .unwrap();
    ...
    for (role_count, tool_count) in [(1, 128), (16, 128), (128, 128), (16, 16), (16, 512)] {
        ...
        for first in (0..role_count).step_by(64) {
            ...
            let root = directory.path().join(".zeta/agents");
