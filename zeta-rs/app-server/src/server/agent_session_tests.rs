...
use zeta_protocol::AgentRoleSelection;
use zeta_protocol::AgentRoleSource;
use zeta_protocol::CommandId;
use zeta_protocol::ModelId;
use zeta_protocol::ModelInstructionSelection;
...
impl ToolService for CatalogTools {
    ...
    fn prepare(&self, _: &ToolCall) -> Result<zeta_action_policy::ActionReviewRequest, CoreError> {
        Err(CoreError::Execution("unexpected tool preparation".into()))
    }
    fn execute(
        &self,
        _: &ToolCall,
        _: &zeta_core::ToolAuthorization,
        _: &CancellationToken,
    ) -> Result<zeta_core::ToolExecutionOutput, CoreError> {
...
fn session_role_is_atomic_replayable_and_applied_to_real_model_input() {
    ...
    let guidance = zeta_prompts::PromptArtifact::new(
        "models-manager",
        "model/test",
        "test-v1",
        "MODEL_GUIDANCE_MARKER\n",
    );
    let catalog = zeta_models_manager::ModelInstructionCatalog::new([
        zeta_models_manager::ModelInstructionProfile {
            model: model(),
    ...
    let mut server = AppServer::new(threads.clone(), Arc::new(CaptureModel(tx)))
        ...
        .unwrap()
        .with_skill_runtime(
            zeta_skills_extension::BuiltInSkillSource::Root(root.path().to_owned()),
            Arc::new(SkillConfig),
            None,
        )
        .unwrap()
