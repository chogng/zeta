...
impl AppServer {
    ...
    pub(super) fn feedback_upload(
        ...
        let timeout = zeta_http_client::Timeout::After(std::time::Duration::from_secs(15));
        let config = zeta_http_client::HttpClientConfig::default()
            .with_redirect_policy(zeta_http_client::RedirectPolicy::Reject)
            .with_network_target_policy(zeta_http_client::NetworkTargetPolicy::PublicInternetOnly)
            .with_proxy_policy(zeta_http_client::ProxyPolicy::Direct)
            .with_timeouts(zeta_http_client::TransportTimeouts::new(
                timeout, timeout, timeout, timeout,
            ));
        let http = zeta_http_client::UreqHttpClient::with_config(config).map_err(internal)?;
        let client = zeta_client::ZetaClient::new(std::sync::Arc::new(http));
        self.feedback
            .upload(
                connection.connection_id,
                &params.digest,
