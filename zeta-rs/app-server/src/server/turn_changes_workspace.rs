...
use zeta_core::{CoreError, ThreadWorkspaceProvisionRequest, ThreadWorkspaceProvisioner};
use zeta_protocol::ThreadOrigin;
use zeta_turn_changes::{CommitState, TurnChangeSet, TurnChangeStore};
use zeta_workspace::WorkspaceRoot;
use zeta_worktree::{
    ThreadRepositoryBinding, ThreadWorkspaceKind, ThreadWorktreeBinding,
...
impl TurnChangesRuntime {
    pub(super) fn enforce_cleanup_policy(&self) -> Result<(), String> {
        ...
        for (thread_id, binding) in bindings {
            let Ok(thread) = self.sessions.threads().read_thread(&thread_id) else {
                continue;
            };
            let Ok(session) = self.sessions.read_session(&thread.session_id) else {
                continue;
            };
            if session.status == zeta_protocol::SessionStatus::Active {
                continue;
            }
            let records = self
                .store
        ...
        for (_, thread_id, binding) in eligible.into_iter().skip(settings.keep_count) {
            ...
            self.bindings
                .write()
                .map_err(|_| "Thread workspace binding lock poisoned".to_string())?
                .remove(&thread_id);
            self.workspace_access.unbind_thread_workspace(&thread_id);
            self.hooks.unbind_thread_workspace(&thread_id);
    ...
    pub(super) fn reset_thread_to_committed_changes(
        ...
        let binding = self
            .binding(thread_id)
            .ok_or_else(|| format!("Thread {thread_id} has no workspace binding"))?;
        self.worktree_runtime.block_on(async {
            if binding.kind() == ThreadWorkspaceKind::Directory {
                ...
                return snapshots.replace_directory(binding.workspace_directory(), &desired);
            }
    ...
    fn source_for(
            ...
            ThreadOrigin::Root => {
                return Ok((
                    ThreadWorktreeSource::WorkspaceSnapshot {
                        source_directory: self.workspace_root.clone(),
                    },
        ...
        let parent = self.binding(parent_id).ok_or_else(|| {
            CoreError::Journal(format!(
                "parent Thread {parent_id} has no workspace binding"
            ))
        })?;
        let source = match origin {
            ThreadOrigin::Rewind { before_turn_id, .. } => {
                ...
                if records.is_empty() {
                    return Err(CoreError::Journal(format!(
                        "rewind Turn {before_turn_id} has no immutable workspace checkpoint"
                    )));
                }
                ...
                ThreadWorktreeSource::ImmutableTree {
                    source_directory: parent.workspace_directory().to_path_buf(),
            ...
            ThreadOrigin::AgentSpawn { .. } => ThreadWorktreeSource::WorkspaceSnapshot {
                source_directory: parent.workspace_directory().to_path_buf(),
            },
    ...
    fn trees_at_sequence(
        ...
        checkpoint_trees(&binding, &records, true).ok_or_else(|| {
            CoreError::Journal(format!(
                "Fork source Turn {turn_id} has no complete sealed workspace checkpoint"
            ))
        })
    }
}
...
impl ThreadWorkspaceProvisioner for TurnChangesRuntime {
            ...
            .block_on(
                self.worktrees
                    .provision_thread(&ThreadWorktreeProvisionRequest {
                        source,
                        target,
                        source_workspace_id: self.workspace_id.to_string(),
                        thread_id: request.thread_id.to_string(),
                    }),
            )
            .map_err(|error| {
                CoreError::Journal(format!("cannot provision Thread workspace: {error}"))
            })?;
        self.bindings
            .write()
            .map_err(|_| CoreError::Journal("Thread workspace binding lock poisoned".into()))?
            .insert(request.thread_id.clone(), binding);
        let binding = self.binding(&request.thread_id).ok_or_else(|| {
            CoreError::Journal("Thread workspace binding disappeared after provision".into())
        })?;
        self.bind_thread_services(&request.thread_id, &binding)
    }
}
...
impl TurnChangesRuntime {
    fn bind_thread_services(
        ...
        self.workspace_access
            .bind_thread_workspace(thread_id.clone(), root);
        self.start_watcher(thread_id.clone(), binding.workspace_directory())
