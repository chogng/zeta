...
pub(in crate::server) struct ResolvedAgentSelection {
    pub(in crate::server) role: Option<AgentRoleSnapshot>,
    pub(in crate::server) capability_scope: AgentCapabilityScope,
}

pub(in crate::server) fn resolve_agent_selection(
    requested: &zeta_protocol::AgentRoleSelection,
...
pub(super) fn select_definition<'a>(
    ...
    let zeta_protocol::AgentRoleSelection::Exact { source, name } = requested else {
        return Ok(None);
    };
...
impl super::AppServer {
    pub(super) fn resolve_root_agent(
        ...
        if !references.is_empty() {
            ...
            for reference in references {
                ...
                let activated = runtime
                    .activate_explicit(&zeta_protocol::SkillRef::follow_latest(
        ...
        let model = self.model_catalog.configured_default()?;
        let profile = self.turn_executor_snapshot().tool_profile_snapshot()?;
        ...
        let selected_model = selected
            ...
            .or(model.as_ref());
        let instructions = zeta_prompts::AGENT_INSTRUCTIONS
            .freeze()
            .with_model_guidance(self.model_instructions.resolve(selected_model));
        let agent = zeta_protocol::AgentConfiguration {
            role: selected.role,
