...
impl ContextInput {
    ...
    pub(crate) fn with_time_context(
        mut self,
        current: &zeta_protocol::TimeContext,
        references: &BTreeMap<ItemId, zeta_protocol::TimeContext>,
    ) -> Result<Self, crate::CoreError> {
        let current = zeta_agent_environment::TimeSnapshot::new(current.clone())
        ...
        for item in &mut self.items {
            if let ThreadItem::UserMessage { item_id, text, .. } = item {
                let reference = match references.get(item_id) {
                    Some(time) => zeta_agent_environment::TimeSnapshot::new(time.clone())
                        .map_err(|error| crate::CoreError::Context(error.to_string()))?
                        .render_reference(),
