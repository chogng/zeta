...
pub(crate) fn scan_workspace(
    ...
    let paths = workspace_paths(root, limits);
...
pub(crate) fn scan_workspace_stamps(
    root: &WorkspaceRoot,
    limits: &FastRegexSearchLimits,
) -> Result<BTreeMap<PathBuf, FileStamp>, FastRegexError> {
    let mut stamps = BTreeMap::new();
    for path in workspace_paths(root, limits) {
        let absolute = root.canonical_path().join(&path);
...
fn workspace_paths(root: &WorkspaceRoot, limits: &FastRegexSearchLimits) -> Vec<PathBuf> {
