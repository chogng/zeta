...
use crate::matcher::sort_hits;
use crate::store::IndexStore;
use crate::store::SourceSymbols;

/// Workspace-side symbol projection backed by one canonical [`CodeIndex`] source authority.
...
impl SymbolIndex {
    /// Opens a rebuildable symbol projection for the exact root owned by `code_index`.
    pub fn open(
        code_index: Arc<CodeIndex>,
        ...
        let store = IndexStore::open(&storage, code_index.root_id())?;
        let projection = store.load_projection(code_index.root_id())?;
    ...
    /// Returns the stable identity of the canonical CodeIndex root projected by this index.
    pub fn root_id(&self) -> &IndexRootId {
        self.code_index.root_id()
    }
    ...
    pub fn snapshot(&self) -> Result<SymbolIndexSnapshot, SymbolIndexError> {
        self.store.snapshot(self.code_index.root_id())
    }

    /// Reconciles this projection against the current CodeIndex manifest.
    ///
    /// Unchanged source revisions reuse persisted symbols; changed sources are materialized through
    /// CodeIndex so this crate never creates a second filesystem scanner or source authority.
    pub fn reconcile(&self) -> Result<SymbolIndexRefreshOutcome, SymbolIndexError> {
        let _operation = self
            .operation
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        let manifest = self.code_index.manifest()?;
        if manifest.snapshot.root_id != *self.code_index.root_id() {
            return Err(SymbolIndexError::SourceRootMismatch);
        }
        let stored = self.store.load_projection(self.code_index.root_id())?;
    ...
    /// Rebuilds the ephemeral symbol candidate set from the canonical CodeIndex overlay.
    pub fn reconcile_overlay(&self) -> Result<(), SymbolIndexError> {
        let snapshot = self.code_index.overlay_snapshot();
        if self
            .overlay
            .read()
            .unwrap_or_else(|poisoned| poisoned.into_inner())
