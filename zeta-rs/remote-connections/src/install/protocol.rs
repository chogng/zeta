...
pub(crate) fn remote_runtime_install_command(
    ...
    format!(
        concat!(
            ...
            "executable=\"$runtime_dir/bin/zeta-remote-server\"; receipt=\"$runtime_dir/.zeta-remote-runtime-sha256\"; ",
            ...
            "[ -f \"$package/bin/zeta-remote-server\" ] || fail 'package-entrypoint-missing' 65; ",
            ...
            "chmod 700 \"$package/bin/zeta-remote-server\" \"$package/bin/zeta-app-server-daemon\" \"$package/zeta-path/rg\" \"$package/zeta-resources/node/bin/node\" || fail 'package-executable-permissions-failed' 73; ",
...
pub(super) fn parse_install_receipt(
    ...
    if digest != artifact.integrity.sha256
        ...
        || !executable.ends_with("/bin/zeta-remote-server")
    {
        return Err(RemoteRuntimeInstallError::new(
            RemoteRuntimeInstallFailureKind::RemoteRejected,
