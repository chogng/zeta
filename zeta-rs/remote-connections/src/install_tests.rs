...
fn installer_probes_uploads_and_returns_an_exact_immutable_runtime() {
    ...
    let executable = format!(
        "/home/remote/.local/share/zeta/remote/runtimes/{}/{}/{}/bin/zeta-remote-server",
...
fn package_artifact(root: &Path, variant: PackageVariant) -> RemoteRuntimeArtifact {
    ...
    match variant {
        PackageVariant::LinkedEntrypoint => {
            ...
            header.set_cksum();
            builder
                .append_data(&mut header, "bin/zeta-remote-server", std::io::empty())
                .unwrap();
        }
        _ => unpacked_size += append_file(&mut builder, "bin/zeta-remote-server", b"zeta", 0o755),
    }
    unpacked_size += append_file(&mut builder, "bin/zeta-app-server-daemon", b"daemon", 0o755);
    unpacked_size += append_file(&mut builder, "bin/zeta-app-server", b"daemon", 0o755);
