...
#[test]
fn child_fixture() {
    if let Some(marker) = std::env::var_os("ZETA_JOB_TEST_MARKER") {
...
async fn failed_assignment_does_not_execute_child_code() {
    let marker = std::env::temp_dir().join(format!("zeta-job-rejected-{}", std::process::id()));
    ...
    command.env("ZETA_JOB_TEST_MARKER", &marker);
...
async fn missing_executable_returns_creation_error() {
    let job = JobObject::create().unwrap();
    let mut command = Command::new("zeta-no-such-job-test-executable.exe");
    assert_eq!(
