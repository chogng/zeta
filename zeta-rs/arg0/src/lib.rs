//! Shared internal process roles for executables that embed Zeta helper capabilities.

use std::ffi::OsString;
use std::path::PathBuf;
use zeta_fast_regex_search::FastRegexWorkerCommand;

const FAST_REGEX_WORKER: &str = "--zeta-fast-regex-worker";
...
pub fn dispatch(arguments: impl IntoIterator<Item = OsString>) -> Option<Result<(), String>> {
    ...
    Some(zeta_fast_regex_search::serve_worker_from_environment().map_err(|error| error.to_string()))
}
