...
fn layout() -> (tempfile::TempDir, String) {
    ...
    std::fs::write(
        temp.path().join("bin/zeta-windows-sandbox.exe"),
        b"owned runner",
    )
    .unwrap();
    for name in [
    ...
    let digest = hash(&temp.path().join("bin/zeta-windows-sandbox.exe")).unwrap();
    (temp, digest)
}
...
fn a_changed_runtime_is_not_silently_deleted() {
    ...
    .unwrap();
    assert!(clean_files(temp.path(), &digest, 2).is_err());
    assert!(temp.path().join("bin/zeta-windows-sandbox.exe").exists());
    assert!(temp.path().join("state.dpapi").exists());
}
