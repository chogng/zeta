...
pub enum AdditionalDirectoryContributionPolicy {
    ...
    FileAccessOnly,
    /// Transient source exposing Skills, Subagents, and selected Plugin declarations.
...
impl AdditionalDirectoryContributionPolicy {
    ...
    pub fn contributions(self) -> &'static [AdditionalDirectoryContribution] {
        use AdditionalDirectoryContribution::{
            EnabledPlugins, ExtraKnownMarketplaces, InstructionRules, LocalInstructions,
            ProjectInstructions, Skills, Subagents,
        };

        match self {
            Self::FileAccessOnly => &[],
            Self::AllowlistedProjectContributions => {
                &[Skills, Subagents, EnabledPlugins, ExtraKnownMarketplaces]
            }
