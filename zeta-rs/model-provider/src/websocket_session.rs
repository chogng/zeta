...
use zeta_api::RealtimeSession;
use zeta_api::ResponsesWebSocketSession;
use zeta_api::WebSocketSessionConfig;
use zeta_async_utils::CancellationToken;
use zeta_client::ResolvedApiTarget;
use zeta_model_provider_config::ModelProviderConfig;
use zeta_model_provider_config::RealtimeApiProfile;
use zeta_model_provider_config::WebSocketApiProfile;
...
impl ModelProviderRuntime {
    ...
    pub async fn connect_realtime(
        ...
        if zeta_model_provider_config::find_static_model(model).is_some_and(|model| {
            !matches!(
                model.runtime,
                zeta_model_provider_config::StaticModelRuntime::ProviderApi
            )
        }) {
            return Err(ModelProviderError::Unavailable(
...
impl ResponsesModelSession {
    pub fn connection_stats(&self) -> zeta_api::ResponsesConnectionStats {
        self.session.stats()
    }
    ...
    pub async fn invoke(
        ...
        let model = self.provider.resolve_model(
            &zeta_model_provider_config::ModelId::new(&self.model).expect("validated model ID"),
        )?;
    ...
    pub async fn warm_up(
        ...
        request: &ModelRequest,
        cancellation: &CancellationToken,
    ) -> Result<zeta_api::ResponsesWarmup, ModelProviderError> {
        self.refresh_auth(cancellation).await?;
        let model = self.provider.resolve_model(
            &zeta_model_provider_config::ModelId::new(&self.model).expect("validated model ID"),
        )?;
