...
const PROBE_USAGE: &str =
    "usage: zeta-server remote probe --host <ssh-host> [--ssh <openssh-path>]";
const INSTALL_USAGE: &str = concat!(
    "usage: zeta-server remote install --host <ssh-host> --archive <zeta-package.tar.gz> ",
...
const FETCH_USAGE: &str = concat!(
    "usage: zeta-server remote fetch-runtime --catalog-url <https-catalog.json> ",
