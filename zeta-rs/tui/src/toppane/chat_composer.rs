...
use super::textarea::TextElementId;
...
impl ChatComposer {
    ...
    pub(super) fn with_slash_commands(slash_commands: SlashCommandRegistry) -> Self {
        Self {
    ...
    pub(super) fn handle_paste(&mut self, pasted: String) -> Result<(), String> {
        match self
            .attachments
    ...
    pub(super) fn attach_image_bytes(&mut self, bytes: Vec<u8>) -> Result<(), String> {
        self.attachments
    ...
    pub(super) fn slash_popup(&self) -> Option<SlashPopupView<'_>> {
        self.slash_popup.view()
    }

    pub(super) fn mention_popup(&self) -> Option<MentionPopupView<'_>> {
        self.mentions.view()
    }
    ...
    pub(super) fn activate_slash_command(&mut self, index: usize) -> Option<ComposerOutcome> {
        let command = self.slash_popup.command_at(index)?;
        self.complete_slash_command(&command);
        Some(self.submit())
    }

    pub(super) fn activate_mention(&mut self, index: usize) -> bool {
