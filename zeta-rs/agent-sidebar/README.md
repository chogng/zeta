...
| Changes functional layout and multi-diff presentation | `scm::ScmLayout` / `scm::EditorPane` | Owns the Changes content bounds and diff viewport interactions. |
...
icons. It must not depend on `zeta-native`, app-server transport crates, a
...
Run `cargo check -p zeta-agent-sidebar -p zeta-native`,
`cargo test -p zeta-agent-sidebar`, and `cargo test -p zeta-native`. Files
