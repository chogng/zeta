# `zeta-message-history`

- 定义本机 Profile 的用户输入记录、追加、分页搜索、清理和保留策略。
- 输入记录用于再次编辑和提交；不参与 Thread 恢复、分支、模型上下文或输入分类。
- `zeta-state` 实现 SQLite 存储；`MessageHistory` 提供有界、按顺序执行的后台读写，关闭时完成已接收的写入。
...
验证：`just test zeta-message-history` 与 `just test zeta-state message_history`。

TypeScript Chat 尚未接入，后续工作见 [Chat 输入历史接入待办](../../zeta-ts/docs/chat-input-history.md)。
