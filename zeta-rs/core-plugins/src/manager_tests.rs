...
use crate::MarketplaceInstallCapability;
...
fn local_manager_persists_the_full_installed_state() {
    ...
    let manager = PluginsManager::open(root.path(), registry.clone()).unwrap();
    ...
    let reopened = PluginsManager::open(root.path(), registry).unwrap();
    assert_eq!(
...
fn local_manager_updates_uninstalls_and_owns_resource_leases() {
    let root = tempfile::tempdir().unwrap();
    let manager = PluginsManager::open(root.path(), Arc::new(FakeRegistry)).unwrap();
    let installed = manager
        .install(InstallPackageRequest {
            package_id: "example/demo".into(),
            version: Some("1.0.0".into()),
...
fn packaged_stdio_mcp_uses_a_path_free_executable_resource() {
    let root = tempfile::tempdir().unwrap();
    let manager = PluginsManager::open(root.path(), Arc::new(StdioRegistry)).unwrap();
    let installed = manager
        .install(InstallPackageRequest {
