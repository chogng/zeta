 ...
 *
 * A Sessions workbench is a product-owned surface beside the regular
...
/** Creates an immutable Sessions profile after checking stable product identity. */
export function createSessionsProfile(profile: SessionsProfile): SessionsProfile {
  ...
  if (typeof profile.productId !== "string" || !/^[a-z][a-z0-9-]*$/.test(profile.productId)) throw new TypeError("Sessions profile product id must be stable kebab-case");
