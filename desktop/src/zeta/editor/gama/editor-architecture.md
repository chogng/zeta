...
> Gama 是 `editor` 下与 Alpha 并列的结构化编辑器域。它沿用 Alpha 的 input → pane → session → model reference → widget 骨架，以树形 `DocumentModel` 取代行式 `TextModel`。行为契约、测试和限制见 [`README.md`](./README.md)。

`textBlock` 是 Gama 的普通文本块；Alpha 的 `codeBlock` 是 Alpha 的代码语义。`TextEditorWidget` 只代表一个 `textBlock` 中的嵌入式行编辑器，它包裹 Alpha 的 `CodeEditorWidget`，不再是 Gama 的 pane、session 或目录名。
...
| `AlphaEditorPane` | `GamaEditorPane` | `IEditorPane` 生命周期、布局、可见性与 working copy 暴露 | Gama 创建 document-model service |
| `AlphaEditorSession` | `GamaEditorSession` | 一次打开的 editor 状态、DOM projection、focus 与命令输入 | Gama 投影树节点、marks、node views 与结构化 selection |
...
editor/gama/
  ...
  browser/
    ...
    gamaEditorPane.ts             # IEditorPane host
    gamaEditorSession.ts          # structured editor session + DOM projection
    services/
...
| `browser/gamaEditorPane` | browser services、Workbench editor contracts | pane host、session 生命周期、layout、visibility、working-copy bridge |
| `browser/gamaEditorSession` | Gama common、document-model service、DOM、embedded-editor contract | 一个 document reference 的 node/mark projection、input、toolbar、outline 与 focus；不得注册产品 pane |
...
Gama 的对外入口分工镜像 VS Code：`editor.api.ts` 只公开 DOM-free document model、schema、transaction 与 serialization；`editor.all.ts` 是 product entry 使用的 contribution bundle；`editor.main.ts` 是 all + api 的完整入口。当前 Gama 没有浏览器 Worker consumer，因此不提供虚假的 `editor.worker.start.ts`；未来 collaboration/layout worker 出现时才在此添加其真实启动协议。Academic 产品入口只导入 `gama/editor.all.ts` 与 `academicWorkbenchSession`，Code 只导入 Alpha 的对应入口与 `codeWorkbenchSession`，Complete 明确导入两者。`academicEditor.contribution.ts` 在唯一的 composition seam 注入 `AlphaEmbeddedTextEditorFactory`。Alpha 从不依赖 Gama document types。
