...
test("Gama collaboration session keeps canonical and optimistic snapshots separate", () => {
...
test("Gama collaboration session rejects stale updates and local echoes", () => {
  const schema = createDefaultDocumentSchema();
...
test("Gama collaboration session reports dropped pending steps after remote deletion", () => {
  const schema = createDefaultDocumentSchema();
