...
export class RemoteDocumentCollaborationService extends DisposableOwner implements IDocumentCollaborationService {
  ...
  async updatePresence(connection: RemoteDocumentCollaborationConnection, selection: DocumentSelection | undefined, signal: AbortSignal): Promise<void> {
    throwIfCancelled(signal, "Publishing remote Gama collaboration presence was cancelled");
  ...
  async createInvite(connection: RemoteDocumentCollaborationConnection, displayName: string, role: DocumentCollaborationRoomRole, signal: AbortSignal): Promise<DocumentCollaborationInvite> {
    throwIfCancelled(signal, "Creating a remote Gama collaboration invitation was cancelled");
  ...
  async listMembers(connection: RemoteDocumentCollaborationConnection, signal: AbortSignal): Promise<readonly DocumentCollaborationMember[]> {
    throwIfCancelled(signal, "Reading remote Gama collaboration members was cancelled");
  ...
  async rotateMemberAccessToken(connection: RemoteDocumentCollaborationConnection, principalId: string, signal: AbortSignal): Promise<DocumentCollaborationInvite> {
    throwIfCancelled(signal, "Rotating a remote Gama collaboration credential was cancelled");
  ...
  async revokeMember(connection: RemoteDocumentCollaborationConnection, principalId: string, signal: AbortSignal): Promise<void> {
    throwIfCancelled(signal, "Revoking a remote Gama collaboration member was cancelled");
...
class RemoteDocumentCollaborationConnection extends DisposableOwner implements DocumentCollaborationConnection {
  ...
  async updatePresence(selection: DocumentSelection | undefined, signal: AbortSignal): Promise<void> {
    if (this.disposed) throw new ReferenceError("Remote Gama collaboration connection is disposed");
  ...
  private async pollPresence(): Promise<void> {
    ...
    while (!this.disposed) {
      ...
      try {
        ...
        if (this.disposed || polling.signal.aborted) return;
        const failure = error instanceof Error ? error : new Error("Remote Gama collaboration presence updates failed");
        if (!isRetryablePollFailure(failure)) {
          this.failureEmitter.fire(failure);
          return;
        }
  ...
  private heartbeatPresence(): void {
    ...
    void this.updatePresence(this.presence, new AbortController().signal).catch(error => {
      if (!this.disposed) this.failureEmitter.fire(error instanceof Error ? error : new Error("Remote Gama collaboration presence heartbeat failed"));
    });
  }
}
...
function decodeInvite(value: Readonly<Record<string, unknown>>, expectedRoomId: string): DocumentCollaborationInvite {
  ...
  if (roomId !== expectedRoomId) throw new TypeError("Remote Gama collaboration invitation belongs to a different room");
  return Object.freeze({
    roomId,
...
function decodeSelection(value: string): DocumentSelection {
  ...
  try {
    ...
    throw new TypeError("Remote Gama collaboration presence selection must contain JSON");
  }
  ...
  switch (selection.kind) {
    ...
    default: throw new TypeError("Remote Gama collaboration presence selection has an unknown kind");
  }
}
...
function validateDisplayName(value: string): string {
  if (typeof value !== "string" || value.trim().length === 0) throw new TypeError("Remote Gama collaboration invitation display name must be non-empty");
  return value.trim();
}

function validatePrincipalId(value: string): string {
  if (!/^[A-Za-z0-9_-]{1,128}$/.test(value)) throw new TypeError("Remote Gama collaboration member principalId must contain between 1 and 128 letters, numbers, '-' or '_'");
  return value;
}
...
function validateRoomRole(value: unknown): DocumentCollaborationRoomRole {
  ...
  throw new TypeError("Remote Gama collaboration invitation role must be owner, editor, or viewer");
}
