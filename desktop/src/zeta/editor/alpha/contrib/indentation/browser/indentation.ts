export interface AlphaIndentationGuide {
  readonly columnIndex: number;
  readonly level: number;
}

/** Returns one guide at every complete visual indentation unit in leading whitespace. */
export function createAlphaIndentationGuides(text: string, tabSize: number): readonly AlphaIndentationGuide[] {
  ...
  const guides: AlphaIndentationGuide[] = [];
  let visualColumn = 0;
