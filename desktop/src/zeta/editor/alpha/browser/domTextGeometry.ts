...
/**
 * Resolves logical UTF-16 offsets against one rendered Alpha text fragment.
 *
 ...
 */
export function createAlphaDomTextRange(element: HTMLElement, startOffset: number, endOffset: number): Range | undefined {
  ...
  if (!isOffset(startOffset, textLength) || !isOffset(endOffset, textLength) || endOffset < startOffset) {
    throw new RangeError("Alpha DOM text range offsets must be ordered UTF-16 positions");
  }
...
export function getAlphaDomTextCaretLeft(element: HTMLElement, offset: number, relativeTo: HTMLElement): number | undefined {
  const range = createAlphaDomTextRange(element, offset, offset);
...
export function getAlphaDomTextOffsetAtClientPoint(element: HTMLElement, clientX: number, clientY: number): number | undefined {
