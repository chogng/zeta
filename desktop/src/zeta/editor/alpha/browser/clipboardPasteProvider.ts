...
export interface AlphaClipboardTextTransfer {
  readonly types: readonly string[];
  getText(type: string): string;
}

/** Extends Alpha paste with one declared clipboard representation. */
export interface AlphaClipboardPasteProvider {
  readonly id: string;
  readonly mimeTypes: readonly string[];
  providePaste(transfer: AlphaClipboardTextTransfer): string | undefined | PromiseLike<string | undefined>;
}

/** Creates a transferable-safe snapshot before a clipboard event returns to the browser. */
export function captureAlphaClipboardTextTransfer(clipboardData: DataTransfer): AlphaClipboardTextTransfer {
...
export function normalizeAlphaClipboardPasteProviders(providers: readonly AlphaClipboardPasteProvider[] | undefined): readonly AlphaClipboardPasteProvider[] {
  ...
  return Object.freeze(providers.map((provider: AlphaClipboardPasteProvider) => {
...
export async function provideAlphaClipboardPaste(providers: readonly AlphaClipboardPasteProvider[], transfer: AlphaClipboardTextTransfer): Promise<string | undefined> {
...
export const AlphaUriListPasteProvider: AlphaClipboardPasteProvider = Object.freeze({
  id: "alpha.uri-list",
  mimeTypes: Object.freeze(["text/uri-list"]),
  providePaste: (transfer: AlphaClipboardTextTransfer): string | undefined => {
