...
import { type EditorFoldingModel } from "../language/common/folding.js";
import { TextPosition, type TextRange } from "../common/text.js";
...
import { AlphaFoldedVisualLineProjection } from "./foldedVisualLineProjection.js";
import { type AlphaBracketColorizationSource } from "../language/browser/bracketColorizationPresentation.js";
import { AlphaLineWidthIndex } from "./lineWidthIndex.js";
import { createAlphaIndentationGuides } from "./indentationGuides.js";
import { AlphaGpuMinimapRenderer } from "./gpuMinimapRenderer.js";
...
import { projectAlphaSemanticTokenLine, type AlphaResolvedSemanticToken, type AlphaSemanticTokenSource } from "../language/browser/semanticTokenPresentation.js";
import { type AlphaBracketColorizationSpan } from "../language/browser/semanticTokenPresentation.js";
...
export class AlphaEditorViewport extends DisposableOwner {
  ...
  private readonly foldedVisualLineProjection: AlphaFoldedVisualLineProjection;
  private readonly foldingModel: EditorFoldingModel | undefined;
  ...
  constructor(options: AlphaEditorViewportOptions) {
    ...
    this.foldingModel = options.foldingModel;
    ...
    this.foldedVisualLineProjection = this.own(new AlphaFoldedVisualLineProjection(
      this.visualLineProjection,
      options.foldingModel,
    ));
    const viewport = this.own(new EditorViewportModel(this.model, {
      lineHeight: options.lineHeight,
      overscanLineCount: options.overscanLineCount,
      lineSource: this.foldedVisualLineProjection.lineSource,
    }));
    this.viewport = viewport;
    this.onDidChangeLayout = viewport.onDidChange;
    this.own(this.foldedVisualLineProjection.onDidChange(() => this.project(viewport.layout)));
  ...
  private reconcileLines(layout: EditorViewportLayout): void {
    const visualProjection = this.visualProjection;
    const visualProjectionRevision = this.foldedVisualLineProjection.revision;
    if (
    ...
    for (
      ...
      if (!existing || this.renderedVisualProjectionRevision !== visualProjectionRevision) {
        line.numberElement.textContent = visualLine.firstForLogicalLine
          ...
          : "";
        this.projectFoldingToggle(line, visualLine.logicalLineIndex, visualLine.firstForLogicalLine);
      }
      if (
        !existing ||
        this.renderedModelVersion !== layout.modelVersion ||
  ...
  private projectFoldingToggle(line: AlphaRenderedLine, logicalLineIndex: number, firstForLogicalLine: boolean): void {
    const region = firstForLogicalLine
      ? this.foldingModel?.regions.find(candidate => candidate.startLineIndex === logicalLineIndex)
      : undefined;
    line.foldingElement.hidden = !region;
    if (!region) return;
    line.foldingElement.dataset.logicalLineIndex = String(logicalLineIndex);
    line.foldingElement.classList.toggle("collapsed", region.collapsed);
    line.foldingElement.setAttribute("aria-expanded", String(!region.collapsed));
    line.foldingElement.setAttribute("aria-label", region.collapsed ? "Expand folded lines" : "Collapse lines");
    line.foldingElement.textContent = region.collapsed ? "›" : "⌄";
  }
  ...
  private get visualProjection() {
    return this.foldedVisualLineProjection.ensureCurrent();
  }
