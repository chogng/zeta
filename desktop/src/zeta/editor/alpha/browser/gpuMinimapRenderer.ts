import { type AlphaMinimapRow } from "./minimapProjection.js";
...
/**
 * GPU-backed density projection for Alpha's minimap.
 *
...
export class AlphaGpuMinimapRenderer {
  ...
  private constructor(
    ...
    if (positionLocation < 0 || !colorLocation || !vertexBuffer) {
      ...
      throw new Error("Alpha GPU minimap could not initialize its rendering resources");
    }
  ...
  resize(width: number, height: number): void {
    if (!Number.isFinite(width) || width < 0 || !Number.isFinite(height) || height < 0) {
      throw new RangeError("Alpha GPU minimap size must be non-negative and finite");
    }
...
function createProgram(context: WebGLRenderingContext, vertexSource: string, fragmentSource: string): WebGLProgram {
  ...
  if (!program) throw new Error("Alpha GPU minimap could not create a WebGL program");
  ...
  if (!context.getProgramParameter(program, context.LINK_STATUS)) {
    context.deleteProgram(program);
    throw new Error("Alpha GPU minimap could not link its WebGL program");
  }
  return program;
}
...
function compileShader(context: WebGLRenderingContext, kind: number, source: string): WebGLShader {
  ...
  if (!shader) throw new Error("Alpha GPU minimap could not create a WebGL shader");
