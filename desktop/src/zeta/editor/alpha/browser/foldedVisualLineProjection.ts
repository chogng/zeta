...
import { type EditorViewportLineSource } from "../common/viewport.js";
import { AlphaVisualLineProjection } from "./visualLineProjection.js";

/** Filters Alpha's wrapped visual rows through one editor-owned folding model. */
export class AlphaFoldedVisualLineProjection extends DisposableOwner {
  ...
  constructor(private readonly source: AlphaVisualLineProjection, private readonly folding: EditorFoldingModel | undefined) {
    super();
    ...
    if (folding) this.own(folding.onDidChange(() => this.rebuild()));
  }
  ...
  private visibleLogicalLines(lineCount: number): readonly boolean[] {
    ...
    if (!folding) return Object.freeze(Array.from({ length: lineCount }, () => true));
    return Object.freeze(Array.from({ length: lineCount }, (_, lineIndex) => !folding.isLineHidden(lineIndex)));
  }
}
...
function createVisualLineAnchors(source: EditorVisualLineProjection, visibleLogicalLines: readonly boolean[], lines: readonly { readonly logicalLineIndex: number; readonly firstForLogicalLine: boolean; readonly lastForLogicalLine: boolean }[]): readonly number[] {
  ...
  for (let logicalLineIndex = 0; logicalLineIndex < source.logicalLineCount; logicalLineIndex += 1) {
    if (visibleLogicalLines[logicalLineIndex]) {
      ...
      if (previousVisible < 0) throw new Error("A folded visual-line projection must retain the first logical line");
