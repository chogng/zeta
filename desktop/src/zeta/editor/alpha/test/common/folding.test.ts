...
test("Folding model hides only folded bodies and toggles header ranges", () => {
  ...
  assert.deepEqual(folding.getVisibleLineIndexes(), [0, 4]);
  assert.equal(folding.isLineHidden(0), false);
  assert.equal(folding.isLineHidden(1), true);
  ...
  assert.deepEqual(folding.getVisibleLineIndexes(), [0, 1, 2, 3, 4]);
});
