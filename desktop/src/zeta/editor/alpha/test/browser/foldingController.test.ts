...
test("Folding controller routes platform chords and gutter toggles through the front-end folding model", () => {
  ...
  const toggle = requiredElement<HTMLButtonElement>(viewport.element, ".zeta-alpha-editor-fold-toggle");
...
test("Folding controller routes macOS Command+K chords without accepting Control", () => {
  ...
  using viewport = new AlphaEditorViewport({ container, model, lineHeight: 20, textMeasurer: new FixedTextMeasurer(), selectionController: selections, foldingModel: folding });
  const input = dom.window.document.createElement("textarea") as unknown as HTMLTextAreaElement;
  viewport.element.append(input);
...
function renderedLogicalLines(root: ParentNode): readonly string[] {
  return [...root.querySelectorAll<HTMLElement>(".zeta-alpha-editor-line")].map(line => line.dataset.logicalLineIndex!);
}
