...
test("Remove-brackets shortcut mutates through an isolated Alpha transaction", () => {
  ...
  using controller = new AlphaBracketEditingController(input, viewport, selections, matcher);
...
test("Bracket editing controller rejects cross-model wiring and preserves unsupported chords", () => {
  ...
  assert.throws(() => new AlphaBracketEditingController(input, viewport, selections, otherMatcher), /must share one text model/);
