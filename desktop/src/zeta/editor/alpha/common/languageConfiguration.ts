...
function normalizeComments(comments: LanguageConfiguration["comments"]): NormalizedLanguageCommentConfiguration | null | undefined {
  ...
  const blockComment = comments.blockComment === undefined || comments.blockComment === null
    ...
    : normalizeMatchingPair(comments.blockComment, "Language block comment");
  return Object.freeze({
...
function normalizeBrackets(brackets: LanguageConfiguration["brackets"]): readonly LanguageCharacterPair[] | null | undefined {
  ...
  for (const pair of normalized) {
    if (opens.has(pair.open) || closes.has(pair.close)) {
      throw new RangeError("Language bracket open and close tokens must be unique");
    }
    opens.add(pair.open);
...
function normalizePairs(pairs: readonly LanguageCharacterPair[] | null | undefined, owner: string): readonly LanguageCharacterPair[] | null | undefined {
  ...
  const normalized = pairs.map(pair => normalizeCharacterPair(pair, owner));
  const opens = new Set<string>();
  for (const pair of normalized) {
...
function normalizeMatchingPair(pair: LanguageCharacterPair, owner: string): LanguageCharacterPair {
  const normalized = normalizeCharacterPair(pair, owner);
  if (normalized.open === normalized.close) throw new RangeError(`${owner} open and close tokens must differ`);
  return normalized;
}

function normalizeCharacterPair(pair: LanguageCharacterPair, owner: string): LanguageCharacterPair {
  ...
  const close = normalizeToken(pair.close, `${owner} close token`);
