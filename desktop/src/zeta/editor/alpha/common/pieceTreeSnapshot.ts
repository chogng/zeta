...
export function createTextBufferSnapshot(
  segments: readonly TextBufferSnapshotSegment[],
  length: number,
  lineCount: number,
): TextBufferSnapshot {
  const capturedSegments = Object.freeze(
    segments.map(segment => Object.freeze({ ...segment })),
  );
  return Object.freeze({
    length,
    lineCount,
    getText: () => capturedSegments
      .map(segment => segment.source.slice(
        segment.startOffset,
...
function readTextBetweenOffsets(
  segments: readonly TextBufferSnapshotSegment[],
  length: number,
  startOffset: number,
  endOffset: number,
  ...
  for (const segment of segments) {
    ...
    if (segmentEndOffset > startOffset && segmentStartOffset < endOffset) {
      parts.push(segment.source.slice(
        segment.startOffset +
          Math.max(startOffset, segmentStartOffset) -
          segmentStartOffset,
        segment.startOffset +
          Math.min(endOffset, segmentEndOffset) -
          segmentStartOffset,
      ));
    }
