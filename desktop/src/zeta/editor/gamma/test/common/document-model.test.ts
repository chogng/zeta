...
import { createDefaultDocumentSchema, DocumentSchema } from "../../common/schema.js";
...
import { createAcademicDocumentSchema, createEmptyAcademicDocument } from "../../academic/common/schema.js";
import { DocumentModel } from "../../common/model.js";
import { createDocumentDecoration, DocumentDecorationSet } from "../../common/decoration.js";
import { buildDocumentOutline } from "../../common/documentOutline.js";
import { documentContentSize, documentNodeSize, documentPointToPosition, documentPositionToPoint, resolveDocumentPosition } from "../../common/documentPosition.js";
import { documentSelectionToText } from "../../common/documentText.js";
import { extractDocumentFragment } from "../../common/fragment.js";
import { createDocumentPlugin, DocumentPluginKey } from "../../common/plugin.js";
import { deserializeDocument, deserializeDocumentFragment, DocumentSerializationError, serializeDocument, serializeDocumentFragment } from "../../common/serialization.js";
import { allSelection, nodeSelection, textSelection } from "../../common/selection.js";
import { DocumentTransaction } from "../../common/transaction.js";
import { deserializeDocumentTransaction, serializeDocumentTransaction } from "../../common/transactionSerialization.js";
...
test("Gamma input matching is supplied by the active profile", () => {
  ...
  assert.equal(matchDocumentEditorProfiles({ resource: URI.file("C:\\project\\paper.zeta-academic") }, [academicProfile]), EditorPaneMatch.Default);
  assert.equal(findDocumentEditorProfile({ resource: URI.file("C:\\project\\paper.txt") }, [academicProfile]), undefined);
});

test("Gamma profile materialization keeps schema and browser extensions together", () => {
...
test("DocumentSchema creates and validates block, inline, and code nodes", () => {
...
test("DocumentModel updates plugin state across edits, history, and reset", () => {
  ...
  using model = new DocumentModel(schema, createDocument(schema), { plugins: [plugin] });
  ...
  model.reset(resetDocument);
  assert.deepEqual(model.getPluginState(key), { origins: ["user", "undo", "redo", "reset"], selections: ["text"], versions: [1, 2, 3, 4, 5] });
});

test("DocumentModel keeps document and plugin state unchanged when a plugin rejects a transaction", () => {
  const schema = createDefaultDocumentSchema();
...
test("DocumentModel exposes plugin-owned decoration sources without merging identities", () => {
  const schema = createDefaultDocumentSchema();
  const document = createDocument(schema);
  ...
  using model = new DocumentModel(schema, document, { plugins: [plugin] });
...
test("Gamma maps decoration ranges through one transaction and drops ranges with no text", () => {
  const schema = createDefaultDocumentSchema();
...
test("Gamma converts nested text points to absolute positions with stable boundary bias", () => {
  const schema = createDefaultDocumentSchema();
  const paragraph = schema.createNode("paragraph", {
    id: "paragraph-inline",
    content: [
...
test("DocumentModel applies text transactions and preserves transaction-level undo", () => {
  const schema = createDefaultDocumentSchema();
...
test("DocumentModel maps implicit selections through text edits and node removal", () => {
  const schema = createDefaultDocumentSchema();
  const first = schema.createNode("paragraph", { id: "paragraph-1", content: [schema.createText("Hello", { id: "text-1" })] });
  const second = schema.createNode("paragraph", { id: "paragraph-2", content: [schema.createText("World", { id: "text-2" })] });
  using model = new DocumentModel(schema, schema.createDocument([first, second], "document-1"), { selection: textSelection({ nodeId: "text-1", offset: 5 }) });
...
test("DocumentModel coalesces adjacent transactions with the same history group", () => {
  const schema = createDefaultDocumentSchema();
  const paragraph = schema.createNode("paragraph", { id: "paragraph-1", content: [schema.createText("a", { id: "text-1" })] });
  using model = new DocumentModel(schema, schema.createDocument([paragraph], "document-1"), { selection: textSelection({ nodeId: "text-1", offset: 1 }) });
...
test("DocumentSchema supports child groups, cardinality, and valid multi-step assembly", () => {
  ...
  using model = new DocumentModel(defaultSchema, defaultSchema.createDocument([], "assembly-document"));
...
test("Gamma Academic schema composes title, abstract, and section wrappers", () => {
...
test("Gamma builds an outline across nested structured nodes", () => {
  const schema = createAcademicDocumentSchema();
...
test("Citation reference command creates and appends a bibliography", () => {
  ...
  using model = new DocumentModel(schema, document, { plugins: [createReferenceIndexPlugin()] });
...
test("DocumentModel handles insertion, attributes, movement, and deletion as one history unit", () => {
  const schema = createDefaultDocumentSchema();
  const document = createDocument(schema);
...
test("DocumentModel rejects invalid transactions without partial mutation", () => {
  const schema = createDefaultDocumentSchema();
  using model = new DocumentModel(schema, createDocument(schema));
...
test("Gamma serializes every transaction step and preserves transport metadata", () => {
  const schema = createDefaultDocumentSchema();
  const document = createDocument(schema);
...
test("DocumentModel applies remote transactions outside local history", () => {
  const schema = createDefaultDocumentSchema();
...
test("Gamma converts inline and cross-block selections into clipboard text", () => {
  const schema = createDefaultDocumentSchema();
...
test("Gamma supports whole-document clipboard text, fragments, replacement, and deletion", () => {
  const schema = createDefaultDocumentSchema();
  ...
  using model = new DocumentModel(schema, schema.createDocument([targetParagraph], "target-document"), { selection });
...
test("Gamma extracts, serializes, and inserts structured clipboard fragments", () => {
  const schema = createDefaultDocumentSchema();
  ...
  using model = new DocumentModel(schema, schema.createDocument([target], "target-document"));
...
test("Gamma block commands split, join, move, and insert through model transactions", () => {
  const schema = createDefaultDocumentSchema();
  const first = schema.createNode("paragraph", {
    id: "paragraph-1",
    content: [schema.createText("Hello", { id: "text-1" })],
  ...
  using model = new DocumentModel(schema, schema.createDocument([first, second], "document-1"));
...
test("Gamma moves a block down to the requested sibling index", () => {
  const schema = createDefaultDocumentSchema();
  const blocks = ["one", "two", "three"].map((text, index) => schema.createNode("paragraph", {
    id: `paragraph-${index + 1}`,
    content: [schema.createText(text, { id: `text-${index + 1}` })],
  }));
  using model = new DocumentModel(schema, schema.createDocument(blocks, "document-1"));
...
test("Gamma inline mark commands split text runs and preserve the selection", () => {
  const schema = createDefaultDocumentSchema();
  const paragraph = schema.createNode("paragraph", {
    id: "paragraph-1",
    content: [schema.createText("Hello", { id: "text-1" })],
...
test("Gamma stores collapsed mark toggles for subsequent text insertion", () => {
  const schema = createDefaultDocumentSchema();
...
test("Gamma link mark commands set, update, remove, and undo link attributes", () => {
  const schema = createDefaultDocumentSchema();
  const paragraph = schema.createNode("paragraph", {
    id: "paragraph-1",
    content: [
...
test("Gamma block commands preserve inline runs while splitting and joining", () => {
  const schema = createDefaultDocumentSchema();
  const paragraph = schema.createNode("paragraph", {
    id: "paragraph-1",
    content: [
...
test("Gamma structural block commands toggle blockquotes and insert horizontal rules", () => {
  const schema = createDefaultDocumentSchema();
  const first = schema.createNode("paragraph", {
    id: "paragraph-1",
...
test("Gamma replaces and undoes a text selection spanning sibling blocks", () => {
  const schema = createDefaultDocumentSchema();
...
test("Gamma pastes multiline text across sibling blocks", () => {
  const schema = createDefaultDocumentSchema();
  const blocks = ["First", "Middle", "Third"].map((text, index) => schema.createNode("paragraph", {
    id: `paragraph-${index + 1}`,
    content: [schema.createText(text, { id: `text-${index + 1}` })],
...
test("Gamma inline commands handle selections spanning multiple text runs", () => {
  const schema = createDefaultDocumentSchema();
  const paragraph = schema.createNode("paragraph", {
    id: "paragraph-1",
    content: [
...
test("Gamma paste commands turn multiline text into sibling blocks", () => {
  const schema = createDefaultDocumentSchema();
  const paragraph = schema.createNode("paragraph", {
    id: "paragraph-1",
    content: [schema.createText("Hello", { id: "text-1" })],
...
test("Gamma splits a list paragraph into sibling list items", () => {
  const schema = createDefaultDocumentSchema();
  const paragraph = schema.createNode("paragraph", {
    id: "paragraph-1",
  ...
  using model = new DocumentModel(schema, schema.createDocument([list], "document-1"));
...
test("Gamma joins, indents, and outdents list items as atomic commands", () => {
  const schema = createDefaultDocumentSchema();
...
test("Gamma exits an empty list item at the list boundary", () => {
  const schema = createDefaultDocumentSchema();
...
test("Gamma block format commands change block and list types without replacing content", () => {
  const schema = createDefaultDocumentSchema();
  const paragraph = schema.createNode("paragraph", { id: "paragraph-1", content: [schema.createText("Hello", { id: "text-1" })] });
  using model = new DocumentModel(schema, schema.createDocument([paragraph], "document-1"));
...
test("Gamma inserts validated tables and inline images", () => {
  const schema = createDefaultDocumentSchema();
  const paragraph = schema.createNode("paragraph", { id: "paragraph-1", content: [schema.createText("Hello", { id: "text-1" })] });
  using model = new DocumentModel(schema, schema.createDocument([paragraph], "document-1"));
...
test("Gamma inserts images at text selections while preserving inline runs", () => {
  const schema = createDefaultDocumentSchema();
  const paragraph = schema.createNode("paragraph", {
    id: "paragraph-1",
    content: [
...
test("Gamma deletes adjacent inline nodes through common boundary commands", () => {
  const schema = createDefaultDocumentSchema();
  const paragraph = schema.createNode("paragraph", {
    id: "paragraph-1",
    content: [
...
test("Gamma deletes and restores a selected inline node", () => {
  const schema = createDefaultDocumentSchema();
  ...
  using model = new DocumentModel(schema, schema.createDocument([paragraph], "document-1"), { selection: nodeSelection(image.id) });
...
test("Gamma inserts hard breaks and deletes selections spanning inline nodes", () => {
  const schema = createDefaultDocumentSchema();
  const paragraph = schema.createNode("paragraph", {
    id: "paragraph-1",
    content: [
...
test("Gamma navigates table cells and applies row and column transactions", () => {
  const schema = createDefaultDocumentSchema();
