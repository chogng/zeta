...
import type { DocumentNodeId } from "../common/document.js";
import type { DocumentOutline } from "../common/documentOutline.js";
...
export class DocumentOutlineNavigator extends DisposableOwner {
  ...
  constructor(private readonly options: DocumentOutlineNavigatorOptions) {
    super();
    const element = options.ownerDocument.createElement("nav");
    element.className = "zeta-document-outline";
    element.hidden = true;
    element.setAttribute("aria-label", "Document outline");
    const title = options.ownerDocument.createElement("div");
    title.className = "zeta-document-outline-title";
    title.textContent = "Outline";
    const list = options.ownerDocument.createElement("ol");
  ...
  setOutline(outline: DocumentOutline): void {
    ...
    const fragment = this.options.ownerDocument.createDocumentFragment();
    for (const entry of outline) {
      const item = this.options.ownerDocument.createElement("li");
      item.className = "zeta-document-outline-item";
      const button = this.options.ownerDocument.createElement("button");
      button.type = "button";
