# Gamma structured editor

Gamma owns the structured-document domain. Its `common` layer is DOM-free and
does not depend on Workbench, Electron, Alpha, or an external editor runtime.
...
- schema-validated block-type transactions and a Gamma-owned block toolbar for
...
- selection-aware inline image insertion/rendering; image files pasted into a
  Gamma text surface are read as data URLs, replace the active text selection,
...
- profile-owned inline atomic nodes through `inlineNodeViews` and generic
  ...
  actions without adding citation semantics to Gamma common;
...
- common plain-text extraction for inline and cross-block selections, with
  rich-surface copy/cut routed through Gamma's model transactions;
...
- Gamma-owned structured clipboard fragments with a versioned custom MIME
...
The document model is deliberately separate from Alpha's line-oriented
`TextModel`. The browser pane owns block projection, while
`DocumentWorkingCopy` adapts Gamma serialization, dirty/revert/conflict state,
and untitled Save As to the shared Workbench working-copy contract.
`AlphaEmbeddedTextEditorFactory` supplies the controlled Alpha line editor
inside `codeBlock` nodes without leaking Gamma document types into Alpha.
...
`DocumentEditorPaneOptions.schema` lets a product provide the corresponding
...
Gamma child projections.
`DocumentEditorPaneOptions.inlineNodeViews` is the atomic-inline counterpart:
...
`DocumentEditorPaneOptions.createEmptyDocument` is the matching lifecycle
...
`DocumentEditorProfile` groups the profile matcher, schema factory, empty-document
...
working-copy, and embedded line-editor services. The Gamma contribution selects
...
paragraphs, marks, selection, and input remain Gamma-owned child projections.
...
images use Gamma's run-based `contenteditable` surface; each rendered run keeps
...
last Gamma snapshot. Shift+Enter is projected
...
Gamma clipboard fragments are extracted from the common selection model and
...
Gamma plugins are common-layer state extensions, not browser contributions.
...
changes that do not modify the document. `DocumentEditorPaneOptions.plugins`
...
`DocumentEditorPane` resolves its identity-based ranges to text-node spans,
