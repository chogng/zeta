...
/** Schema and content validator for Gamma's structured document tree. */
...
function defaultNodeSpecs(): Readonly<Record<string, DocumentNodeSpec>> {
  ...
  return {
    ...
    codeBlock: { kind: "block", allowedChildren: ["text"], defaultAttrs: { language: "text" }, validateAttributes: attrs => validateStringAttribute(attrs, "language", false) },
