...
import { LanguageAnalysisService } from "../../../alpha/common/languageAnalysisService.js";
...
test("vendored VS Code JSON grammar tokenizes through the common service", async () => {
  const content = await readFile(resolve("src/zeta/editor/textmate/browser/grammars/JSON.tmLanguage.json"), "utf8");
  using registry = new TextMateGrammarRegistry();
