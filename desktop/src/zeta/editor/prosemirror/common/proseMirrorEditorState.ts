...
function requiredMark(name: string): MarkType {
  ...
  if (!mark) {
    throw new ReferenceError(
      `ProseMirror document schema is missing mark '${name}'`,
    );
  }
  return mark;
}

function documentFromText(text: string): ProseMirrorNode {
  const lines = text.replaceAll("\r\n", "\n").split("\n");
  const paragraph = proseMirrorDocumentSchema.nodes.paragraph;
  if (!paragraph) {
    throw new ReferenceError(
      "ProseMirror document schema is missing paragraph nodes",
    );
  }
  ...
  return proseMirrorDocumentSchema.topNodeType.create(
    undefined,
    content.length > 0 ? content : [paragraph.create()],
  );
}
