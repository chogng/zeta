...
test("Aster remote collaboration uses authenticated HTTP without App Server or session state", async () => {
  ...
  globalThis.fetch = (async (input, init) => {
    ...
    if (request.url.endsWith("/rooms/open")) {
      return jsonResponse({ clientId: "client-a", schemaId: "aster-document-v1", snapshot: { roomId: "aster-remote", version: 0, document: serializeDocument(document, schema) } });
    }
    if (request.url.endsWith("/rooms/submit")) {
      const transaction = new DocumentTransaction().replaceText("text-1", 0, 0, "A");
      return jsonResponse({ status: "accepted", update: { roomId: "aster-remote", clientId: "client-a", sequence: 1, baseVersion: 0, version: 1, transaction: serializeDocumentTransaction(transaction, schema) } });
    }
  ...
  try {
    ...
    using connection = await service.open({ clientId: "client-a", schemaId: "aster-document-v1", schema, document, target: { kind: "remote", endpoint: "https://collaboration.zeta.example", bearerToken: TOKEN } }, new AbortController().signal);
    const transaction = new DocumentTransaction().replaceText("text-1", 0, 0, "A");
    ...
    assert.equal(connection.roomId, "aster-remote");
...
test("Aster remote collaboration delivers ordered long-poll updates to its connection", async () => {
  const schema = createDefaultDocumentSchema();
  const document = createDocument(schema);
  ...
  globalThis.fetch = ((input, init) => {
    const request = new Request(input, init);
    if (request.url.endsWith("/rooms/open")) return Promise.resolve(jsonResponse({ clientId: "client-a", schemaId: "aster-document-v1", snapshot: { roomId: "aster-remote", version: 0, document: serializeDocument(document, schema) } }));
    ...
    return new Promise<Response>((resolve, reject) => {
      ...
      resolvePoll = () => {
        ...
        resolve(jsonResponse({ status: "updates", updates: [{ roomId: "aster-remote", clientId: "client-b", sequence: 1, baseVersion: 0, version: 1, transaction: serializeDocumentTransaction(remoteTransaction, schema) }] }));
      };
    });
  }) as typeof fetch;
  try {
...
test("Aster remote collaboration retries a transient long-poll transport failure", async () => {
  const schema = createDefaultDocumentSchema();
  const document = createDocument(schema);
  const remoteTransaction = new DocumentTransaction().replaceText("text-1", 0, 0, "R");
  let polls = 0;
  ...
  globalThis.fetch = ((input, init) => {
    ...
    if (polls === 2) return Promise.resolve(jsonResponse({ status: "updates", updates: [{ roomId: "aster-remote", clientId: "client-b", sequence: 1, baseVersion: 0, version: 1, transaction: serializeDocumentTransaction(remoteTransaction, schema) }] }));
    return new Promise<Response>((_resolve, reject) => request.signal.addEventListener("abort", () => reject(new DOMException("Aborted", "AbortError")), { once: true }));
  }) as typeof fetch;
  try {
    using service = new RemoteDocumentCollaborationService();
...
test("Aster remote collaboration publishes local presence and projects remote selections", async () => {
  const schema = createDefaultDocumentSchema();
  const document = createDocument(schema);
  ...
  try {
    ...
    assert.deepEqual(await publish?.json(), { roomId: "aster-remote", clientId: "client-a", selection: JSON.stringify({ kind: "text", anchor: { nodeId: "text-1", offset: 1 }, head: { nodeId: "text-1", offset: 1 } }) });
...
test("Aster remote collaboration exposes a viewer room as read-only", async () => {
  const schema = createDefaultDocumentSchema();
  const document = createDocument(schema);
  const originalFetch = globalThis.fetch;
  globalThis.fetch = ((input, init) => {
    const request = new Request(input, init);
    if (request.url.endsWith("/rooms/open")) return Promise.resolve(jsonResponse({ clientId: "client-viewer", schemaId: "aster-document-v1", snapshot: { roomId: "aster-remote", version: 0, document: serializeDocument(document, schema) }, canEdit: false, canManageMembers: false }));
    return new Promise<Response>((_resolve, reject) => request.signal.addEventListener("abort", () => reject(new DOMException("Aborted", "AbortError")), { once: true }));
  }) as typeof fetch;
  try {
    using service = new RemoteDocumentCollaborationService();
    using connection = await service.open({ clientId: "client-viewer", schemaId: "aster-document-v1", schema, document, target: { kind: "remote", endpoint: "https://collaboration.zeta.example", bearerToken: TOKEN } }, new AbortController().signal);
...
test("Aster remote collaboration owners create typed room invitations", async () => {
  const schema = createDefaultDocumentSchema();
  const document = createDocument(schema);
  const requests: Request[] = [];
  const originalFetch = globalThis.fetch;
  globalThis.fetch = ((input, init) => {
    const request = new Request(input, init);
    requests.push(request);
    if (request.url.endsWith("/rooms/open")) return Promise.resolve(jsonResponse({ clientId: "client-owner", principalId: "owner-1", schemaId: "aster-document-v1", snapshot: { roomId: "aster-remote", version: 0, document: serializeDocument(document, schema) }, canEdit: true, canManageMembers: true }));
    if (request.url.endsWith("/rooms/invites")) return Promise.resolve(jsonResponse({ roomId: "aster-remote", principalId: "member-1", displayName: "Writer", role: "editor", accessToken: "member-token" }));
    if (request.url.endsWith("/aster-remote/members")) return Promise.resolve(jsonResponse({ members: [{ principalId: "owner-1", displayName: "Owner", role: "owner" }, { principalId: "member-1", displayName: "Writer", role: "editor" }] }));
    if (request.url.endsWith("/rooms/members/rotate-token")) return Promise.resolve(jsonResponse({ roomId: "aster-remote", principalId: "member-1", displayName: "Writer", role: "editor", accessToken: "rotated-token" }));
    if (request.url.endsWith("/rooms/members/revoke")) return Promise.resolve(new Response(null, { status: 204 }));
    return new Promise<Response>((_resolve, reject) => request.signal.addEventListener("abort", () => reject(new DOMException("Aborted", "AbortError")), { once: true }));
  }) as typeof fetch;
  try {
    using service = new RemoteDocumentCollaborationService();
    using connection = await service.open({ clientId: "client-owner", schemaId: "aster-document-v1", schema, document, target: { kind: "remote", endpoint: "https://collaboration.zeta.example", bearerToken: TOKEN } }, new AbortController().signal);
    ...
    assert.deepEqual(invite, { roomId: "aster-remote", principalId: "member-1", displayName: "Writer", role: "editor", accessToken: "member-token" });
    ...
    assert.deepEqual(await request?.json(), { roomId: "aster-remote", displayName: "Writer", role: "editor" });
    const rotateRequest = requests.find(candidate => candidate.url.endsWith("/rooms/members/rotate-token"));
    assert.deepEqual(await rotateRequest?.json(), { roomId: "aster-remote", principalId: "member-1" });
    const revokeRequest = requests.find(candidate => candidate.url.endsWith("/rooms/members/revoke"));
    assert.deepEqual(await revokeRequest?.json(), { roomId: "aster-remote", principalId: "member-1" });
  } finally {
    globalThis.fetch = originalFetch;
  }
});
