...
const { createAlphaDomTextRange, getAlphaDomTextCaretLeft, getAlphaDomTextOffsetAtClientPoint, getAlphaDomTextRangeRectangles } = await import("../../browser/view/domTextGeometry.js");
...
test("DOM text geometry keeps one UTF-16 offset space across syntax spans", () => {
  ...
  const range = createAlphaDomTextRange(text, 1, 5);
  assert.equal(range?.toString(), "b😊c");
  assert.throws(() => createAlphaDomTextRange(text, 5, 1), /ordered UTF-16/);
  ...
  assert.equal(getAlphaDomTextOffsetAtClientPoint(text, 20, 40), 2);
  dom.window.close();
});
...
test("DOM text geometry preserves browser visual rectangles for mixed-direction rendering", () => {
  ...
  assert.deepEqual(getAlphaDomTextRangeRectangles(text, 0, 3, line), [
    { left: 50, width: 20 },
    { left: 20, width: 15 },
  ]);
  assert.equal(getAlphaDomTextCaretLeft(text, 3, line), 35);
  dom.window.close();
});
