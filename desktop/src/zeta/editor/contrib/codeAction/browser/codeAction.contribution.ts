...
registerEditorContribution({ id: "editor.contrib.codeAction", install: context => {
  ...
  const service = context.own(context.languageFeaturesService.createCodeActionService(context.model));
  ...
  context.own(new CodeActionController(context.textInput.element, context.viewport, context.selections, service, diagnostics, context.languageId, context.onLanguageError));
} });
