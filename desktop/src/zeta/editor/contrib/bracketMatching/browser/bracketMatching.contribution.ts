...
registerEditorContribution({ id: "editor.contrib.bracketMatching", configure: context => {
  const matcher = context.own(new LanguageBracketMatcher(context.model, context.languageId, context.configurations));
  const lexicalContext = context.own(new LanguageLexicalContextIndex(context.model, context.languageId, context.configurations));
