...
import { type IDebugBreakpoint, type IDebugConfiguration, type IDebugScope, type IDebugSession, type IDebugStackFrame, type IDebugVariable, type DebugSessionState } from "../common/debugService.js";
...
export class DebugAdapterSession extends DisposableOwner implements IDebugSession {
  ...
  private constructor(readonly configuration: IDebugConfiguration, private readonly processService: IDebugAdapterProcessService, sessionId: string, private readonly breakpoints: () => readonly IDebugBreakpoint[], private readonly runInTerminal: DebugAdapterSessionStartOptions["runInTerminal"], private readonly updateBreakpoints: DebugAdapterSessionStartOptions["updateBreakpoints"]) {
    super();
  ...
  static async start(options: DebugAdapterSessionStartOptions): Promise<DebugAdapterSession> {
    ...
    const session = new DebugAdapterSession(options.configuration, options.processService, sessionId, options.breakpoints, options.runInTerminal, options.updateBreakpoints);
    try {
  ...
  async stackTrace(): Promise<readonly IDebugStackFrame[]> {
    const body = record((await this.request("stackTrace", { threadId: await this.requireThreadId(), startFrame: 0, levels: 100 })).body, "stackTrace body");
  ...
  async disconnect(): Promise<void> {
    if (this._state !== "terminated") {
      try { await this.request("disconnect", { restart: false, terminateDebuggee: true }); } catch (error) { this.outputEmitter.fire(`Debug disconnect failed: ${message(error)}\n`); }
    }
  ...
  private async requireThreadId(): Promise<number> {
    ...
    const first = array(body.threads, "threads").map((value, index) => record(value, `threads[${index}]`))[0];
    if (!first) throw new Error("The Debug Adapter did not report any threads");
    this._threadId = positiveInteger(first.id, "threads[0].id");
