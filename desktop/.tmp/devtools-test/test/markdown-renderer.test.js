...
import { MarkdownElement, renderWorkbenchMarkdown, sanitizeMarkdownHtmlToString, } from "../src/base/browser/markdownRenderer.js";
import { MarkdownPreview, } from "../src/platform/markdown/browser/markdownPreview.js";
import { MarkdownDocumentView, } from "../src/workbench/contrib/markdown/browser/markdownDocumentRenderer.js";
function createDom() {
...
test("MarkdownPreview sanitizes content before creating iframe srcdoc", () => {
    ...
    assert.doesNotMatch(preview.element.srcdoc, /href\s*=\s*["']javascript:/i);
    preview.dispose();
    dom.window.close();
});
test("MarkdownPreview validates iframe link messages before emitting", () => {
    ...
    const channel = preview.element.getAttribute("data-zeta-webview-channel");
    assert.ok(channel);
