...
import { BROWSER_VIEW_CREATE_CHANNEL, BROWSER_VIEW_LAYOUT_CHANNEL, BROWSER_VIEW_NAVIGATE_CHANNEL, BROWSER_VIEW_VISIBILITY_CHANNEL, normalizeBrowserViewUrl, validateBrowserViewCreateRequest, validateBrowserViewLayoutRequest, validateBrowserViewNavigateRequest, validateBrowserViewVisibilityRequest, } from "../src/platform/browser/common/browserView.js";
import { browserViewIpcRoutes, } from "../src/platform/browser/electron-main/browserViewIpc.js";
...
test("browser view validators admit secure and loopback navigation", () => {
    assert.equal(normalizeBrowserViewUrl("https://example.com/path"), "https://example.com/path");
    assert.equal(normalizeBrowserViewUrl("http://localhost:3000/path"), "http://localhost:3000/path");
    assert.equal(normalizeBrowserViewUrl("about:blank"), "about:blank");
    assert.deepEqual(validateBrowserViewNavigateRequest({
        targetId,
...
test("browser view validators reject privileged URLs and malformed geometry", () => {
    ...
    assert.throws(() => validateBrowserViewCreateRequest({
        url: "https://example.com",
        preload: "unsafe.js",
    }));
    assert.throws(() => validateBrowserViewLayoutRequest({
        targetId,
    ...
    assert.throws(() => validateBrowserViewVisibilityRequest({
...
test("browser view IPC routes delegate only validated commands", async () => {
    ...
    assert.throws(() => navigate.validate({
        targetId,
        url: "file:///secret.txt",
    }));
});
