import { validateKeybindingsResource, } from "../common/keybindingsResource.js";
 ...
 */
export async function migrateLegacyKeybindings(configuration, keybindings) {
    const configurationSnapshot = configuration.read();
    if (!Object.hasOwn(configurationSnapshot.document.values, legacyConfigurationKey)) {
        return false;
    }
    const migrated = validateKeybindingsResource(configurationSnapshot.document.values[legacyConfigurationKey]);
    const keybindingsSnapshot = keybindings.read();
    if (keybindingsSnapshot.bindings.length === 0 &&
        migrated.length > 0) {
