...
import { isSafeMarkdownLink, sanitizeMarkdownHtmlToString, } from "../../../base/browser/markdownRenderer.js";
...
export class MarkdownPreview extends DisposableOwner {
    #ownerDocument;
    #webview;
    #onDidOpenLink = this.own(new Emitter());
    #active = true;
    element;
    onDidOpenLink = this.#onDidOpenLink.event;
    constructor(options) {
        super();
        ...
        this.own(this.#webview.onDidMessage((message) => {
            ...
            if (openLink)
                this.#onDidOpenLink.fire(openLink.href);
        }));
        this.defer(() => {
            this.#active = false;
        });
    ...
    setMarkdown(markdown) {
        ...
        this.#webview.setHtml(`<style>${PREVIEW_STYLE}</style>` +
...
function validateOpenLinkMessage(message) {
