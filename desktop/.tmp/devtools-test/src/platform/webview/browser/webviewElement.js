...
export class WebviewElement extends DisposableOwner {
    #onDidMessage = this.own(new Emitter());
    #channel;
    #active = true;
    element;
    onDidMessage = this.#onDidMessage.event;
    constructor(options) {
        super();
        ...
        element.setAttribute("title", validateTitle(options.title ?? "Webview"));
        element.style.border = "0";
        element.style.display = "block";
        element.style.width = "100%";
        element.style.height = "100%";
        element.srcdoc = createWebviewDocument(this.#channel, validateHtml(options.initialHtml ?? ""));
        this.own(addDisposableListener(targetWindow, "message", (event) => {
            if (event.source !== element.contentWindow)
                return;
            const envelope = validateEnvelope(event.data, this.#channel);
            if (envelope)
                this.#onDidMessage.fire(envelope.message);
        }));
        this.defer(() => {
            this.#active = false;
            element.srcdoc = "";
    ...
    setHtml(html) {
        this.#requireActive();
        this.element.srcdoc = createWebviewDocument(this.#channel, validateHtml(html));
    }
    /**
    ...
    postMessage(message, transfer = []) {
        if (!this.#active || !this.element.contentWindow)
            return false;
...
function createWebviewDocument(channel, html) {
    const bootstrap = `(() => {
...
function validateEnvelope(value, channel) {
