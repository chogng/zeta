...
export function resolveBrowserWindowOptions({ state, webPreferences, platform = process.platform, }) {
...
export function applyWindowState(target, state) {
...
export function validateWindowState(state, displays, workbenchState) {
    ...
    if (state.mode === WindowMode.Fullscreen && state.displayId !== undefined) {
        const fullscreenDisplay = usableDisplays.find(({ display }) => display.id === state.displayId);
    ...
    const intersectsCurrentDisplay = usableDisplays.some(({ area }) => x + stateWidth > area.x &&
