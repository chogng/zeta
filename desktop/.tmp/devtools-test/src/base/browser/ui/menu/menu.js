import { Separator, SubmenuAction, } from "../../../common/actions.js";
...
import { ButtonActionViewItem, SeparatorActionViewItem, } from "../actionbar/actionViewItems.js";
...
class MenuActionViewItem extends ButtonActionViewItem {
    ...
    render(container) {
        ...
        if (this.action.checked === undefined) {
            ...
            this.button.element.setAttribute("aria-checked", String(this.action.checked));
...
function createMenuActionViewItem(action, options = {}) {
...
export class Menu extends DisposableOwner {
    ...
    constructor(options) {
        ...
        for (const action of options.actions) {
            const item = this.own(createMenuActionViewItem(action, {
        ...
        this.own(addDisposableListener(element, "keydown", (event) => {
            ...
            switch (event.key) {
                case "ArrowDown":
                    moveFocus(element, FocusNavigationDirection.Forward, FocusNavigationBoundary.Wrap);
                    break;
                case "ArrowUp":
                    moveFocus(element, FocusNavigationDirection.Backward, FocusNavigationBoundary.Wrap);
                    break;
                case "Home":
                    focusFirst(element);
                    break;
                ...
                case "ArrowRight":
                    {
                        ...
                        const submenu = this.#submenus.find((item) => item.ownsTrigger(activeElement));
                        if (submenu)
                            submenu.openFromKeyboard();
                        else
                            handled = false;
                    }
                    break;
