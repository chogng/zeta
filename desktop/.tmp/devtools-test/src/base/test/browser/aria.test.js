...
test("AriaLiveRegion announces repeated status and alert messages", async () => {
    ...
    assert.deepEqual([...dom.window.document.querySelectorAll(".zeta-aria-status")]
        .map((element) => element.textContent), ["Ready", ""]);
    region.status("Ready");
    await nextAnimationFrame(dom.window);
    assert.deepEqual([...dom.window.document.querySelectorAll(".zeta-aria-status")]
        .map((element) => element.textContent), ["", "Ready"]);
    ...
    region.clear();
    assert.equal(dom.window.document.querySelector(".zeta-aria-live")?.textContent, "");
    region.dispose();
    assert.equal(dom.window.document.querySelector(".zeta-aria-live"), null);
    dom.window.close();
});
