...
export const IConfigurationService =
  createServiceIdentifier<IConfigurationService>("configurationService");

export const CONFIGURATION_READ_CHANNEL =
  "zeta:configuration:read";
export const CONFIGURATION_UPDATE_CHANNEL =
  "zeta:configuration:update";
export const CONFIGURATION_CHANGED_CHANNEL =
  "zeta:configuration:changed";
...
/**
 * Narrow host capability for Desktop configuration.
 *
 * Reads and notifications remain `unknown` across the contextBridge boundary;
 * the workbench validates them before use.
 */
...
export function validateConfigurationUpdateRequest(
  value: unknown,
): IConfigurationUpdateRequest {
...
export function validateConfigurationRead(
  value: unknown,
): undefined {
  if (value !== undefined) {
...
function exactRecord(
  ...
  if (
    ...
    throw new Error(
      `configuration object must contain exactly: ${
        expected.join(", ")
      }`,
    );
  }
