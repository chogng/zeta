...
```ts
onNotification(definition, listener, options)
```
...
* 清理全部 timeout
* 清理全部 abort listener
* abort 所有正在执行的入站 request handler
* 后续 `request()` 立即失败
* 后续 `notify()` 立即失败
...
* handler 返回非法结果
* Rust 发送取消通知
* handler context signal 被 abort
* 同一个 request 只响应一次
* 多个 handler 并发执行
