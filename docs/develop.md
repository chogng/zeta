...
本文正在设计一个系统。`ash.md`、`intent.md`、`spec.md` 和 `plan.md` 是该系统未来读取或产生的开发产物，不用于把当前系统设计拆成四份自我描述的文档。
...
本文先固定产品语义和责任边界，再定义上下文、Agent 专业化、Team、恢复和实施顺序。当前全部内容均为计划设计，不代表 Ash 已经具备该产品流程。
...
- 不从 Ash 当前的 Session、Thread 或其他实现结构反向定义开发方法。
...
| --- | --- | --- | --- | --- |
| `ash.md` | 项目或目录 | 人与 Agent 共同维护 | 项目负责人 | 每次进入项目必须知道什么 |
...
`ash.md` 对应项目级 Agent 开发上下文，包含构建与验证入口、长期约束、架构入口以及 Agent 容易重复犯下的错误。它不是某一次开发工作的意图，也不复制全部系统文档。
...
| D5 `.ash` 固定产物 | 待开始 | 根据真实使用结果确定项目知识和工作产物的文件契约 | 发现、版本、权限、迁移和多工作并存通过端到端验证 |
...
- 项目级 `ash.md` 与现有 AGENTS、instructions、skills 和领域文档如何保持单一权威；
- 工作产物最终使用普通文件、持久化领域对象还是两者绑定；
- 私有 Agent 的最大深度、并发、预算和超时由什么策略决定；
- 什么真实使用证据足以把 `.ash` 文件格式从计划设计改为稳定契约。
...
- Ash 服务于已经存在的开发活动，不能反过来让开发适应当前实现。
