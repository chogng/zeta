...
通用 `marketplace/*` API。
`zeta-code/docs/design/tui.md` 也明确要求 Plugin domain projection 进入 canonical App Server contract 后，TUI 才能增加管理 feature。TUI 已有可复用的 tabs/search/selection 组件，但当前没有 Plugin view model 或 `/plugins` command；
