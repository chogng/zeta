...
### 4.1 Current `zeta-rmcp-client` 拥有
...
Current tools-only runtime 保持单 crate、private modules 和显式 public export：
...
### Phase M0：低层 contract 与 fixture（tools 范围完成）
...
### Phase M3：Streamable HTTP + OAuth（基础 transport 部分具备）
