...
| Element、Scene、Interaction、Animation、Retained Runtime | `zeterm/crates/zui` | zeterm-owned crates in root workspace | 已迁入 zeterm |
| Button、Tree、List、Editor/Sidebar presentation | `zeterm/crates/ui`、`editor`、`agent-sidebar` | zeterm-owned crates in root workspace | 已迁入 zeterm |
| Renderer、wgpu、winit | `zeterm/crates/renderer`、`wgpu`、`winit` | zeterm-owned crates in root workspace | 已迁入 zeterm |
...
│   └── crates/                 # native-only UI crates, staged in phase two
├── zeta-rs/
...
`zeterm/crates`。现在由仓库根 Cargo workspace 统一解析所有 crate，zeterm 仍作为独立 package 和发布
边界存在；Bazel 使用同一个 root graph。

### 阶段二：分离 Native UI workspace（核心迁移已完成）

- [x] 将 `zui`、`zeta-ui`、`zeta-renderer`、`zeta-wgpu`、`zeta-winit` 迁入 `zeterm/crates`；
...
- [x] 为 `zeterm` 建立发布输入和 boundary CI graph；`//zeterm:zeterm_ci` 验证 workspace ownership，
      ...
      Rust build graph；
- [x] 建立 `zeterm/crates` 与 `zeta-rs` shared crates 的 metadata-derived hermetic Bazel Rust graph；
...
- [x] 接入 provider-neutral 的 platform signing/verification job；`scripts/release_zeterm_package.sh`
...
- 不在旧 Native 中修建新功能；若迁移过程中发现缺少通用能力，先在正确的下层 owner 实现，再由
  zeterm 做最小接线。
- 不把 `zeterm/crates/editor`、`zeterm/crates/agent-sidebar` 等 presentation crate 直接误判为 shared backend；
...
签名发布流程使用 `scripts/release_zeterm_package.sh`，发布细节见
