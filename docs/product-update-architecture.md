...
`.pkg` 或 `.dmg` 必须提交公证；Ash Code 的 `.zip` 提交公证但无法附加离线票据，Desktop 的
...
Remote runtime 的下载、兼容握手、安装与回滚继续由 `ash-remote-connections` 拥有。它可以复用签名
...
- Windows 每个 Ash 可执行文件和最终安装器通过 `signtool verify /pa /all /v`，且签名包含
