...
Zeta 已经具备可持续运行、调用工具、等待批准、运行中追问、自动与手动压缩上下文、恢复执行、usage/资源预算治理、按模型校准未来预算、模型输入逐项限幅、冻结的统一代码编辑工具面、durable 计划和委托 Codex 的 Agent Loop。后续建设不再以“搭出第一轮模型调用”为目标，而是优先建立端到端评测，再扩展 Codex、Skills、MCP 与多 Agent 的产品完整性。
