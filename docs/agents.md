...
| 会继承主模型吗？ | 没有“主模型”这一特殊概念。默认使用启动基线：会话入口取会话已解析模型，委托运行取调用方已解析模型，工作流取工作流基线；定义可显式覆盖。 |
...
| App Server | 合并 catalog；按会话、委托或工作流筛选定义；解析模型、工具和能力；创建冻结运行快照 | 维护来源专属选择分支、把内置定义变成设置项、保存前端状态 |
...
- `model` 决定使用本次启动已经解析的模型基线还是明确覆盖。会话入口的基线来自会话，委托的基线来自调用方，工作流的基线来自工作流；不存在单独的“主模型”。
...
创建会话入口或委托 Thread 前，App Server 必须冻结启动来源、定义来源、定义版本、内容摘要、提示词摘要、最终模型、工具集合、Skill 集合、能力上限、上下文输入和选择原因。恢复旧 Thread 时继续使用冻结值，不能因应用升级或定义变化改写历史执行身份。
...
| 模型 | 默认使用调用方已解析的模型作为启动基线 | 定义显式覆盖时使用覆盖值；不可用就失败，不自动替换。 |
| 推理等级与服务等级 | 默认使用调用方的模型调用配置作为启动基线 | 定义显式覆盖后也受调用方预算和产品策略限制。 |
...
- **失败即关闭**：提示词缺失、摘要不一致、模型不可用、必需工具缺失、能力越权、上下文来源无效或定义版本未知时，不启动 Agent。
...
当前代码里“委托定义未声明模型时使用调用方当前模型”和“未传上下文时使用 `fresh`”已经存在，但会话入口还不能选择并冻结 Agent 定义，内置 catalog 也尚未实现。后续应让会话、委托和工作流复用同一 `AgentDefinition` 解析契约；委托继续复用既有子 Thread、上下文种子和工具/Skill 冻结机制，不建立第二套运行时。
