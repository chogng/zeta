...
> 状态：Current contract。本文是 `zui`、`zeta-ui` 与 `zeterm` product host 之间的 Native UI 编写和样式边界；具体 crate API 由 [`zui` README](../zui/README.md) 和 [`zeta-ui` README](../ui/README.md) 维护。
...
本文适用于 `zeterm/zui` 的 backend-neutral UI contract、`zeterm/ui` 的可复用 Native 组件，以及 `zeterm` 和领域 crate 的产品 presentation adapter。
...
| Native host 的 pane/product composition | [`zeterm` 文档导航](README.md) 与对应 domain crate README |
