...
/// A request to execute one stable zeterm command.
///
...
impl CommandRequest {
    ...
    pub const fn new(command_id: ZetermCommandId) -> Self {
        Self { command_id }
    }

    /// Returns the stable command identity carried by this request.
    pub const fn command_id(self) -> ZetermCommandId {
        self.command_id
    }
}

impl From<ZetermCommandId> for CommandRequest {
    fn from(command_id: ZetermCommandId) -> Self {
        Self::new(command_id)
    }
}
