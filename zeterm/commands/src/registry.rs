...
pub enum CommandRegistryError {
    ...
    AlreadyRegistered(ZetermCommandId),
    /// The request refers to a command without a registered handler.
    NotRegistered(ZetermCommandId),
}
...
pub struct CommandRegistry<Context> {
    handlers: HashMap<ZetermCommandId, CommandHandler<Context>>,
}
...
impl<Context> CommandRegistry<Context> {
    ...
    pub fn handler(&self, command_id: ZetermCommandId) -> Option<CommandHandler<Context>> {
    ...
    pub fn contains(&self, command_id: ZetermCommandId) -> bool {
