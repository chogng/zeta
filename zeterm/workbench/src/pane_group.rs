...
impl PaneGroup {
    ...
    pub fn activate(&mut self, pane: PaneId) -> bool {
    ...
    pub fn split_active(&mut self, direction: PaneSplitDirection) -> PaneId {
    ...
    pub fn close_active(&mut self) -> Option<PaneId> {
    ...
    pub fn grid_node(&self, bounds: Rect) -> GridNode<PaneId, PaneSplitId> {
        self.root.grid_node(bounds)
    }

    /// Resolves the current PaneTree into leaf and sash geometry for one frame.
    pub fn layout(&self, bounds: Rect) -> PaneGroupLayout<PaneId, PaneSplitId> {
