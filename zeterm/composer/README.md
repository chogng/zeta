...
`zeta-composer` owns zeterm's Composer-local state and presentation contracts: multiline input,
...
owner; the zeterm host adapts product state and executes returned submissions or model selections.
...
| Thread/history and App Server catalog adaptation | `zeterm/src/composer_host.rs` | ✅ host-owned |
| Submission/model-selection side effects | `zeterm` host | ✅ host-owned |
| Scene painting, accessibility registration and workspace toolbar | `zeterm/src/composer_panel.rs` | ✅ host-owned |
| Renderer, window and platform event APIs | lower framework/backend and host adapters | ❌ |

The dependency direction is:

```text
zeterm host → zeta-composer → zeta-editor / zeta-input-classifier / zeta-slash-commands
                             → zeta-ui → zui
```
