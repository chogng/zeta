...
pub(crate) struct RemoteConnectionManager<'a> {
    viewport: Rect,
    panel: Rect,
...
impl<'a> RemoteConnectionManager<'a> {
    ...
    pub(crate) fn caret_bounds(&self, field: RemoteConnectionManagerField) -> Option<Rect> {
        match field {
    ...
    fn interaction_regions(&self) -> Vec<InteractionRegion> {
        ...
        let mut regions = vec![
            ...
            button_region(
                ...
                "Connect in a new zeterm window",
                navigation,
    ...
    fn paint_content(&self, scene: &mut UiScene) {
        ...
        draw_text(
            scene,
            "Targets are credential-free; zeterm invokes your local OpenSSH configuration.",
            Rect::from_xywh(
                self.panel.origin.x + CONTENT_INSET,
...
#[cfg(test)]
#[path = "remote_connection_manager_view_tests.rs"]
mod tests;
