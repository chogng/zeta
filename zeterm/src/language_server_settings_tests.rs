...
use super::{
    ...
    LanguageServerSettings, LanguageServerSettingsState, LanguageServerSettingsTarget,
};
...
#[test]
fn missing_preference_projects_the_disabled_product_default() {
    let mut settings = LanguageServerSettingsState::default();

    settings.synchronize(&configuration(2, 3, None));

    assert_eq!(settings.mode(), LanguageServerModeDto::Disabled);
    assert_eq!(settings.switch_selection(), SwitchSelection::Off);
    assert!(settings.executable_input().text().is_empty());
    assert!(!settings.can_reset());
}

#[test]
fn switch_enables_automatic_mode_and_can_disable_the_service_again() {
    let mut settings = LanguageServerSettingsState::default();
    ...
    assert_eq!(settings.switch_selection(), SwitchSelection::On);
...
#[test]
fn closing_settings_hides_the_settings_without_changing_selection() {
    let mut settings = LanguageServerSettingsState::default();
    settings.synchronize(&configuration(1, 1, None));
    settings.open();
    settings.set_enabled(true);
...
fn visible_settings_register_a_modal_input_and_save_action() {
    ...
    assert!(nodes.iter().any(|node| {
        node.id == LANGUAGE_SERVER_SWITCH && node.role == AccessibilityRole::Button
    }));
}
