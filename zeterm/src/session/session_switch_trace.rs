...
pub(crate) fn enabled() -> bool {
    *ENABLED.get_or_init(|| {
        matches!(
            std::env::var("ZETERM_SESSION_TRACE").as_deref(),
            Ok("1") | Ok("true") | Ok("yes")
        )
    })
}
...
pub(crate) fn frames_enabled() -> bool {
    enabled()
        && *FRAMES_ENABLED.get_or_init(|| {
            matches!(
                std::env::var("ZETERM_SESSION_TRACE_FRAMES").as_deref(),
                Ok("1") | Ok("true") | Ok("yes")
            )
        })
}
...
pub(crate) fn event(id: Option<SwitchId>, label: &'static str, details: impl Display) {
    ...
    match id {
        Some(id) => eprintln!(
            "[zeterm session-trace] switch={} event={} {}",
            id.0, label, details
        ),
        None => eprintln!("[zeterm session-trace] event={} {}", label, details),
    }
}
...
impl Drop for Span {
    fn drop(&mut self) {
        ...
        match self.id {
            Some(id) => eprintln!(
                "[zeterm session-trace] switch={} phase={} elapsed_us={}",
                id.0,
                self.label,
                elapsed.as_micros()
            ),
            None => eprintln!(
                "[zeterm session-trace] phase={} elapsed_us={}",
                self.label,
                elapsed.as_micros()
            ),
        }
