...
fn narrow_viewport_temporarily_hides_the_expanded_sidebar() {
    ...
    assert!(!sidebar.is_visible_for(MINIMUM_WIDTH + MINIMUM_MAIN_WIDTH - 1.0));
    assert!(sidebar.is_visible_for(MINIMUM_WIDTH + MINIMUM_MAIN_WIDTH));
}
...
fn resizing_clamps_the_sidebar_and_persists_across_visibility() {
    ...
    assert_eq!(sidebar.preferred_width(), MINIMUM_WIDTH);
    ...
    assert!(sidebar.finish_resizing());
    assert_eq!(sidebar.preferred_width(), MAXIMUM_WIDTH);
}
