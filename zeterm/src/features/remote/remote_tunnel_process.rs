//! zeterm's adapter for the product-neutral Remote Tunnel host.
//!
//! The lifecycle supervisor lives in `zeta-remote-host`. This module only translates its typed
//! events into zeterm's `NativeEvent` channel and keeps the host owned by the native application.
...
/// zeterm-owned adapter around the shared Remote Tunnel supervisor.
