...
Invoke-Checked python @('-B', 'scripts/cargo.py', 'build', '-p', 'zeta-windows-sandbox', '--bin', 'zeta-windows-sandbox', '--locked', '--target', $Target)
Invoke-Checked python @('-B', 'scripts/cargo.py', 'build', '-p', 'zeta-network-proxy', '--example', 'probe', '--locked', '--target', $Target)
$bin = Join-Path $output 'bin'
New-Item -ItemType Directory -Force -Path $bin | Out-Null
$binary = Join-Path $bin 'zeta-windows-sandbox.exe'
Copy-Item -LiteralPath (Join-Path $workspace ".build/cargo/$Target/debug/zeta-windows-sandbox.exe") -Destination $binary
$env:ZETA_WINDOWS_SANDBOX_BIN = $binary
$env:ZETA_NETWORK_PROBE = Join-Path $workspace ".build/cargo/$Target/debug/examples/probe.exe"
...
try {
    ...
    Invoke-Checked python @('-B', 'scripts/cargo.py', 'test', '-p', 'zeta-windows-sandbox', '--lib', '--test', 'windows', '--locked', '--target', $Target, '--', '--include-ignored', '--test-threads=1')
} catch {
    $failure = $_
} finally {
