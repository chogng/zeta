...
test("development Server Host restart selects the new executable after stopping", async () => {
	...
	await restartDevelopmentAppServer(supervisor, launcher, "/test/ash-app-server.123.0");
	...
	assert.deepEqual(lifecycle, ["stop", "start:/test/ash-app-server.123.0"]);
});
...
test("development Server Host restart restores the previous generation after failure", async () => {
	...
	await assert.rejects(restartDevelopmentAppServer(supervisor, launcher, "/test/ash-app-server.123.0"), /new generation failed/u);
