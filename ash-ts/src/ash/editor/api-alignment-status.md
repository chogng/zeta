...
| 文件 | 声明 | 此前结论（已撤回） |
| --- | --- | --- |
| `browser/controller/editContext/editContext.ts` | `AbstractEditContext` | 已进入 `ViewPart` 生命周期并统一剪贴板事件、输入路由和组合输入状态；抽象层仍保留 Ash 的公共输入契约，需继续缩小与上游声明成员的差异 |
