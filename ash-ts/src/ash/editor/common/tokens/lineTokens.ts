...
import { ColorId, FontStyle, type ITokenPresentation, MetadataConsts, StandardTokenType, TokenMetadata } from '../encodedTokenAttributes.js';
...
export class LineTokens implements IViewLineTokens {
	...
	public _lineTokensBrand: void = undefined;
	...
	public constructor(tokens: Uint32Array, text: string, decoder: ILanguageIdCodec) {
	...
	/** Find the token containing the offset. */
...
class SliceLineTokens implements IViewLineTokens {
	...
	public constructor(source: LineTokens, startOffset: number, endOffset: number, deltaOffset: number) {
		this._source = source;
...
/** A sequence of token lengths and metadata values. */
export class TokenArray {
	...
	public toLineTokens(lineContent: string, decoder: ILanguageIdCodec): LineTokens {
		return LineTokens.createFromTextAndMetadata(this.map((range, token) => ({ text: range.substring(lineContent), metadata: token.metadata })), decoder);
	}

	public forEach(callback: (range: OffsetRange, tokenInfo: TokenInfo) => void): void {
		let lengthSum = 0;
		for (const tokenInfo of this._tokenInfo) {
			const range = new OffsetRange(lengthSum, lengthSum + tokenInfo.length);
			callback(range, tokenInfo);
			lengthSum += tokenInfo.length;
		}
	}

	public map<T>(callback: (range: OffsetRange, tokenInfo: TokenInfo) => T): T[] {
		const result: T[] = [];
		let lengthSum = 0;
		for (const tokenInfo of this._tokenInfo) {
			const range = new OffsetRange(lengthSum, lengthSum + tokenInfo.length);
			result.push(callback(range, tokenInfo));
			lengthSum += tokenInfo.length;
		}
		return result;
	}
	...
	public slice(range: OffsetRange): TokenArray {
		...
		for (const tokenInfo of this._tokenInfo) {
			...
			const tokenEndExclusive = tokenStart + tokenInfo.length;
			if (tokenEndExclusive > range.start) {
				if (tokenStart >= range.endExclusive) {
					break;
				}
				const deltaBefore = Math.max(0, range.start - tokenStart);
				const deltaAfter = Math.max(0, tokenEndExclusive - range.endExclusive);
	...
	public append(other: TokenArray): TokenArray {
		return TokenArray.create(this._tokenInfo.concat(other._tokenInfo));
	}
}
...
export function getStandardTokenTypeAtPosition(model: ITextModel, position: IPosition): StandardTokenType | undefined {
	...
	if (!model.tokenization.isCheapToTokenize(lineNumber)) return undefined;
	model.tokenization.forceTokenization(lineNumber);
	const lineTokens = model.tokenization.getLineTokens(lineNumber);
	return lineTokens.getStandardTokenType(lineTokens.findTokenIndexAtOffset(position.column - 1));
}
